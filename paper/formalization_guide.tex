...
\usepackage{xr-hyper}
\usepackage{hyperref}
\externaldocument[R-]{blowup_density_revised}[blowup_density_revised.pdf]
...
\code{85e3a25e006c35636f0e53b0e9296caca2685bc0}. Build in Linux:
\begin{verbatim}
lake -d verification exe cache get
lake -d verification build
\end{verbatim}
The build checks 37 current publication interfaces. The separate declaration
...
All 26 numbered statements and the numbered remark appear below and are Closed.
...
\coverage{Closed}\source{B/LocalForceTheory}{16}{localForceTheory}{theorem};
\source{B/LocalTheoryV2}{93}{localTheoryV2}{def};
...
\source{B/ContinuationV3}{29}{continuationV3_holds}{theorem};
\source{B/TorusLocalTheoryV2}{48}{torusLocalTheoryV2_holds}{theorem};
\source{F/Section4/A04/H1Restart}{261}{h1RestartR}{theorem};
\source{F/Section3/T11/H1Restart}{576}{h1RestartT}{theorem}.
Local existence, full local regularity, maximality, uniqueness and integral
continuation hold without global time-integrability or temporal support
assumptions. The additional fixed-force $H^1$ restart interfaces retain
the density force classes. \\
...
\mapped{Proposition}{prop:multiple} & \coverage{Closed}\source{F/Section3/T24/MultipleAssembly}{51}{multipleRegionsStatement_holds}{theorem};
\source{F/Section3/T24/MultipleOmegaAssembly}{59}{multipleRegionsOmegaStatement_holds}{theorem};
\source{B/MultipleRegionsV2}{167}{multipleRegionsStatementV2_holds}{theorem}.
Both torus and arbitrary nonempty bounded open domain branches, with homogeneous no-slip on the latter. \\
\mapped{Proposition}{prop:conservative} & \coverage{Closed}\source{F/Section3/T24/ConservativeAssembly}{45}{conservativeForcing}{theorem};
\source{F/Section3/T24/ConservativeOmega}{127}{conservativeForcingOmega}{theorem}.
Periodic potentials on the torus. On any nonempty bounded open domain with homogeneous no-slip, the potential need only be smooth near the closed spatial slab during the lifespan; no boundary regularity is required. \\
...
\section{Formalized scope and proof methods}\label{guide:methods}
Section~\ref{guide:correspondence} records the full formalized statements.
The comparisons below describe the original proof routes retained by the
density theorems and the broader results now available.

\paragraph{Local theory and continuation (Proposition~\paperref{prop:local}).}
Proposition~\paperref{prop:local} is formalized for locally smooth forcing on
both domains, without global time-integrability or compact temporal support
assumptions. The theorem \code{localForceTheory} provides local existence
with full Sobolev regularity and pressure recovery, maximal development,
uniqueness and the squared-$H^2$ integral continuation criterion. Whole-space
forcing is represented by smooth future Sobolev paths; periodic forcing is
represented by smooth extensions on each finite closed future slab.
The whole-space proof localizes the force in time, while the periodic proof
uses force bounds on a finite time window. Force agreement and uniqueness
transfer the constructed solution and its continuation to the original force.

The downstream density proofs continue to invoke the original $H^7$ local
construction on $\mathbb R^3$ and $H^3$ construction on the torus for their
density force classes. Their continuation argument applies the high-order
energy estimate and Gr\"onwall to obtain
\[
 \sup_{t<S}\|u(t)\|_{H^m}
 \le\left(\|a\|_{H^m}+\int_0^S\|f(t)\|_{H^m}\,dt\right)
 \exp\!\left(C_{m,\nu}\int_0^S\|u(t)\|_{H^2}^2\,dt\right)<\infty.
\]
Taking $m=7$ or $3$ gives a bounded set of restart data. A uniform positive
restart time for the fixed force on a compact time window then allows
uniqueness to glue the solution across $S$.
Tao's Theorems~5.1 and~5.4~\cite{tao} start with a contraction in the $H^1$
mild-solution space and propagate higher regularity on the same interval;
Theorem~5.4(iv), p.~53, treats smooth data in all Sobolev orders.

Fixed-force $H^1$-uniform restart and endpoint continuation are also formally
proved for the density force classes, using the enstrophy inequality and a
uniform ODE barrier. The original $H^7/H^3$ route is recorded in
\source{F/Section4/A04/RestartFixedForce}{41}{restartFixedForce_of_memForceR}{theorem},
\source{F/Section4/A04/ShiftedExtension}{247}{extendsBeyond_of_memForceR'}{theorem}
and \source{F/Section3/T11/ExistenceInputH3}{456}{extendsBeyondH3}{theorem};
the registered $H^1$ conclusions appear in the correspondence row for
Proposition~\paperref{prop:local}.

\paragraph{Bounded domains (Propositions~\paperref{prop:multiple} and~\paperref{prop:conservative}).}
Both bounded-domain statements are formalized for every nonempty bounded
open set $\Omega$. Proposition~\paperref{prop:multiple} uses the article's
finite superposition of building blocks supported in disjoint interior balls.
Proposition~\paperref{prop:conservative} uses the vanishing force--velocity
pairing and the energy identity from rest. The required integration-by-parts
identity is proved on arbitrary bounded open sets; the potential need only
be smooth near $[0,T)\times\overline\Omega$.
...
the resulting estimate, including both endpoints, is
\source{F/Section3/T13/Assembly}{330}{localizationAPI}{theorem}.
...
$L^2_tH^{-1/2}_x$ case. The density proofs retain their original route through the formalized
critical energy inequalities, a smallness bootstrap and the high-order
continuation argument above. See the proof locations for
