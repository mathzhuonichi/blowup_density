...
\coverage{Closed}\source{B/LocalTheoryV2}{93}{localTheoryV2}{def};
...
Proved for the used density force classes, including the fixed-force
$H^1$-uniform restart and endpoint clauses on both domains. \\
...
Both torus and bounded box-or-regular-level domain branches, with homogeneous no-slip on the latter. \\
...
\source{F/Section3/T24/ConservativeOmega}{126}{conservativeForcingOmega}{theorem}.
Periodic potentials on the torus and smooth potentials on bounded box-or-regular-level domains with homogeneous no-slip. \\
...
The stronger fixed-force $H^1$-uniform restart is also proved directly on the
maximal smooth solution.  With $Y=\|u\|_{H^1}^2$ and
$Z=\|u\|_{H^2}^2$, the formalized enstrophy estimate has the form
\[
 Y' + \nu Z \le C_\nu(1+Y)^3 + C_\nu\|f(t)\|_{L^2}^2.
\]
A uniform ODE barrier chooses a time depending only on $\nu$, the $H^1$ ball,
and the force cap on $[0,S+1]$; endpoint integration and the existing integral
continuation criterion then give a common restart interval.  This stronger
result does not change the article's continuation proof, which already uses
the proved high-order route. Tao's higher-regularity argument
...
the registered $H^1$ conclusions are the four sources in the correspondence
row above.
