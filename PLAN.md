...
| 205-A01-coordinate-tame | 已合并（#210）；补审在 sol 上三次停滞，放弃——其内容由 207 的对抗式审稿（ACCEPT-WITH-NOTES，含变异）覆盖 | 09-16 0512Z | #210 | — | A01 **CylinderCoordinateTame**：逐坐标柱面 Kato–Ponce/Moser 估计——word Leibniz 展开 + 柱面/下降 tame 插值引理 + 光滑逼近到相容有限元；允许 re-cut 到角不变子空间；先证存在常数 |
| 206-A01-smooth-tame | 已合并（#211）；补审在 sol 上三次停滞，放弃——由 207 的对抗式审稿覆盖 | 09-16 0512Z | #211 | — | A01 **SmoothCylinderCoordinateTame**（A3-M2 最后一条分析输入）：先走角不变 re-cut + 下降到 R³ 用 A03 tame 积（若成立则整个 A01 只剩接线，并导出无条件构造器探针）；退路是柱面插值本体或孤立精确的 R³ tame 不等式 |
| 207-A01-tame-assembly | 已合并（#212，wave 40 门禁绿；**A3-M2 闭合，A01 经典解构造器在集成分支上无分析输入（存在正视界）**） | 09-16 0500Z | #212 | — | A01 **tame 装配**（路线 A）：反向混合积估计 + Leibniz word 识别 + 组合求和 ⇒ SmoothCylinderCoordinateTame（显式 C q）⇒ 换位子界 ⇒ ForcingFamilyBound ⇒ FiniteMildEnergy ⇒ hb 无条件 ⇒ A01 无条件构造器探针；若 206 审稿推荐不变类 re-cut（路线 B）则改走 B |
| 208-A01-datum-horizon | 已合并（#213，wave 41 门禁绿；LocalSolution.lean 在集成分支） | 09-16 0512Z | #213 | — | A01 **合同注册 1/3**：初值识别 velocity(0,·)=a（连续 + a.e.）、initialClassR 桥、总函数 localHorizon 与 LocalTheoryAPI 的 solution 字段 |
| 209-A01-manuscript-regularity | 已合并（#214，wave 42 门禁绿；ManuscriptRegularity.lean 在集成分支） | 09-16 0518Z | #214 | — | A01 **合同注册 2/3**：ManuscriptLocalRegularity 四字段（全阶 C∞ 数据路径经 hslice；eq:Rpressure 含 t=0 由 189/194 的 Leray 余项恒等式；projected 由 momentum；径向势规范） |
| 210-A01-horizon-uniform | 已合并（#215）；事后审稿 REJECT 针对本 lane 自身声称（物理 H⁷ 界未在本 lane 证出、报告陈述过时）——已由 211 证出并在 REPORT_210 追加更正；审稿另指出 re-cut 需对重启时刻一致（vendor forced_uniform_restart_time 路线）→ 215 | 09-16 0706Z | #215 | — | A01 **合同注册 3/3**：horizon_lower_bound——树支持的 H⁷ 球一致下界（vendor 时间预算单调性）；H¹ 子句（Tao H¹ 局部理论）诚实评估，给 lead/owner 两个选项（实现 H¹/H³ 定量局部理论 或 V2 re-cut 到 H⁷） |
| 211-A01-local-theory-bundle | 已合并（#216，wave 44 门禁绿；LocalTheoryBundle.lean 在集成分支） | 09-16 0541Z | #216 | — | A01 **局部理论装配（不碰 Contracts）**：载体见证捆绑结构 LocalCarrier、用 210 的一致定理选视界 localHorizon′（反单调）、209 正则性实例化、horizon_lower_bound 的两个变体（H⁷/固定力已证；H¹ 原文仅 def 不断言）、LocalTheoryAPI 形状探针；V2 注册（212）等用户决定 |
| 213-A02-A04-wiring | 已合入 #218；事后审稿（astra）REJECT 仅因 A04 无条件链缺失——215/217 随后补齐，无需改动；审稿记录已落地 | 09-16 0927Z | #218 | — | A02/A04 **接线**：exists_maximal 用 211 的 localHorizon′/localCarrier 无条件实例化；A04 的 Restart 由捆绑 + 179 的 Grönwall 一致 H^m 界 + horizon_lower_bound_H7_fixedForce 构造 ⇒ restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite 无条件 |
| 214-R43-parseval | 已合并（#217，wave 45 门禁绿；Parseval.lean 在集成分支；eq:Rcritical1 只条件于 CriticalDatumPath） | 09-16 0557Z | #217 | — | R43 **分数阶 Parseval**：pairing_identity（半阶数据配对 = 物理平流与 Λu 的 L² 配对，Plancherel + 191 的 abs(ξ)·û 实现）⇒ CriticalAdvectionLpBridge 完整 ⇒ eq:Rcritical1 只条件于 CriticalDatumPath |
| 215-A04-restart-fixed-force | 已合入 #219（wave 47 门禁 OK）；restartFixedForce_of_memForceR 与 higherOrderBound_of_gronwall 无条件；延拓固定力副本条件于 ShiftedLocalExtension（217 已闭合，审稿中） | 09-16 0917Z | #219 | — | A04 **固定力重启**：RestartFixedForce（一个 δ 对所有 t₀∈[0,S] 与 H⁷ 重启数据；力平移的 sup 范数由 [0,S+1] 上界控制 + 211 的反单调选择/vendor forced_uniform_restart_time）、179 Grönwall→HigherOrderBound 桥、A04 三条延拓定理的固定力副本并实例化；不改 owner 的 Restart 定义 |
| 216-R43-critical-datum-path | 已合入 #221（wave 49 门禁 OK）；六条半阶路径 + 四个空间字段无条件；装配条件于 CriticalDatumInputs（219 在闭合） | 09-16 0936Z | #221 | — | R43 **半阶载体 G3/G4**：为经典解构造 CriticalDatumPath（六条 1/2 阶数据路径 + 符号恒等式 + 横向/纵向 + velocityHalf 光滑与动量恒等式，169 的齐次类比）；装配 exists_criticalDatumPath，使 214 的 eq:Rcritical1 去掉 hcrit 绑定；若动量恒等式不闭合则隔离为 CriticalDatumInputs |
| 217-A04-shifted-extension | 已合入 #220（wave 48 门禁 OK）；shiftedLocalExtension 无条件；A04 restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite 对 MemForceR 力在固定力 + H⁷ 形状下无具名输入 | 09-16 0925Z | #220 | — | A04 **平移拼接**：证 215 的唯一具名输入 ShiftedLocalExtension——把从 b 重启的经典解与原解拼成 [0,b+L) 上的经典解（速度在重叠区由唯一性相等；压力用时间截断凸组合避免跨 T 延拓常数），推出 maximalLifespanR ≥ b+L；给出无具名输入的 restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite（MemForceR 力） |
| 218-R44-s1a-jweight | 已合入 #222（wave 50 门禁 OK）；R44 S1a 数据层闭合（Jmul、权恒等式、力对偶，负阶载体） | 09-16 0948Z | #222 | — | R44 **S1a**：数据层 J=(I−Δ)^{1/2} 乘子（等距 H^s→H^{s−1}）、权恒等式 ‖u‖²_{H^{3/2}} = Y²+Z²（Y=‖u‖_{H^{1/2}}，Z=‖∇u‖_{H^{1/2}}，191 的平移数据）、力对偶 abs⟪f,Ju⟫ ≤ B·sqrt(Y²+Z²)（B=‖f‖_{H^{-1/2}}，需负阶载体或直接定义）；静态、逐切片，不含 PDE |
| 219-R43-critical-momentum | 已合入 #223（wave 51 门禁 OK）；eq:Rcritical1 对任意经典解无具名输入（rcritical1_of_classical'） | 09-16 0954Z | #223 | — | R43 **动量传输**：闭合 216 的 CriticalDatumInputs——半阶速度路径的时间光滑性（215 的整数阶光滑路径 + lowerVectorL + 216 乘子为 CLM）与数据层动量恒等式（局部载体窗口覆盖 + 169 导数=投影残差 + 189/195/197 Leray 识别 + 齐次数据唯一性）；给出无条件 exists_criticalDatumPath 与 rcritical1_of_classical |
| 220-R44-s1c-trilinear | 已合入 #229（wave 57 门禁 OK）；R44 S1c 三线性估计无条件 | 09-16 1354Z | #229 | — | R44 **S1c**：J 权三线性估计 abs⟪(u·∇)u,Ju⟫ ≤ C₀·Y·(Y²+Z²)（三因子 L³ Hölder + 165/182 临界嵌入 + 164 齐次≤非齐次 + 218 的 Jmul_norm/weight_identity）；静态逐切片；新增 AdvectionJDatum 不改 JWeightDatum |
| 221-R43-force-path | 已合入 #224（wave 52 门禁 OK）；G2/G3/G4 力路径闭合；critical_bootstrap_zero_datum 无条件 | 09-16 1326Z | #224 | — | R43 **G2/G3/G4 时间路径**：b(t)=‖f(t)‖_{Ḣ^{1/2}}（214 的 criticalForceAt）连续、可积、原函数 N 的 FTC 与单调性（匹配 Paper1.critical_norm_bound / criticalNormBound_radius 的假设形状）、G2 时间积分单调性；探针：a=0 的 bootstrap y(t) ≤ cν 仅条件于 ν>0、MemForceR、小性 |
| 222-R44-s1b-energy-identity | 已合入 #226（wave 54 门禁 OK）；R44 S1b 能量恒等式无条件 | 09-16 1338Z | #226 | — | R44 **S1b**：沿经典解微分 Y²=‖u‖²_{H^{1/2}}（215 光滑整数阶路径经 lowerVectorL 降到 1/2 + DerivNorm），(Y²)′=2⟪∂ₜu,Ju⟫，A04.momentum_datum 二阶动量恒等式降阶，压力项由 Leray 正交消去，耗散项 ⟪νΔu,Ju⟫=−νZ²；得 energy_identity |
| 223-R43-endpoint | 已合入 #225（wave 53 门禁 OK；第一次合并因 221 docstring 修改与分支上的 ForcePath.lean 副本冲突，lead 对齐后重合）；inhomogeneousAtZero 无条件、逐字同 Spec | 09-16 1341Z | #225 | — | R43 **端点定理 S4/S5/S6**：inhomogeneousAtZero——a=0、‖f‖ < c₀ν ⇒ T_max = ∞：221 的 bootstrap → S3 门 → C01 V4 吸收 eq:RH1 → ∫‖u‖²_{H²} 有限（G5 端点粘合）→ 217 的 lifespanInfiniteOfLocallyFinite_of_memForceR′；陈述逐字取 Spec.lean |
| 224-R41-nondensity-q1 | 已合入 #227（wave 55 门禁 OK）；thm:Rmain (ii) q=1 非稠密子句无条件 | 09-16 1346Z | #227 | — | R41 **q=1 非稠密子句**：forceSobolevENorm 按阶单调（lowerVectorL 降阶路径）+ 223 端点定理的逆否 ⇒ s ≥ 1/2 时 ¬BreakdownDenseR ν 0 T 1 s（Data.lean 逐字词汇，ρ = criticalConst·ν） |
| 225-R43-universal | 已合入 #228（wave 56 门禁 OK）；Prop. 4.3 两个子句均无条件（同一 criticalConst） | 09-16 1351Z | #228 | — | R43 **universal（一般初值）**：Spec 的 universal 字段逐字——一般 a 的 bootstrap（sqrt_energy_le_primitive′ + continuous_bootstrap，ρ = y(0)+∫b）+ C01 V4 带初值项的 H² 预算 + A04 延拓；若字段有误不 re-cut，写 COMPARISON_UNIVERSAL 给 owner |
| 226-R43-contract-v1 | 已合入 #230（wave 58 门禁 OK）；第 30 个合同 R43.critical_regularity V1 注册完成 | 09-16 1411Z | #230 | — | R43 **V1 合同注册**（第 30 个合同 R43.critical_regularity）：Contracts/V1/CriticalRegularity.lean = Spec 的 RCritical1API 逐字（dotHomogeneousENorm 用已注册的 V1 定义）、Bindings 见证 {c := criticalConst, universal, inhomogeneousAtZero}（复用 223/225 审计里的 Data 词汇一致性证明与 maximalLifespanR 桥）、Tests、registry |
| 227-R44-endpoint | 已合入 #231（wave 59 门禁 OK）；Prop. 4.4 端点装配，仅条件于 S1 微分不等式（229 在闭合） | 09-16 1415Z | #231 | — | R44 **端点装配 S2–S6**：以 S1 微分不等式（拆分表 :56-64 的精确目标）为唯一具名输入 RCritical2Differential，走 166 的 Grönwall/首次逃逸 → L³ 门 → C01 V4 零初值 H² 预算 → G5 端点粘合 → A02/A04 延拓 ⇒ Prop. 4.4 的 a=0 结论（r_{ν,S} 显式）；S1d 之后即无条件 |
| 228-R44-s1d-absorption | worker 完成（sol，14 分钟）：rcritical2_differential 无条件（θ=(4C₀)⁻¹，C₂=3/2，C₃=3；E′ 单函数，闭窗口可积）+ 222/220 命名桥；但与 227 在 R44 命名空间重复声明 theta/C₂/C₃/RCritical2Differential 且常数不同 → 不单独开 PR，由 229 吸收（改 import、去重、按单调性实例化 227 的结构） | 09-16 1411Z | — | — | R44 **S1d Young 吸收**：222 能量恒等式 + 220 三线性 + 218 力对偶，在 Y ≤ θν 下得 (Y²)′ + νZ² ≤ C₂νY² + C₃ν⁻¹B²（θ、C₂、C₃ 显式），装成一个可积 E′（拆分表 :56-64 / Pieces.lean:153-170 的精确 S1 目标）；含 222 审稿留下的 advectionJPairing 命名桥；若 227 的 RCritical2Differential 已存在则给实例 |
| 229-R44-prop44 | 已合入 #232（wave 60 门禁 OK）；Prop. 4.4（a=0）无条件；R41 q=2 非稠密子句 | 09-16 1426Z | #232 | — | R44 **Prop. 4.4 无条件 + R41 q=2**：把 228 的 Absorption.lean 改造（import 227 的 Endpoint、去重、按单调性实例化 227 的 RCritical2Differential）；rcritical2_endpoint_unconditional；R41 q=2 非稠密子句 not_breakdownDenseR_zero_L2（s ≥ −1/2，ρ = radius ν T，224 模板） |
| 231-R44-contract-v1 | 已合入 #236（wave 64 门禁 OK）；第 31 个合同 R44.critical_finite_horizon V1 注册完成 | 09-16 1443Z | #236 | — | R44 **V1 合同注册**（第 31 个合同 R44.critical_finite_horizon）：CriticalFiniteHorizonAPI = Spec 的 RCritical2API 逐字；Bindings 见证来自 229 的 Prop44（c=θ/20，C=3，radius 显式）；Tests；registry |
| 232-R41-nondensity-both | 已合入 #235（wave 63 门禁 OK）；nonDensityZero 两个 q 装配（RMainNonDensity） | 09-16 1441Z | #235 | — | R41 **nonDensityZero 两个 q 的装配**：按 RMainAPI 骨架（q=1∨q=2，ThresholdAPI 的 exponent q 0 ≤ s）由 224/229 的两条非稠密定理拼出 nonDensityZero_of_q 与 not_breakdownDenseR_zero_of_q，并打包成可 ⟨…⟩ 实例化的结构 |
| 233-R42-family-from-data | 已合入 #233（wave 61 门禁 OK）；R41D G1 闭合 | 09-16 1435Z | #233 | — | R41D **G1**：从原始数据 (ν,T,a∈X_R,g∈F_R, T < T_max) 构造已注册的 R42 插入记录 InsertionLifespanV2API（严格寿命 ⇒ RegularThrough ⇒ 参考解 ⇒ 校正/缩放/族/寿命记录），family.a/g/T 相符；缺的构造器隔离为具名输入并注明归属 lane |
| 234-R41D-small-gaps | 已合入 #248（wave 76 门禁 OK）；G2–G5 落地 | 09-16 1713Z | #248 | — | R41D **G2–G5**：F_rd ⊆ F_R、快速类在紧支撑校正下封闭、S_σ ⊆ X_R、‖0‖_{L^qH^s}=0（先核对是否已有，再证；不成立则写 COMPARISON_SMALL_GAPS 给 owner） |
| 235-R41D-density | 已合入 #234（wave 62 门禁 OK）；thm:Rmain (i) 稠密性 + (ii) if 半 | 09-16 1440Z | #234 | — | R41D **稠密分支（Y = F_R）**：thm:Rmain (i) 与 (ii) 的"if"半：q∈{1,2}、s < 2/q−3/2、任意 a∈X_R ⇒ BreakdownDenseR ν a T q s（两情形：T_max ≤ T 取 f=g；否则 233 的 R42 记录 + forceConvergence + lifespan = T）；a=0 实例；若 232 可读则给 iff |
| 236-SPEC-r41-draft-a | 草案 A 完成（sol，80 分钟）：RMainAPI 四字段（q:ℕ；rider 打包为 Type 值结构 RegularReferenceApproximation）；与草案 B 高度一致；lead 已写 research/R41/RECONCILIATION.md（取 B 的 Prop 内联 rider、q:ℝ≥0∞、criticalOrder；与 STATEMENTS 骨架的差异记录） | 09-16 1602Z | — | — | SPEC **R41 双盲草案 A**：只读论文 + Data/Thresholds 词汇，写 Theorem 4.1 的 Lean 合同陈述（(i)、(ii) iff、rider），不看 STATEMENTS/R41D/R41 现有文件；输出 Draft_A.lean（可 elaborate）+ COMPARISON_A |
...
| 238-SPEC-r45-draft-a | 草案 A 完成（sol，9 分钟）：RClassesAPI 七字段（按类拆分 compact*/rapid* + rapidSchwartzDenseBelow，q:ℕ，rider 打包为 HasReferenceApproximationRider）；草案 B 参数化 Y、q:ℝ、rider ε 形式；等 R41 定稿后统一形状写 RECONCILIATION | 09-16 1454Z | — | — | SPEC **R45 双盲草案 A**（cor:Rclasses：F_c/F_rd 上的 Theorem 4.1）：只读论文 + Data 词汇写 RClassesAPI 陈述 + COMPARISON_A |
| 239-SPEC-r45-draft-b | 草案 B 完成（astra）：见 research/R45/DraftB.lean + COMPARISON_B；等草案 A 后比对 | 09-16 1449Z | — | — | SPEC **R45 双盲草案 B**：同上，独立模型 |
| 240-SPEC-r47-draft-a | 草案 A 完成（sol，18 分钟）：以 InsertionFamilyAPI 记录为参数的 RGridAPI（12 字段）；草案 B 为原始数据上的存在陈述 RGridFamily/choose；lead 已写 RECONCILIATION（取 B 形状、A 的 ball ⊆ cell、forceSobolevENorm 1 0） | 09-16 1504Z | — | — | SPEC **R47 双盲草案 A**（thm:Rgrid 相同网格观测）：只读论文 + Data/InsertionFamily 词汇写 RGridAPI 陈述 + COMPARISON_A |
| 241-SPEC-r47-draft-b | 草案 B 完成（astra）：见 research/R47/DraftB.lean + COMPARISON_B；等草案 A 后比对 | 09-16 1450Z | — | — | SPEC **R47 双盲草案 B**：同上，独立模型 |
...
| 244-SPEC-r45-spec | 已合入 #238（wave 66 门禁 OK）；R45 定稿规范落地 | 09-16 1514Z | #238 | — | SPEC **R45 定稿**：按 lead 的 research/R45/RECONCILIATION.md 合并草案 A/B → Spec.lean（可 elaborate）+ COMPARISON.md；复制两份草案与报告留档 |
| 245-SPEC-r46-spec | 已合入 #237（wave 65 门禁 OK）；R46 定稿规范落地 | 09-16 1514Z | #237 | — | SPEC **R46 定稿**：按 research/R46/RECONCILIATION.md 合并草案 A/B → Spec.lean + COMPARISON.md（含 CompletedDense 缩写的 rfl 核对） |
| 246-SPEC-r47-spec | 已合入 #239（wave 67 门禁 OK）；R47 定稿规范落地（mixedLebesgueENorm 拼写问题留给 owner） | 09-16 1614Z | #239 | — | SPEC **R47 定稿**：按 research/R47/RECONCILIATION.md 合并草案 A/B → Spec.lean + COMPARISON.md |
| 247-R47-grid-lemmas | 已合入 #241（wave 69 门禁 OK）；R47 网格引理落地 | 09-16 1624Z | #241 | — | R47 **网格引理**：有限多个网格的公共格子里选球（exists_ball_in_common_cell）+ gridObservation 局部性（支撑在某格子内且（若论文需要）该格子上均值为零 ⇒ 观测相等）；Bindings 层，使用注册的 Grid/gridObservation |
| 248-SPEC-r41-spec | 已合入 #240（wave 68 门禁 OK）；R41 定稿规范 + 绑定计划落地 | 09-16 1616Z | #240 | — | SPEC **R41 定稿**：按 research/R41/RECONCILIATION.md 合并草案 A/B（+ STATEMENTS 骨架）→ Spec.lean + COMPARISON.md（含"绑定计划"：232/233/235 的定理如何绑各字段） |
| 249-R41-contract-v1 | 已合入 #242（wave 70 门禁 OK）；**Theorem 4.1（Y = F_R）= 第 32 个合同 R41.main_thresholds V1，四字段全绑定** | 09-16 1627Z | #242 | — | R41 **V1 合同注册**（第 32 个合同 R41.main_thresholds）：MainThresholdsAPI = research/R41/Spec.lean 的 RMainAPI 逐字；Bindings 按绑定计划接 235/232/233 + R42 V2 字段（rider 用同一记录）；若某字段不可绑则注册 partial 变体并报告缺口 |
| 250-I03-homogeneous-scaling | 已合入 #244（wave 72 门禁 OK）；齐次缩放恒等式落地；两字段条件于 CompactHomogeneousRealization（255 在闭合） | 09-16 1650Z | #244 | — | I03 **齐次负阶缩放**：Spec 里缺失的 packetNegativeHomogeneous / correctionNegativeHomogeneous（‖F_ε‖_{L^q_tḢ^s} ≤ Cε^β，抛物缩放的 Fourier 齐次权计算，以 IsHomogeneousPath 路径证 forceHomogeneousENorm 上界）——R46/R47 强闭包的 L²_tḢ⁻¹ 项所需 |
| 251-R47-force-cell-integral | 已合入 #243（wave 71 门禁 OK）；速度零均值/动量之差/观测相等落地；力的 t∈(0,T) 条件于 hcompactMomentumIntegral（253 在闭合） | 09-16 1645Z | #243 | — | R47 **eq:gridforce**：插入族速度差/力差在含球格子上积分为零（速度：div-free + 支撑；力：两条动量方程之差在格子上积分，时间导数项由速度零均值消去、散度/梯度/Laplace 项由紧支撑消去），并与 247 的局部性引理合出 Spec 的 velocity_/force_observations 字段 |
| 252-R45-compact-class | 已合入 #245（wave 73 门禁 OK）；cor:Rclasses 的 F_c 稠密与零初值 iff 落地 | 09-16 1651Z | #245 | — | R45 **F_c 实例**：Spec 的 density/zeroIff 在 Y = forceClassCompact 下（235 的两情形模板 + forceDifference_compact 保持紧支撑；F_c ⊆ F_R 由 memForceR_insertedForce 取 g=0；非稠密由 breakdownSetIn F_c ⊆ breakdownSetRZero + 232/249 反证） |
| 253-R47-flux-cancellation | 已合入 #246（wave 74 门禁 OK）；eq:gridforce 闭合，R47 观测相等无条件 | 09-16 1655Z | #246 | — | R47 **通量抵消**：证 251 的唯一具名输入 hcompactMomentumIntegral——动量残差之差的空间项（div(u_ε⊗u_ε − v⊗v)、∇δp、−νΔw）都是紧支撑于 cell 内的光滑场的空间导数，在 cell 上积分为零，只剩 ∫∂_t w；力观测相等无条件 |
| 254-R45-compact-rider | 已合入 #247（wave 75 门禁 OK）；F_c 的正则参考 rider + F_R 的 ε 形式落地 | 09-16 1659Z | #247 | — | R45 **F_c 的正则参考 rider**：Spec 的 regularReference 在 Y = forceClassCompact 下（249 的 R41 rider 绑定为模板：233 记录一次调用 + 唯一性对齐 + R42 history/forceConvergence/速率夹逼；ε 形式与任意历史截断 τ 由极限导出；f ∈ F_c 由紧支撑闭包） |
| 255-I03-path-measurability | 已合入 #249（wave 77 门禁 OK）；I03 齐次负阶字段无条件 | 09-16 1715Z | #249 | — | I03 **路径可测性**：证 250 的唯一具名输入 CompactHomogeneousRealization（紧支撑光滑力的 compactHomogeneousPath 在 forceTimeMeasure 下 a.e. 强可测：切片连续 + 齐次实现连续，−3/2<s<0 的低频可积），从而 packet/correctionNegativeHomogeneous 无条件 |
| 256-R46-sobolev-density | 已合入 #250（wave 78 门禁 OK）；Prop. 4.6 第一字段落地 | 09-16 1716Z | #250 | — | R46 **完备 Sobolev 稠密**（Spec 第一字段）：B01 approxCompact 逼近完备目标 → 252 的 F_c 相对稠密 → 路径可加 + Bochner 数据范数三角不等式 ⇒ CompletedDense q s (breakdownSetIn F_c ν a T) |
| 257-R45-rapid-class | 已合入 #253（wave 81 门禁 OK）；cor:Rclasses 的 F_rd 实例 + Schwartz 落地 | 09-16 1726Z | #253 | — | R45 **F_rd 实例 + Schwartz**：density_rapid / zeroIff_rapid / regularReference_rapid（252/254 模板 + 234 的 G2/G3）与 schwartzDensity（G4）；density/zeroIff 的参数化 Y 合并（Or.elim） |
| 258-R47-assembly | 已合入 #252（wave 80 门禁 OK）；Theorem 4.7 装配落地（条件于已证的可测性输入；260 注册时实例化） | 09-16 1719Z | #252 | — | R47 **装配**：Spec 的 RGridAPI.choose——公共格子选球（247）+ 233 记录（球钉在该 cell）+ 251/253 观测相等 + 249 能量极限路线 + forceConvergence + 250 齐次界；唯一具名输入 CompactHomogeneousRealization（255 在闭合） |
| 259-R46-closure | 已合入 #251（wave 79 门禁 OK）；Prop. 4.6 后两字段（条件于已证的可测性）落地 | 09-16 1719Z | #251 | — | R46 **第二、三字段**：completedHomogeneousDensity（HomogeneousPartial V2 approxCompactHomogeneous + 缩放界 + 齐次路径可加）与 strongTrajectoryClosure（233 记录 + energyRate + forceConvergence + 250 齐次界）；唯一具名输入 CompactHomogeneousRealization |
| 212-A01-contract-v2 | worker 完成（sol，21 分钟）：A01.local_theory_v2 注册（extends V1；horizon_lower_bound = 固定外力 + H⁷，绑 211；H¹ 句子仅作未注册的未证定义 ManuscriptHorizonLowerBoundH1；非零外力/初值取得真实 δ>0；COMPARISON 有论文 | 09-16 1735Z | #vs | — | A01 **V2 合同**（选项 1，用户 2026-09-17 决定）：horizon_lower_bound 固定外力 + H⁷ 措辞注册；论文 H¹ 句子逐字保留为命名的未证定义；每字段 docstring 写清差异；非零外力/初值的非空洞实例；COMPARISON 加"论文 vs V2"对比表 |
| 230-A04-v2-restart-fixed-force | worker 完成（sol，23 分钟）：A04.continuation_v2 注册（独立结构，不 extends 含未证 Restart 的 V1；restart = RestartFixedForce（参数化 horizon，Bindings 用 localHorizon′ rfl）；HigherOrderBound + 217 三条延拓；H¹ 句子仅作未证定义；非零外力/初值取得真实 δ；论文 | 09-16 1736Z | #vs | — | A04 **V2 合同**（选项 1）：Restart = RestartFixedForce（固定外力、对 t₀∈[0,S] 与 H⁷ 数据一致）+ HigherOrderBound + 217 形状的三条延拓定理；V1 的 H¹ Restart 留作明确未证；同样的防 reward-hacking 要求 |
| 260-R47-contract-v1 | worker 完成（sol，10 分钟）：R47.grid_observations V1（GridFamilyAPI/GridObservationsAPI = Spec 逐字；见证 = 258 × 255；十九字段逐一赋值，不新选任何对象）；分支已并入集成分支；审稿中 | 09-16 1729Z | — | — | R47 **V1 合同注册**（第 35 个 R47.grid_observations）：Spec 的 RGridFamily/RGridAPI 逐字；见证 = rGrid_choose_of_realization 实例化 255 的 compactHomogeneousRealization |
| 261-R46-contract-v1 | worker 完成（sol，9 分钟）：R46.completed_density V1（CompletedDensityAPI = REnergyAPI 字节相同；见证 = 256 + 259×255）；L¹L² 拼写差异记为 owner 问题；分支已并入集成分支；审稿中 | 09-16 1728Z | — | — | R46 **V1 合同注册**（第 34 个 R46.completed_density）：Spec 的 REnergyAPI 逐字；见证 = 256 的 completedSobolevDensity + 259 的两条 _of_realization 实例化 255；L¹L² 项拼写差异记为 owner 问题 |
| 262-R45-contract-v1 | worker 完成（sol，9 分钟）：R45.force_classes V1（ForceClassesAPI = RClassesAPI 字节相同；guarded regularReference 由 254/257 Or.elim）；分支已并入集成分支；审稿中 | 09-16 1734Z | — | — | R45 **V1 合同注册**（第 33 个 R45.force_classes）：Spec 的 RClassesAPI 逐字；见证 = 257 的 guarded density/zeroIff + schwartzDensity + regularReference 由 254/257 Or.elim |
| 263-SPEC-t10-draft-a | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 1739Z | — | — | **Section 3 启动（S3-0）**：T10 周期数据层的双盲陈述草案 A——系数侧 H^s(T³) 载体、IsPeriodicDatum 桥、均值/均值零、周期 Leray、∫p=0、X_T/F_T/B/E_T/ClassicalSolutionT/maximalLifespanT（照抄 D01 架构） |
| 264-SPEC-t10-draft-b | worktree 安装中→自动启动（codex astra low，带重试） | 09-16 1739Z | — | — | T10 双盲陈述草案 B：同上，独立模型 |
