...
| 321-T11-U10-U11-restart-horizon | worktree 安装中→自动启动（codex astra low，sol 备用） | 09-18 0415Z | — | — | T11 U10+U11：`restart`（对着具名输入 `PeriodicQuantitativeLocalInput'`，按阶力界 M m := ‖f‖_{L¹H^m}）与 `horizon`/`solution`/`regularity`（M，sol） |
...
| 323-T11-U15-maximal | worktree 安装中→自动启动（codex astra low，sol 备用） | 09-18 0415Z | — | — | T11 U15：`exists_maximal`/`maximal_unique` + `maximalLifespanT = PeriodicLifespan.lifespan`（复用 Paper1 无条件粘合）（M，sol） |
