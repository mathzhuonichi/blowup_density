...
| 213-A02-A04-wiring | 两次审稿因路由 524/容量在 60–70 分钟后死掉；lead 复核（两模块构建、审计干净）；PR 已开，合并链运行中（wave 46）；事后审稿休息后重启 | 09-16 0848Z | #218 | — | A02/A04 **接线**：exists_maximal 用 211 的 localHorizon′/localCarrier 无条件实例化；A04 的 Restart 由捆绑 + 179 的 Grönwall 一致 H^m 界 + horizon_lower_bound_H7_fixedForce 构造 ⇒ restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite 无条件 |
