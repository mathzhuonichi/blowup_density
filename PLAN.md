...
| 125-D01-finite-order-datum | PR 已开 | 09-14 0130Z | #129 | — | D01 有限阶 datum 构造子（C1b 真正的阻塞 C1b-m-D，M–L，拆分起步）：`MemLp z 2` + 各阶 ≤ m 的 L² 弱导数 ⇒ `∃ A, IsSobolevDatum m z A`（D01 目前只有 0 阶 `orderZeroDatum` 与全阶 `smoothAngularDatum`）；产出 `research/D01/FINITE_ORDER_SPLIT.md` + 证第一个 S 单元 |
