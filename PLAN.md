...
| 251-R47-force-cell-integral | 审稿 REJECT 仅因基线未同步（兼容检查把新注册的 MainThresholds 当作删除）；lead 并入集成分支并重跑门禁后开 PR，合并链运行中（wave 71） | 09-16 1644Z | #243 | — | R47 **eq:gridforce**：插入族速度差/力差在含球格子上积分为零（速度：div-free + 支撑；力：两条动量方程之差在格子上积分，时间导数项由速度零均值消去、散度/梯度/Laplace 项由紧支撑消去），并与 247 的局部性引理合出 Spec 的 velocity_/force_observations 字段 |
