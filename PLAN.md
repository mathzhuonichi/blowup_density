...
| 189-A01-pressure-regularity | fix2 装配完成（astra：pressureSupply_of_pieces 逐字证出 180 fix6 的 PressureSupply，无额外假设，含 t=0 的对称 Jacobian；管线探针只剩 hb；零对探针）；codex 审稿中（astra） | 09-16 0037Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
