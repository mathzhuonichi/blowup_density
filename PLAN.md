...
| 247-R47-grid-lemmas | 审稿 ACCEPT（sol，无修改）；分支已并入集成分支；PR 已开，合并链运行中（wave 69） | 09-16 1623Z | #241 | — | R47 **网格引理**：有限多个网格的公共格子里选球（exists_ball_in_common_cell）+ gridObservation 局部性（支撑在某格子内且（若论文需要）该格子上均值为零 ⇒ 观测相等）；Bindings 层，使用注册的 Grid/gridObservation |
...
| 249-R41-contract-v1 | worker 完成（astra，7 分钟）：**第 32 个合同 R41.main_thresholds V1——Theorem 4.1（Y = F_R）四字段全部绑定**（235 稠密、232 非稠密按 q 分情况、233 记录一次调用 + R42 速率经 ENNReal 夹逼得能量极限；结构体字节比对通过）；分支已并入集成分支；审稿中 | 09-16 1621Z | — | — | R41 **V1 合同注册**（第 32 个合同 R41.main_thresholds）：MainThresholdsAPI = research/R41/Spec.lean 的 RMainAPI 逐字；Bindings 按绑定计划接 235/232/233 + R42 V2 字段（rider 用同一记录）；若某字段不可绑则注册 partial 变体并报告缺口 |
