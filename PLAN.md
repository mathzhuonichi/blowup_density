...
| 190-A01-b1-r4-joint-smooth | 审稿 ACCEPT-WITH-NOTES（保真；接口探针接通 178/180/192；两处记录措辞已改）；PR 已开，合并链运行中（wave 21） | 09-15 2252Z | #194 | — | A01 **B1 R4 联合光滑代表元**：由 178 的全阶 C^j_t 数据路径造一个 u(t,x)，切片 a.e. 等于 U t 且在 [0,S)×R³ 上联合 C∞（Sobolev 嵌入 + 连续线性求值）——180 的 `hc3`；新增可满足性规则 |
