...
| 231-R44-contract-v1 | worker 完成（sol，10 分钟）：第 31 个合同 R44.critical_finite_horizon V1（CriticalFiniteHorizonAPI = RCritical2API 九字段逐字，token 比对 True；见证 c=θ/20、C=3、radius）；分支已并入集成分支；审稿中 | 09-16 1430Z | — | — | R44 **V1 合同注册**（第 31 个合同 R44.critical_finite_horizon）：CriticalFiniteHorizonAPI = Spec 的 RCritical2API 逐字；Bindings 见证来自 229 的 Prop44（c=θ/20，C=3，radius 显式）；Tests；registry |
| 232-R41-nondensity-both | worker 完成（sol，11 分钟）：nonDensityZero_of_q / not_breakdownDenseR_zero_of_q（q=1∨q=2，ThresholdAPI 形状的本地记录 RMainThresholds，q:ℝ 经 ofReal 转成范数指数）；RMainNonDensity 结构可 ⟨…⟩ 实例化；14 声明审计；分支已并入集成分支；审稿中 | 09-16 1431Z | — | — | R41 **nonDensityZero 两个 q 的装配**：按 RMainAPI 骨架（q=1∨q=2，ThresholdAPI 的 exponent q 0 ≤ s）由 224/229 的两条非稠密定理拼出 nonDensityZero_of_q 与 not_breakdownDenseR_zero_of_q，并打包成可 ⟨…⟩ 实例化的结构 |
| 233-R42-family-from-data | worker 完成（astra，5 分钟）：insertionLifespanV2_of_data 无条件闭合 R41D G1（A02 参考解选择 → I01 packet → I02 correction → I03 scaling → R42 family → V2 lifespan；Bindings/InsertionFromData.lean）；发现既有 Bindings.Packet/Scaling 的 navierStokesResidual_eq 重名（未改，复制装配）；审稿中 | 09-16 1428Z | — | — | R41D **G1**：从原始数据 (ν,T,a∈X_R,g∈F_R, T < T_max) 构造已注册的 R42 插入记录 InsertionLifespanV2API（严格寿命 ⇒ RegularThrough ⇒ 参考解 ⇒ 校正/缩放/族/寿命记录），family.a/g/T 相符；缺的构造器隔离为具名输入并注明归属 lane |
...
| 235-R41D-density | worktree 安装中→自动启动（codex astra low，带重试；基于 233 分支） | 09-16 1428Z | — | — | R41D **稠密分支（Y = F_R）**：thm:Rmain (i) 与 (ii) 的"if"半：q∈{1,2}、s < 2/q−3/2、任意 a∈X_R ⇒ BreakdownDenseR ν a T q s（两情形：T_max ≤ T 取 f=g；否则 233 的 R42 记录 + forceConvergence + lifespan = T）；a=0 实例；若 232 可读则给 iff |
