...
| 234-R41D-small-gaps | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 1422Z | — | — | R41D **G2–G5**：F_rd ⊆ F_R、快速类在紧支撑校正下封闭、S_σ ⊆ X_R、‖0‖_{L^qH^s}=0（先核对是否已有，再证；不成立则写 COMPARISON_SMALL_GAPS 给 owner） |
...
| 253-R47-flux-cancellation | worker 完成（astra，10 分钟）：compactMomentumIntegral 无条件闭合 251 的输入（对流项写成张量差散度、压力/黏性项紧支撑导数积分为零、残差差 = 紧力差换到全空间积分）；force_gridObservation_eq′ 覆盖 [0,T)；12 定理；分支已并入集成分支；审稿中 | 09-16 1648Z | — | — | R47 **通量抵消**：证 251 的唯一具名输入 hcompactMomentumIntegral——动量残差之差的空间项（div(u_ε⊗u_ε − v⊗v)、∇δp、−νΔw）都是紧支撑于 cell 内的光滑场的空间导数，在 cell 上积分为零，只剩 ∫∂_t w；力观测相等无条件 |
