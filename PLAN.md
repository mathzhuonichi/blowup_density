...
| 192-A01-wiring-hsob | 复审 REJECT（合并导出丢了角不变性与 Duhamel，只导出 j=0）；fix2 运行中 | 09-15 2251Z | #— | — | A01 **接线**：186′+188（公共视界无条件）+187（hfs）+178（全阶 C^j 数据路径）→ 仅由全阶先验界 hb 得到构造器的 hsob/hU/hdiv；记录 A01 管线剩下的唯一分析输入（A3-M2 的 hb，加 190 的 hc3、189 的 hpg） |
