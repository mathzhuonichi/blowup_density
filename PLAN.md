...
| 366-T12-U1-haar-cube | codex（astra）只交基础；审稿 REJECT（缺全 p 传输含 0/⊤、标量/梯度张量/tsupport 陈述、真实 probe、3 个 warning）→ 排队 Opus 补全（等空位；可复用 364 的无假设桥 eLpNorm_torusLift_restrict 与 359 的 torus_cube_L2） | 09-18 1221Z | — | — | T12 U1：Haar ↔ 立方体 ↔ 全空间的 L^p 传输（周期场；支撑在 Q 内的场；标量/向量；periodicLpENorm 桥） |
