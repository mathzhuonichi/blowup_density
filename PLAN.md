...
| 307-SPEC-t24-draft-b | sol 掉线（已有 925 行 DraftB）→ 06:34Z 续跑简报重启 | 09-18 0634Z | — | — | SPEC **T24 双盲草案 B**（同上，互不可见） |
...
| 336-T11-U12b-pairing-bound | Opus 完成：配对界 + higherOrderBound 无条件（torusHigherOrderBound；33 声明）→ 排队 codex 审稿 | 09-18 0700Z | — | — | T11 U12b：配对层 tame 估计 abs⟨(u·∇)u,u⟩_{H^m} ≤ C‖u‖_{H²}‖u‖_{H^m}‖∇u‖_{H^m}（Fourier 侧 Peetre 分裂） |
...
| 338-T11-U9e-existence-input-h3 | Opus 完成：H³ 球存在性输入无条件（picardHorizon 显式只依赖 ν,K,M 3）+ restart/restartBeyond/extendsBeyond/lifespan/maximal 的 H³ 版全部无条件；H1_GAP.md；发现 334 与去重后 327 的语义合并冲突并修复 → 排队 codex 审稿 | 09-18 0711Z | — | — | T11 U9e（lead 修正 2）：证 `PeriodicQuantitativeLocalInputH3`（H³ 球一致 δ）→ 存在性线闭合；H¹ 球版本保留为手稿具名未证谓词（照第 4 节 ManuscriptHorizonLowerBoundH1）；重实例化 restart/extendsBeyond/lifespan 的 H³ 版本；写 H1_GAP.md |
| 339-T11-U6b-regularity-of-classical | Opus prover 子代理（基座 = integration + 335 + 331 本地合并） | 09-18 0640Z | — | — | T11 U6b：任意 `ClassicalSolutionT` 满足 `PeriodicLocalRegularity`（335 的系数积分下求导迭代 → 光滑 lp 路径；Poisson 由动量+无散；projected 现成）→ 331/321 的条件结论无条件化 |
| 340-T11-U17-assembly-contract | 排队（待 334/336/338/339 合入；Opus；简报已写） | 09-18 0702Z | — | — | T11 U17：装配四个 API（H³ 收窄的延拓 API + 两个 H¹ 具名手稿谓词）+ 合同 `T01.torus_local_theory`（结构体例外转换）+ Bindings/Tests/注册（合同数 39） |
