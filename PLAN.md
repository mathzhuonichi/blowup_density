...
| 200-A01-forcing-bound | worker 完成（astra：Z 精确识别、源-压力抵消、源范数界；ForcingFamilyBound 条件于唯一空间估计 CylinderCommutatorBound（柱面 word 级 Kato–Ponce 换位子估计）；E q = mildNormConstant q，A q = E q·(1+outerTameConst)；18 项公理审计干净）；codex 审稿中（astra） | 09-16 0123Z | #— | — | A01 **ForcingFamilyBound**（A3-M2 forcing 子行）：正则化柱面系统的 word 强迫/换位子族范数 ≤ tame 配对（vendor 换位子引理 → A03 outerProductTame → A04 outerSobolevNormAt_le → inner_energy_Rhigh；压力 word 项走柱面 Leray 投影有界性），显式 E(q),A(q) |
| 201-A01-root-comparison | 审稿 REJECT（机械 + 范围：需 rebase 取集成的 envelope 模块并改名唯一性定理；CylinderSignedRootLimit 应限定无散数据，且它是带吸收的根积分界而非泛极限引理）；fix 运行中（sol） | 09-16 0125Z | #— | — | A01 **CylinderRootComparison**（A3-M2 envelope 子行）：带号恒等式 + ForcingFamilyBound（作为假设）⇒ 正则化根的微分不等式 → 沿 198 的最大逼近族取带号积分极限 → 标量 Grönwall 比较（含零根极限与端点）⇒ FiniteMildEnergy 条件于 ForcingFamilyBound |
| 203-A01-signed-limit | 等 201 fix 完成后自动建树启动（codex astra low；基于 201 分支） | 09-16 0126Z | — | — | A01 **CylinderSignedRootLimit**（A3-M2 最后两块之一）：把 199 的带号（保留 −ν 梯度平方）正则化能量不等式沿 198 的最大逼近族取积分极限——vendor 极限引理无耗散槽，需新增带号通道（强 TimeLp 极限经导数 word 映射或弱收敛下半连续）、ε 正则化根分母、先取极限再吸收；与 200 协调正则化族一致界 |
