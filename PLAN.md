...
| 349-MAINT-section3-build | worktree 安装中→自动启动（codex astra，sol 备用） | 09-18 0820Z | — | — | MAINT：#313–#316 合入批次后 Section 3 全量编译检查（40 模块一条 lake build、三门禁、39 合同、probe/公理扫描 → logs/SECTION3_BUILD_20260918b.md）+ 台账 T10 补记 T01.torus_data；基座 b7c5083173a6a581daa8f0150602e3d012236645 |
...
| 351-T16-ball-potential | worktree 安装中→Opus prover 子代理（基座 = 347 分支） | 09-18 0830Z | — | — | T16 缺口 1：球上径向势 timePotential_contDiffOn_ball / spatialCurl_timePotential_on_ball（空间 bump 截断 + I02 全局引理 + congr_slice）→ exists_potential_on_ball（potential_smooth/formula/curl 三字段） |
| 352-T16-lattice-lift | worktree 安装中→Opus prover 子代理（基座 = 347 分支） | 09-18 0830Z | — | — | T16 缺口 2：紧支修正的单位周期格点提升 latticeLift（局部有限、光滑、周期、支撑、无散、k=0 公式、eq:bgzero）→ 八个 correction_* 字段 |
