...
| 103-B02-separated-assembly | 审阅中 | — | — | B02 `separatedAssembly`（M）：分离和 `∑ φ_j(t) h_j(x)` 的齐次 datum 路径，经 reviewer 给的短路线（`isHomogeneousSliceDatum_unique` + `isHomogeneousPath_compact` + `homogeneousVectorDatum` 线性性，`-3/2<s`），`Section4/B02/SeparatedAssembly.lean` |
