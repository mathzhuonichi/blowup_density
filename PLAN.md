...
| 214-R43-parseval | 审稿 ACCEPT（链条与简报一致；变异按预期失败）；PR 已开，合并链运行中（wave 45） | 09-16 0556Z | #217 | — | R43 **分数阶 Parseval**：pairing_identity（半阶数据配对 = 物理平流与 Λu 的 L² 配对，Plancherel + 191 的 abs(ξ)·û 实现）⇒ CriticalAdvectionLpBridge 完整 ⇒ eq:Rcritical1 只条件于 CriticalDatumPath |
