...
| 215-A04-restart-fixed-force | worktree 安装中→自动启动（codex astra low，带重试；基于 213 分支） | 09-16 0849Z | — | — | A04 **固定力重启**：RestartFixedForce（一个 δ 对所有 t₀∈[0,S] 与 H⁷ 重启数据；力平移的 sup 范数由 [0,S+1] 上界控制 + 211 的反单调选择/vendor forced_uniform_restart_time）、179 Grönwall→HigherOrderBound 桥、A04 三条延拓定理的固定力副本并实例化；不改 owner 的 Restart 定义 |
