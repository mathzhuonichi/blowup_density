...
| 145-D01-quantitative-constructor | 进行中 | 09-14 0408Z | — | — | D01 **A3-L1·k-quant**（M，≈120–200 行，无外部输入）：给 132 的 `exists_isSobolevDatum_of_memLp_derivs` 补定量版——沿归纳跟踪范数 `‖A‖² ≤ c_m · Σ_{α, 阶≤m} ‖∂^α z‖²_{L²}`（119 曾 disclaim 的带 m 依赖常数的范数比较）；配 140 的 `‖word‖ ≤ ‖u‖`、`‖ordinaryLift Zw‖ = ‖Zw‖` 一行事实，即得 `sobolevNormAt 2 (⇑U t) ≤ c·(‖u₀‖+1)`，从而 `Kbnd`；`exists_local` 到 `Kbnd` 之间唯一剩下的东西 |
| 其余节点 | 未开始 | — | — | — | |
