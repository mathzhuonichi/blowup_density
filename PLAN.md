...
| 189-A01-pressure-regularity | fix 部分完成（astra：去掉 htime/ProjectedMomentumDatumOn/哑输入；余项代表元辅助引理已证；pressureGradientField_of_carrier 与管线未闭合，缺口见 REPORT_189）；待续跑 | 09-15 2301Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
