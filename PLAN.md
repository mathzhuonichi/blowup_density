...
| 195-A01-interior-momentum | worker 完成（astra：内点 ∂ₜ(联合代表元)=导数数据求值、代表元保减法、直接消费 169 的导数定理；最终恒等式条件于 hprojected——柱面 Leray 投影与 Fourier 数据级 lerayComplement 的桥；8 声明公理干净）；codex 审稿中 | 09-15 2321Z | #— | — | A01 **P4b 内点动量恒等式**（189 重设计的另一半）：190 联合代表元的 ∂ₜu = 导数数据的求值（有界求值与 fderiv 交换）+ 代表元线性 ⇒ 在 Ioo 0 S 上 G = residual − ∂ₜu；t=0 不作任何断言 |
| 196-A01-mild-gronwall | worker 完成（astra：word 范数比较、正则化 word PDE 与度量极限、tame/Young 代数、条件 mildGronwall 与族推论；剩一个命名输入 FiniteMildEnergy（标量平方能量包络的微分不等式）；指出 MildGronwall 未限定无散数据的接口错配；19 声明公理干净）；codex 审稿中 | 09-15 2323Z | #— | — | A01 **MildGronwall**（A3 最后一个分析输入）：vendor 正则化 mild 能量链 `mild_majorized_energy_subinterval` 特化到常欧氏度量与 q+1 阶有限 word 族，顶阶非线性项走 A03.outerProductTame → A04.outerSobolevNormAt_le → A04.inner_energy_Rhigh；显式 E q, C q |
