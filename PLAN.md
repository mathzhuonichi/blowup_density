...
| 248-SPEC-r41-spec | 完成（sol，9 分钟）：Spec.lean 可 elaborate；COMPARISON 含绑定计划；lead 复核；PR 已开，链排在 #239 之后（wave 68） | 09-16 1613Z | #240 | — | SPEC **R41 定稿**：按 research/R41/RECONCILIATION.md 合并草案 A/B（+ STATEMENTS 骨架）→ Spec.lean + COMPARISON.md（含"绑定计划"：232/233/235 的定理如何绑各字段） |
| 249-R41-contract-v1 | worktree 安装中→自动启动（codex astra low，带重试；基于 248 分支） | 09-16 1613Z | — | — | R41 **V1 合同注册**（第 32 个合同 R41.main_thresholds）：MainThresholdsAPI = research/R41/Spec.lean 的 RMainAPI 逐字；Bindings 按绑定计划接 235/232/233 + R42 V2 字段（rider 用同一记录）；若某字段不可绑则注册 partial 变体并报告缺口 |
