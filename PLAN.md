...
- **C01 U3 发现（050）**：`difference_energy_bound` 需要 `∇p(t)` 为全阶 `SmoothL2Field`，而 `ClassicalSolutionR` 只给 `∇p` 的零阶 `MemLp` + 空间光滑；要么加**椭圆压力正则性单元**（−Δp = div((u·∇)u) ⇒ ∇p ∈ H^∞，新单元 P1），要么 D01 V2 给 `ClassicalSolutionR` 加压力 datum 子句（论文「压力如上确定」支持后者）。时间导数 datum 由动量方程读出（U4/U7）。
