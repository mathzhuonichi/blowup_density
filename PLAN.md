...
| 208-A01-datum-horizon | 审稿 ACCEPT（9 公理；符号变异按预期失败）；PR 已开，合并链运行中（wave 41） | 09-16 0511Z | #213 | — | A01 **合同注册 1/3**：初值识别 velocity(0,·)=a（连续 + a.e.）、initialClassR 桥、总函数 localHorizon 与 LocalTheoryAPI 的 solution 字段 |
| 209-A01-manuscript-regularity | worker 完成（astra：manuscriptLocalRegularity_of_pipeline 四字段含 t=0 的压力恢复；接口为任意构造器输出 w + hslice/hpaths；11 声明公理干净）；codex 审稿中（重点 t=0 压力恢复不经环境时间导数） | 09-16 0511Z | #— | — | A01 **合同注册 2/3**：ManuscriptLocalRegularity 四字段（全阶 C∞ 数据路径经 hslice；eq:Rpressure 含 t=0 由 189/194 的 Leray 余项恒等式；projected 由 momentum；径向势规范） |
