...
| 265-SPEC-t13-draft-a | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 1902Z | — | — | T13 **lem:localization 双盲草案 A**（第 3 节真正的新分析；从一开始可并行的叶子）：Gagliardo 双积分 I_R/I_T、核 K_s 与格点尾和、常数 c_s、eq:localization 的一致常数——LocalizationAPI 陈述 + COMPARISON_A |
| 266-SPEC-t13-draft-b | worktree 安装中→自动启动（codex astra low，带重试） | 09-16 1902Z | — | — | T13 双盲草案 B：同上，独立模型 |
