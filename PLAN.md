...
| 231-R44-contract-v1 | 审稿 ACCEPT（sol，无修改）；分支已并入集成分支；PR 已开，合并链运行中（wave 64）；第 31 个合同 | 09-16 1441Z | #236 | — | R44 **V1 合同注册**（第 31 个合同 R44.critical_finite_horizon）：CriticalFiniteHorizonAPI = Spec 的 RCritical2API 逐字；Bindings 见证来自 229 的 Prop44（c=θ/20，C=3，radius 显式）；Tests；registry |
