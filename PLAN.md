...
| 187-A01-hfs-force-path | 审稿 ACCEPT-WITH-NOTES（消费者探针接通 186；负变异按预期失败；备注仅为旧基线与措辞）；PR 已开，合并链运行中（wave 20） | 09-15 2200Z | #193 | — | A01 **B1 供给 hfs**：`C01.forcePath hf` 的柱值路径 `sobolevPath F hF q` 在时间上 C^∞（MemForceR 的光滑 datum 路径经连续线性重构）；178 审稿确认树里缺这座桥，178/186 都以它为输入 |
