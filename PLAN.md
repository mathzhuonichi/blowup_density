...
| 079-D01-p2-sl4-transverse | 审阅中 | — | — | P2 SL4 Fourier 横向形式：无散 `SmoothL2Field` 的任一 (m+1) 阶 datum 满足 a.e. `∑ⱼ ξⱼ Âⱼ(ξ) = 0`（经 066 `isSobolevDatum_partialDeriv` + 标量 datum 唯一性 + 符号消去），再推论到 `∂ₜu(t,·)`（D2 + 074 SL4α + `A05.SmoothL2` 包装），`Section4/D01/Transverse.lean` |
...
| 082-A04-sl3-real-pairing | 续改中 | — | — | A04 SL3 步骤 3a：实载体 `RealSobolevHilbert` 上的反自伴 `⟪f, D_a g⟫_ℝ = -⟪D_a f, g⟫_ℝ`（`angularDirectionalDerivativeReal`，实 `L2.inner_def`）、降阶配对 `⟪Λ⁻¹w, Λw⟫ = ‖w‖²`、降阶符号只依赖 r−s，`Section4/A04/RealPairing.lean` |
