...
| 217-A04-shifted-extension | worktree 安装中→自动启动（codex astra low，带重试；基于 215 分支） | 09-16 0905Z | — | — | A04 **平移拼接**：证 215 的唯一具名输入 ShiftedLocalExtension——把从 b 重启的经典解与原解拼成 [0,b+L) 上的经典解（速度在重叠区由唯一性相等；压力用时间截断凸组合避免跨 T 延拓常数），推出 maximalLifespanR ≥ b+L；给出无具名输入的 restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite（MemForceR 力） |
