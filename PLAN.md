...
| 219-R43-critical-momentum | worker 完成（astra）：criticalDatumInputs_of_classical 无条件（216 乘子为 CLM + 齐次唯一性 + 局部载体窗口覆盖 + A04.momentum_datum 二阶导数恒等式 + lowerVectorL 传输）；exists_criticalDatumPath' 与 rcritical1_of_classical' 无任何具名输入；215 的 7 条支撑声明在 R43.CarrierWindow 下复刻（基线无 215）；审稿中 | 09-16 0940Z | — | — | R43 **动量传输**：闭合 216 的 CriticalDatumInputs——半阶速度路径的时间光滑性（215 的整数阶光滑路径 + lowerVectorL + 216 乘子为 CLM）与数据层动量恒等式（局部载体窗口覆盖 + 169 导数=投影残差 + 189/195/197 Leray 识别 + 齐次数据唯一性）；给出无条件 exists_criticalDatumPath 与 rcritical1_of_classical |
| 220-R44-s1c-trilinear | worktree 安装中→自动启动（codex sol xhigh，带重试；基于 218 分支） | 09-16 0936Z | — | — | R44 **S1c**：J 权三线性估计 abs⟪(u·∇)u,Ju⟫ ≤ C₀·Y·(Y²+Z²)（三因子 L³ Hölder + 165/182 临界嵌入 + 164 齐次≤非齐次 + 218 的 Jmul_norm/weight_identity）；静态逐切片；新增 AdvectionJDatum 不改 JWeightDatum |
| 221-R43-force-path | worktree 安装中→自动启动（codex sol xhigh，带重试；基于 219 分支） | 09-16 0941Z | — | — | R43 **G2/G3/G4 时间路径**：b(t)=‖f(t)‖_{Ḣ^{1/2}}（214 的 criticalForceAt）连续、可积、原函数 N 的 FTC 与单调性（匹配 Paper1.critical_norm_bound / criticalNormBound_radius 的假设形状）、G2 时间积分单调性；探针：a=0 的 bootstrap y(t) ≤ cν 仅条件于 ν>0、MemForceR、小性 |
| 222-R44-s1b-energy-identity | worktree 安装中→自动启动（codex astra low，带重试） | 09-16 0949Z | — | — | R44 **S1b**：沿经典解微分 Y²=‖u‖²_{H^{1/2}}（215 光滑整数阶路径经 lowerVectorL 降到 1/2 + DerivNorm），(Y²)′=2⟪∂ₜu,Ju⟫，A04.momentum_datum 二阶动量恒等式降阶，压力项由 Leray 正交消去，耗散项 ⟪νΔu,Ju⟫=−νZ²；得 energy_identity |
