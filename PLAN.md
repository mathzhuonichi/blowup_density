...
| 211-A01-local-theory-bundle | worktree 安装中→自动启动（codex astra low，带重试；基于 210 分支 + 集成的 208/209） | 09-16 0519Z | — | — | A01 **局部理论装配（不碰 Contracts）**：载体见证捆绑结构 LocalCarrier、用 210 的一致定理选视界 localHorizon′（反单调）、209 正则性实例化、horizon_lower_bound 的两个变体（H⁷/固定力已证；H¹ 原文仅 def 不断言）、LocalTheoryAPI 形状探针；V2 注册（212）等用户决定 |
