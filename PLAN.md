...
| 366-T12-U1-haar-cube | Opus 补全（f0d31524）：全 p 传输（含 0/⊤）经单一测度恒等式 map_torusChart；11 定理；0 warning → 排队复审 | 09-18 1306Z | — | — | T12 U1：Haar ↔ 立方体 ↔ 全空间的 L^p 传输（周期场；支撑在 Q 内的场；标量/向量；periodicLpENorm 桥） |
...
| 376-T15-U2-placement | codex（astra）只交假设支撑包含下的引理；审稿 REJECT（需证仿射像 ⊆ chart 球 ⊆ 立方体、紧性、HasCompactSupport；真实 PlacementData probe）→ 排队 Opus 补全（等空位） | 09-18 1313Z | — | — | T15 U2：缩放支撑放置（各切片支撑 ⊆ x₀ + ε•K_* ⊆ chart 球 ⊆ interior fundamentalCube；紧支撑）供 U3–U8 |
