...
| 254-R45-compact-rider | worker 完成（sol，7 分钟）：regularReference_compact（Spec 字段在 Y = F_c 下，233 记录一次调用 + 249 的路线，ε ≤ min 1 ((T−τ)/4) 使历史覆盖任意 τ）+ regularReference_of_memForceR（F_R 的 ε 形式）；分支已并入集成分支；审稿中 | 09-16 1648Z | — | — | R45 **F_c 的正则参考 rider**：Spec 的 regularReference 在 Y = forceClassCompact 下（249 的 R41 rider 绑定为模板：233 记录一次调用 + 唯一性对齐 + R42 history/forceConvergence/速率夹逼；ε 形式与任意历史截断 τ 由极限导出；f ∈ F_c 由紧支撑闭包） |
| 255-I03-path-measurability | worktree 安装中→自动启动（codex sol xhigh，带重试；基于 250 分支） | 09-16 1642Z | — | — | I03 **路径可测性**：证 250 的唯一具名输入 CompactHomogeneousRealization（紧支撑光滑力的 compactHomogeneousPath 在 forceTimeMeasure 下 a.e. 强可测：切片连续 + 齐次实现连续，−3/2<s<0 的低频可积），从而 packet/correctionNegativeHomogeneous 无条件 |
