...
| 338-T11-U9e-existence-input-h3 | codex 审稿 REJECT 仅因流程（改了 MildClassical.lean —— 即 334 的修复，已随 #310 合入）→ Opus 重基 + 负向变异记录后合入 | 09-18 0728Z | — | — | T11 U9e（lead 修正 2）：证 `PeriodicQuantitativeLocalInputH3`（H³ 球一致 δ）→ 存在性线闭合；H¹ 球版本保留为手稿具名未证谓词（照第 4 节 ManuscriptHorizonLowerBoundH1）；重实例化 restart/extendsBeyond/lifespan 的 H³ 版本；写 H1_GAP.md |
