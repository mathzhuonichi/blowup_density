...
| 222-R44-s1b-energy-identity | 首轮（astra）写出 EnergyIdentity.lean 草稿后被 model-at-capacity 切断（未提交）；续跑（fix 标签）休息 30 分钟后启动 | 09-16 0954Z | — | — | R44 **S1b**：沿经典解微分 Y²=‖u‖²_{H^{1/2}}（215 光滑整数阶路径经 lowerVectorL 降到 1/2 + DerivNorm），(Y²)′=2⟪∂ₜu,Ju⟫，A04.momentum_datum 二阶动量恒等式降阶，压力项由 Leray 正交消去，耗散项 ⟪νΔu,Ju⟫=−νZ²；得 energy_identity |
