...
| 187-A01-hfs-force-path | worker 完成（datumSobolevCLM 连续线性重构 + forcePath_sobolevPath_contDiffOn 对所有 q；hfs 完全卸下；17 声明公理干净）；codex 审稿中 | 09-15 2147Z | #— | — | A01 **B1 供给 hfs**：`C01.forcePath hf` 的柱值路径 `sobolevPath F hF q` 在时间上 C^∞（MemForceR 的光滑 datum 路径经连续线性重构）；178 审稿确认树里缺这座桥，178/186 都以它为输入 |
| 188-A01-mild-uniqueness | 对抗式审稿 ACCEPT（因果窗口界正确、常数来自树、覆盖端点；减半质量变异按预期失败）；PR 已开，合并链运行中（wave 18） | 09-15 2147Z | #191 | — | A01 **MildUniqueness**：6 阶 mild 解（quadraticDuhamel 不动点）在整个 [0,S] 上唯一——186 的唯一命名输入；路线：小窗口压缩（vendor mild_solution_unique 要 kernelMass·L<1）+ 重启/首次分离时刻迭代 |
...
| 190-A01-b1-r4-joint-smooth | worktree 安装中→自动启动（codex sol xhigh） | 09-15 2148Z | — | — | A01 **B1 R4 联合光滑代表元**：由 178 的全阶 C^j_t 数据路径造一个 u(t,x)，切片 a.e. 等于 U t 且在 [0,S)×R³ 上联合 C∞（Sobolev 嵌入 + 连续线性求值）——180 的 `hc3`；新增可满足性规则 |
