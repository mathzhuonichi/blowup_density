...
| 344-T13-constant-endpoints | Opus 完成：constant_pos_finite / endpoint_zero / endpoint_one 全证（ConstantEndpoints.lean 35 声明，无残余）→ 排队 codex 审稿 | 09-18 0809Z | — | — | T13 证明：`constant_pos_finite`（cFrac 正有限）、`endpoint_zero`、`endpoint_one` |
| 345-T13-torus-identity | Opus 完成：torus_identity 全证（TorusIdentity.lean 1183 行 83 声明；半开立方体铺砌 + periodicFourierCoeff_shift + 核旋转/伸缩）→ 排队 codex 审稿 | 09-18 0833Z | — | — | T13 证明：环面 Gagliardo 恒等式 `torus_identity`（Fourier 展开 + 周期化核展开为全空间积分 + 核缩放） |
| 346-T14-packet-energy | codex sol 完成：energy_le_work / work_eq_square 由 PacketAPI 子句证出（PacketEnergy.lean 216 行），probe 关闭 packetImportStatement + PacketImportFamily → 排队 codex 审稿 | 09-18 0823Z | — | — | T14 证明：`lem:packetenergy` 两字段（energy_le_work / work_eq_square）由 vendor CompactEnergy 能量恒等式 + Paper1 sqrt_energy_le_primitive；probe 用 Bindings.packet 关闭 packetImportStatement / PacketImportFamily |
...
| 350-SPEC-t24-spec | Opus prover 子代理运行中（statement-only；基座含 RECONCILIATION.md） | 09-18 0824Z | — | — | SPEC **T24 定稿**（343 重做）：按 research/T24/RECONCILIATION.md（B 为基；prop:affine 全空间基于 I01.packet；prop:multiple Type 值、prop:conservative Prop；§2 修正强制）；反 stub 条款 |
| 351-T16-ball-potential | Opus prover 子代理运行中（基座 = 347 分支） | 09-18 0831Z | — | — | T16 缺口 1：球上径向势 timePotential_contDiffOn_ball / spatialCurl_timePotential_on_ball（空间 bump 截断 + I02 全局引理 + congr_slice）→ exists_potential_on_ball（potential_smooth/formula/curl 三字段） |
...
| 353-T13-localization-kernel | worktree 安装中→Opus prover 子代理（基座 = 345 + 344 分支 + integration-section3） | 09-18 0834Z | — | — | T13 `localization` 第 1 部分（核估计）：格点尾和一致界 tailConst、`ITorus s (periodize f) ≤ IReal s f + 4·tailConst·‖f‖₂²`、T³ 上 periodicSobolevENorm ≤ L² + 齐次；装配（需 348）留给 354 |
