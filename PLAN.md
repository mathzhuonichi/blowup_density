...
| 116-B02-homogeneous-v2 | 审阅中 | — | — | B02 `homogeneous_partial` **V2** 合同：`extends` V1 + `separatedAssembly`（在 `−3/2<s` 上，103）+ `approxCompactHomogeneous`（110，`SplitRange`）；`annularPathApprox` 仍排除（无证明、无消费者）；Bindings/Tests/contracts.json（第 20/21 条） |
