...
| 308-T11-U1-flow-conversion | 完成（toFlow/ofFlow + 两条拼写引理 + 力类单向转运）→ codex 审稿中（rev-308） | 09-18 0136Z | — | — | T11 U1：`ClassicalSolutionT ↔ Paper1.PeriodicLifespan.Flow` 逐字段转换 + 往返 + 两条拼写引理（S–M，sol） |
...
| 313-T11-U9b-existence-construction | 排队（待 311 合入；astra；简报已写） | 09-18 0130Z | — | — | T11 U9b：按 311 定的 R2 路线，具名输入改为按阶力界的 `PeriodicQuantitativeLocalInput'`（lead 修正 1，`research/T11/LEAD_AMENDMENTS.md`）；证 EXISTENCE_ROUTE 的前两项（投影对流符号的双线性估计、强迫 Picard 不动点与寿命下界） |
