...
| 247-R47-grid-lemmas | worker 完成（sol，9 分钟）：exists_ball_in_common_cell（经 Paper3.finite_grids_common_ball）、gridObservation_locality（支撑在 cell 内 + 可积 + 该 cell 上差的积分为零 ⇒ 观测相等；速度差的零积分由 div-free 得，力差需论文 eq:gridforce 的动量方程论证——留给装配 lane）；Bindings/GridLemmas.lean；审稿中 | 09-16 1523Z | — | — | R47 **网格引理**：有限多个网格的公共格子里选球（exists_ball_in_common_cell）+ gridObservation 局部性（支撑在某格子内且（若论文需要）该格子上均值为零 ⇒ 观测相等）；Bindings 层，使用注册的 Grid/gridObservation |
