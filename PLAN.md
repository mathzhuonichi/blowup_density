...
| 131-C01-energy-split | 续改中 | 09-14 0156Z | — | — | C01 剩余能量/涡量字段拆分起步（`energyIdentity`/`energyDifferentialBound`/`l2Bound`、`enstrophyIdentity` 等，Spec.lean:344-541）：对照 A04 已有的 `inner_energy_assembly`/`TimeDerivative`/`Continuity` 与 121 的 `pressure_drop`（m=0/1 实例），产出 `research/C01/ENERGY_SPLIT.md` + 证第一个 S 单元 |
