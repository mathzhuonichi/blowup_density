...
| 212-A01-contract-v2 | 审稿 REJECT 四点（基线、H¹ rfl 桥、论文 vs 草案出处、措辞）均由 lead 修正并重建；注册文件并集合并后 PR 已开，合并链运行中（wave 85）；第 36 个合同 | 09-16 1747Z | #257 | — | A01 **V2 合同**（选项 1，用户 2026-09-17 决定）：horizon_lower_bound 固定外力 + H⁷ 措辞注册；论文 H¹ 句子逐字保留为命名的未证定义；每字段 docstring 写清差异；非零外力/初值的非空洞实例；COMPARISON 加"论文 vs V2"对比表 |
| 230-A04-v2-restart-fixed-force | 已合入 #256（wave 84 门禁 OK）；第 35 个合同 A04.continuation_v2 | 09-16 1746Z | #256 | — | A04 **V2 合同**（选项 1）：Restart = RestartFixedForce（固定外力、对 t₀∈[0,S] 与 H⁷ 数据一致）+ HigherOrderBound + 217 形状的三条延拓定理；V1 的 H¹ Restart 留作明确未证；同样的防 reward-hacking 要求 |
