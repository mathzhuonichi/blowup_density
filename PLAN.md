...
| 385-T17-U5U6-derivative-bounds | codex sol 完成；sol 审稿 13:28Z 死于容量 → 改 astra 重审 | 09-18 1329Z | — | — | T17 U5+U6（纯传输）：correction_derivative_bound / force_derivative_bound（correctionData_correction + latticeLift_iteratedFDeriv_eq + Paper1 混合导数界） |
...
| 392-T24-Uc1-Ua1 | worktree 安装中→等 codex 空位启动（sol） | 09-18 1334Z | — | — | T24 wave 1（codex）：Uc1 zero_from_rest（prop:conservative，ConservativeForce.lean:91）+ Ua1 五个几何/运动学字段（prop:affine，原始包字段版） |
| 393-T22-UA5-orderzero | worktree 安装中→Opus prover 子代理（基座 = 387 分支 + integration-section3） | 09-18 1336Z | — | — | T22 U-A5：orderZero 字段（0 阶域 Sobolev 范数 = L²(Ω) 范数；387 等距 + 383 限制桥 + 商范数恒等式） |
