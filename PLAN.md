...
| 217-A04-shifted-extension | 审稿 ACCEPT（sol，无修改；b+L+1 变异按预期失败）；PR 已开，合并链运行中（wave 48）；shiftedLocalExtension 无条件；A04 延拓推论无具名输入 | 09-16 0923Z | #220 | — | A04 **平移拼接**：证 215 的唯一具名输入 ShiftedLocalExtension——把从 b 重启的经典解与原解拼成 [0,b+L) 上的经典解（速度在重叠区由唯一性相等；压力用时间截断凸组合避免跨 T 延拓常数），推出 maximalLifespanR ≥ b+L；给出无具名输入的 restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite（MemForceR 力） |
