...
| 189-A01-pressure-regularity | fix3 完成（import 180 的 PressureSupply；A01 构造器管线探针：仅由 hb 得到 ClassicalSolutionR）；lead 复核并裁掉审稿文件里 2.8 万行 make check 输出；PR 已开，合并链运行中（wave 31） | 09-16 0055Z | #203 | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
