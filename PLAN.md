...
| 337-T11-U14-U16-extends-lifespan | Opus prover 子代理（基座 = integration + 332 + 322 本地合并） | 09-18 0605Z | — | — | T11 U14+U16：`extendsBeyond`（H² 判据 → higherOrderBound(m=1) 的 H¹ 轨迹界 → 332 的 restartBeyond）与 `lifespanInfiniteOfLocallyFinite`（反证）；higherOrderBound 陈述作显式假设，唯一具名输入为存在性输入 |
