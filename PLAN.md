...
| 247-R47-grid-lemmas | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 1514Z | — | — | R47 **网格引理**：有限多个网格的公共格子里选球（exists_ball_in_common_cell）+ gridObservation 局部性（支撑在某格子内且（若论文需要）该格子上均值为零 ⇒ 观测相等）；Bindings 层，使用注册的 Grid/gridObservation |
