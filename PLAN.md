...
| 215-A04-restart-fixed-force | 审稿 ACCEPT（sol，无修改）；PR 已开，合并链运行中（wave 47）；restartFixedForce_of_memForceR 与 higherOrderBound_of_gronwall 无条件；延拓固定力副本条件于 ShiftedLocalExtension（217 已闭合） | 09-16 0916Z | #219 | — | A04 **固定力重启**：RestartFixedForce（一个 δ 对所有 t₀∈[0,S] 与 H⁷ 重启数据；力平移的 sup 范数由 [0,S+1] 上界控制 + 211 的反单调选择/vendor forced_uniform_restart_time）、179 Grönwall→HigherOrderBound 桥、A04 三条延拓定理的固定力副本并实例化；不改 owner 的 Restart 定义 |
| 216-R43-critical-datum-path | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 0852Z | — | — | R43 **半阶载体 G3/G4**：为经典解构造 CriticalDatumPath（六条 1/2 阶数据路径 + 符号恒等式 + 横向/纵向 + velocityHalf 光滑与动量恒等式，169 的齐次类比）；装配 exists_criticalDatumPath，使 214 的 eq:Rcritical1 去掉 hcrit 绑定；若动量恒等式不闭合则隔离为 CriticalDatumInputs |
| 217-A04-shifted-extension | worker 完成（astra，约 50 分钟）：shiftedLocalExtension 无条件（A02 velocity_unique_core/pressure_gauge_core + normalizePressure 内点拼接）；restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite 的 _of_memForceR' 无具名输入；审稿中 | 09-16 0914Z | — | — | A04 **平移拼接**：证 215 的唯一具名输入 ShiftedLocalExtension——把从 b 重启的经典解与原解拼成 [0,b+L) 上的经典解（速度在重叠区由唯一性相等；压力用时间截断凸组合避免跨 T 延拓常数），推出 maximalLifespanR ≥ b+L；给出无具名输入的 restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite（MemForceR 力） |
