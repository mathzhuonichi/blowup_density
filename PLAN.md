...
| 180-A01-b2-assembly | fix3 完成（以 190 的 velocity/hslice/hc3 为参数；管线探针 192→190→189→180）；lead 直接追加 fix4（压力腿同病：pressureGradientOfVelocity 在 t=0 用双侧 fderiv 不可满足 → 以供给方的 G + 内点恒等式为参数）；fix4 运行中 | 09-15 2237Z | #190 | — | A01 **B2 装配**：用已合入部件（161/162/167/168/169/173 + 153/157）装出有条件的 ClassicalSolutionR 构造子 carrierConstructor_of_localTheory，只留联合光滑性 hc3（B1 R3/R4）与可能的 hcurl 为具名假设；CarrierConstructorFull 形状 + 消费者环探针 |
...
| 189-A01-pressure-regularity | 审稿 REJECT（Helmholtz 逆向正确，但 htime 对 [0,S) 上的场在 t=0 一般为假（环境 fderiv 是双侧的）；hmom 是比 169 导出更强的物理谓词且要 t=0 的导数数据；4 个输入是哑的）；方案：压力梯度改由残差的 Leray 余项数据路径 + 190 的联合光滑代表元定义，内点与 residual−∂ₜu 恒等——等 190 落地后写 fix | 09-15 2234Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
| 190-A01-b1-r4-joint-smooth | worker 完成（exists_joint_smooth_representative：切片 a.e. = U t 且 Icc 上联合 C∞；_of_hall 接 178；26 声明公理干净）；codex 审稿中 | 09-15 2241Z | #— | — | A01 **B1 R4 联合光滑代表元**：由 178 的全阶 C^j_t 数据路径造一个 u(t,x)，切片 a.e. 等于 U t 且在 [0,S)×R³ 上联合 C∞（Sobolev 嵌入 + 连续线性求值）——180 的 `hc3`；新增可满足性规则 |
| 191-A05-u4-shifted-data | worker 完成（rieszLambda 物理 Λv 实现 + MemHInfty + 半阶数据；∂_j v 半阶数据与 Riesz 符号；shiftedCriticalData_of_memHInfty；经典解切片的 criticalAdvectionLpBridge_shifted；桥只剩 Parseval 配对；公理干净）；codex 审稿中 | 09-15 2250Z | #— | — | A05 **U4/U8 → R43 桥的 shifted 字段**：H^∞ 场的物理 Λv 实现（Ḣ^{1/2} 数据 = v 的 Ḣ^{3/2} 数据）与 ∂_j v 的半阶数据 + Riesz 坐标符号；打包成经典解切片的 `ShiftedCriticalData`；Parseval 配对留给下一条 |
| 192-A01-wiring-hsob | fix 完成（合并导出单一 U；零数据/零力半径 0 的真实先验界由唯一性证出；路线备注改为基阶半径）；codex 复审中 | 09-15 2242Z | #— | — | A01 **接线**：186′+188（公共视界无条件）+187（hfs）+178（全阶 C^j 数据路径）→ 仅由全阶先验界 hb 得到构造器的 hsob/hU/hdiv；记录 A01 管线剩下的唯一分析输入（A3-M2 的 hb，加 190 的 hc3、189 的 hpg） |
| 193-A01-a3-m2-bounds | worker 完成（astra low：基阶界（vendor 局部存在 + 188 唯一性）、子窗口降阶识别、H² 帽转移、标量 Grönwall；hb_of_base(Inv) 条件于唯一命名输入 MildGronwall（有限阶 mild 能量不等式）；显式半径；14 声明公理干净）；codex 审稿中 | 09-15 2247Z | #— | — | A01 **A3-M2 全阶先验界族 hb**（最后一个 A3 分析单元）：基阶 6 的局部视界与半径先定 → 高阶解降阶+唯一性（186/188）等于基阶解 → H² 积分帽用基阶半径 → mild 级 Grönwall 给每个 R q；若 mild 级 Grönwall 缺则单独命名 |
