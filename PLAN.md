...
| 205-A01-coordinate-tame | 已合并（#210，wave 38 门禁绿）；补审在跑 | 09-16 0459Z | #210 | — | A01 **CylinderCoordinateTame**：逐坐标柱面 Kato–Ponce/Moser 估计——word Leibniz 展开 + 柱面/下降 tame 插值引理 + 光滑逼近到相容有限元；允许 re-cut 到角不变子空间；先证存在常数 |
| 206-A01-smooth-tame | 已合并（#211，wave 39 门禁绿）；补审在跑 | 09-16 0459Z | #211 | — | A01 **SmoothCylinderCoordinateTame**（A3-M2 最后一条分析输入）：先走角不变 re-cut + 下降到 R³ 用 A03 tame 积（若成立则整个 A01 只剩接线，并导出无条件构造器探针）；退路是柱面插值本体或孤立精确的 R³ tame 不等式 |
