...
| 359-T13-localization-assembly | Opus prover 子代理运行中（基座 = 354 分支 + integration-section3） | 09-18 1016Z | — | — | T13 `localization` 装配：torus_identity + endpoint_zero + constant_pos_finite + 353 两估计 + 354 核比较 + 348 全空间恒等式 + Parseval-at-0 桥 → `localization` 字段 → LocalizationAPI 装配 + 合同（T02.localization） |
...
| 362-T15-U1-bridges | sol/astra 各死一次于容量（留部分 Bridges.lean）→ 包装再次重启（sol 优先） | 09-18 1027Z | — | — | T15 U1：规范模块 Section3/T15/Bridges.lean（重标度定义）+ 到 I03.scaling 上游拼写的 rfl 桥 + 352 的 periodize 桥；probe api_on_canonical |
...
| 364-T15-UTB1-haar-bridge | Opus 完成（无残余）；astra 审稿 10:23Z 死于容量 → 包装 30 分钟后换 sol | 09-18 1024Z | — | — | T15 U-TB1：Haar/Lebesgue 单拷贝能量桥 eLpNorm (torusLift (periodize f)) 2 Haar = eLpNorm f 2 volume + 梯度伴随 + 时间切片形式 |
...
| 366-T12-U1-haar-cube | sol 中途 at capacity（仅留草稿 probe）→ 包装重启（sol 优先，astra 备用） | 09-18 1006Z | — | — | T12 U1：Haar ↔ 立方体 ↔ 全空间的 L^p 传输（周期场；支撑在 Q 内的场；标量/向量；periodicLpENorm 桥） |
...
| 370-T17-U3-correction-profile | Opus 完成：六个轮廓字段（CorrectionProfile.lean 11 声明；def 桥 rfl）但需前提 hv : ContDiff ℝ ∞ v（Paper1 引理全局光滑；CorrectionAPI 无 reference_smooth → U12/spec 议题 G1）；identity 非空性待 358 装配 → 排队 codex 审稿 | 09-18 1033Z | — | — | T17 U3：固定柱面修正轮廓六字段（rescaledCorrectionProfile = Paper1 CorrectionProfile.profile 桥；smooth/support/Const/nonneg/uniform/identity） |
