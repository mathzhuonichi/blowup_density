...
| 191-A05-u4-shifted-data | 审稿 ACCEPT（79 公理检查标准；符号变异按预期失败）；PR 已开，合并链运行中（wave 22） | 09-15 2301Z | #195 | — | A05 **U4/U8 → R43 桥的 shifted 字段**：H^∞ 场的物理 Λv 实现（Ḣ^{1/2} 数据 = v 的 Ḣ^{3/2} 数据）与 ∂_j v 的半阶数据 + Riesz 坐标符号；打包成经典解切片的 `ShiftedCriticalData`；Parseval 配对留给下一条 |
