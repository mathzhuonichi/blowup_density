...
| 098-R42-full-horizon | 审阅中 | — | — | R42 导出：由每个 `Ico 0 S`（S<T）上的 datum 路径经唯一性粘成 `Ico 0 T` 上的连续路径（S–M），从而 `ClassicalSolutionR ν a g_ε T`（水平线恰为 T）；A02 `IsMaximalSolution`/`insertion_lifespan_eq` 半边的表，`Section4/R42/FullHorizon.lean` |
| 099-B02-partial-contract | 审阅中 | — | — | 注册 `B02.homogeneous_partial`：16 个已证字段（含带可积性形式的 `homogeneousDatumSub` + 反例注释）逐字进 `Contracts/V1/HomogeneousPartial.lean` + Bindings + Tests + contracts.json（模板 071/091），把 B02 八个模块纳入 CI 闭包；缺 `separatedAssembly`/`annularPathApprox`/`approxCompactHomogeneous` |
...
| 102-A04-sl5-columns-norms | 进行中 | — | — | A04 SL5 行 5b/5f/5g（均 S）：列 `W_j = u_j • u` 的 m+1 阶 datum（`tameProductVector`/`outerProductTame`）；`√(∑‖D_j G'‖²) = gradientSobolevNormAt`；`√(∑‖B_j‖²) ≤ (outerSobolevENorm …).toReal`，`Section4/A04/NonlinearColumns.lean` |
| 其余节点 | 未开始 | — | — | |
