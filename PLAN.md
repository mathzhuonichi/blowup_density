...
| 078-B02-unit-2-split | PR 已开 | #80 | — | B02 单元 2 `annularSchwartz`（L）拆分起步：`|ξ|^{-s}·g` 在环上光滑紧支（SL1）、Schwartz 逆角 Fourier（SL2）、实值性（SL3）、组装到 `IsHomogeneousSliceDatum`（SL4）；`research/B02/U2_SPLIT.md` + `Section4/B02/AnnularSchwartz.lean` |
...
| 080-R42-blowup-esssup | 审阅中 | — | — | R42 子项 2a：逐点 `SpeedUnboundedAt T u` + 切片连续 ⇒ `limsupLeft T (speedENorm (u(t,·))) = ⊤`（开集正测度 ⇒ essSup 下界；limsup=⊤ 的 frequently 刻画），`Section4/R42/BlowupEssSup.lean` |
...
| 083-R42-pressure-gradient | 进行中 | — | — | R42 子项 1f：`∇p_ε = ∇π + ∇P_ε ∈ L²`（参考压力梯度 L² + 修正压力空间紧支光滑 ⇒ 梯度 L²，`MemLp.add`），`Section4/R42/PressureGradient.lean` |
| 其余节点 | 未开始 | — | — | |
