...
| 180-A01-b2-assembly | fix6 完成（PressureSupply 带 ha + 194 形状的同载体全阶 hpairs + hpaths + 190 代表元数据；消费者环用 cylinderPair_of_bounds 保留族；真实 194→195→197 管线探针；0 behind）；第六轮 codex 审稿中 | 09-16 0018Z | #190 | — | A01 **B2 装配**：用已合入部件（161/162/167/168/169/173 + 153/157）装出有条件的 ClassicalSolutionR 构造子 carrierConstructor_of_localTheory，只留联合光滑性 hc3（B1 R3/R4）与可能的 hcurl 为具名假设；CarrierConstructorFull 形状 + 消费者环探针 |
...
| 189-A01-pressure-regularity | 装配简报已写（194 的 G + 195/197 的内点恒等式 + Helmholtz 逆向 ⇒ PressureSupply）；挂在 197 合入后自动以 astra 启动 fix2 | 09-16 0004Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
...
| 197-A01-leray-bridge | 复审 REJECT（数学正确、25 声明公理标准；但 residual_difference_gradient 超 400000 heartbeats、落后 29 提交、未按 fix6 binder 导出）；fix2 运行中（astra：rebase、拆证明降 heartbeats、按 PressureSupply binder 导出并回放落地的 195 消费者） | 09-16 0019Z | #— | — | A01 **P4c 投影桥 hprojected**（195 的唯一命名输入）：柱面 Leray 投影 `leray 1 q` 与数据级 Fourier `lerayComplement 0` 在下降数据上一致——梯度部分纵向、投影部分无散、物理残差数据与下降柱面残差一致（`physicalResidual_datum_eq`，194 也用） |
| 198-A01-energy-premises | worker 完成（astra：vendor 能量定理全部前提（恒等度量、全 word 族、限制恒等式、任意竞争者的无散保持、最大正则态）+ 积分根能量估计（Z 显式、耗散已丢）+ finiteMildEnergy_of_estimate 恰以 ForcingFamilyBound/EnvelopeConversion 为输入；负例说明仅由积分估计不能恢复带耗散的包络；27 声明公理干净）；codex 审稿中（重点 EnvelopeConversion 可满足性与 199 路线） | 09-16 0000Z | #— | — | A01 **A3-M2 能量前提**（FiniteMildEnergy 上半）：为柱面 NS 解构造 vendor `mild_majorized_energy_subinterval` 的前提（无散路径、梯度压力、最大正则 Bochner 态 U∈TimeLp(H^{q+2})、源/压力代表元及限制恒等式），恒等度量、全 word 族 N=q−5；导出积分根能量估计，Z 的界与包络转换留给 199 |
