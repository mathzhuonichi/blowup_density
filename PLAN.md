...
| 119-A01-c1b-split | PR 已开 | #121 | — | A01 L 单元 **C1b** 拆分起步：D01 角向 datum `IsSobolevDatum m` ⟷ Euler `ordinarySobolev`/`EulerMeanSolenoidal.L2` 坐标（经 cylinder `ordinaryLift` 伴随）逐阶桥；产出 `research/A01/C1B_SPLIT.md`（S/M 子引理表 + file:line 输入）+ 证第一个 S 单元 |
