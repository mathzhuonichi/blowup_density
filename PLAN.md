...
| 193-A01-a3-m2-bounds | fix 完成（astra：探针改用 192 最终导出；记录 196 路线）；PR 已开，合并链运行中（wave 24） | 09-15 2313Z | #197 | — | A01 **A3-M2 全阶先验界族 hb**（最后一个 A3 分析单元）：基阶 6 的局部视界与半径先定 → 高阶解降阶+唯一性（186/188）等于基阶解 → H² 积分帽用基阶半径 → mild 级 Grönwall 给每个 R q；若 mild 级 Grönwall 缺则单独命名 |
