...
| 192-A01-wiring-hsob | fix2 完成（合并导出在同一 U,u 上带齐角不变性、Duhamel、j=0 与全 j 族；探针经 190 接口消费全部输出）；第三轮 codex 审稿中 | 09-15 2258Z | #— | — | A01 **接线**：186′+188（公共视界无条件）+187（hfs）+178（全阶 C^j 数据路径）→ 仅由全阶先验界 hb 得到构造器的 hsob/hU/hdiv；记录 A01 管线剩下的唯一分析输入（A3-M2 的 hb，加 190 的 hc3、189 的 hpg） |
| 193-A01-a3-m2-bounds | 审稿 ACCEPT-WITH-NOTES（MildGronwall 诚实；基阶界与半径精确；256→255 变异按预期失败；备注：组合探针改用 192 的最终导出、记录 vendor 正则化 mild 能量路线）；fix 运行中 | 09-15 2308Z | #— | — | A01 **A3-M2 全阶先验界族 hb**（最后一个 A3 分析单元）：基阶 6 的局部视界与半径先定 → 高阶解降阶+唯一性（186/188）等于基阶解 → H² 积分帽用基阶半径 → mild 级 Grönwall 给每个 R q；若 mild 级 Grönwall 缺则单独命名 |
| 194-A01-complement-path | worktree 安装中→自动启动（codex astra low） | 09-15 2303Z | — | — | A01 **P4a 余项路径**（189 重设计的一半）：残差 νΔu+f−(u·∇)u 的 Leray 余项在每个阶的 C^j_t 数据路径 + 一条物理路径 w + 逐时"w t 是残差 0 阶数据的余项"；经 190 得联合光滑场 G |
| 195-A01-interior-momentum | worktree 安装中→自动启动（codex astra low） | 09-15 2303Z | — | — | A01 **P4b 内点动量恒等式**（189 重设计的另一半）：190 联合代表元的 ∂ₜu = 导数数据的求值（有界求值与 fderiv 交换）+ 代表元线性 ⇒ 在 Ioo 0 S 上 G = residual − ∂ₜu；t=0 不作任何断言 |
| 196-A01-mild-gronwall | worktree 安装中→自动启动（codex astra low；基于 193 分支） | 09-15 2308Z | — | — | A01 **MildGronwall**（A3 最后一个分析输入）：vendor 正则化 mild 能量链 `mild_majorized_energy_subinterval` 特化到常欧氏度量与 q+1 阶有限 word 族，顶阶非线性项走 A03.outerProductTame → A04.outerSobolevNormAt_le → A04.inner_energy_Rhigh；显式 E q, C q |
