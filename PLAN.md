...
| 322-T11-U12-high-order | Opus 诚实部分交付：Grönwall 链/压力消去/Laplacian 配对/力项/Young 全证，higherOrderBound 由 hRhigh 推出；hRhigh（系数路径时间可导 + 配对层 tame 乘积）→ U12a/U12b；审计卫生修正中，之后审稿 | 09-18 0547Z | — | — | T11 U12：`higherOrderBound`（周期 eq:Rhigh 的 H^m 能量恒等式 + Grönwall；tame product 作为唯一具名输入）（L，astra） |
...
| 327-T11-U9d2b-momentum | Opus 完成（Duhamel 一阶时间微分、动量/projected 方程，74 声明；残余：全阶时间光滑）→ 并入含 326 终版的基座去重中，之后审稿 | 09-18 0542Z | — | — | T11 U9d2b：Duhamel 时间微分 → 系数路径时间可导、物理速度时间光滑、动量方程（对任意梯度 datum 为 (I−P)(F−Q) 的压力） |
