...
| 090-SIMP-B02 | PR 已开 | #92 | — | B02 七个已合模块（LowFrequency, Annular, LowHigh, Cutoff, LebesgueDatum, AnnularSchwartz, AnnularReal）的 simplifier+tester（陈述逐字不变；负向检查 autoImplicit off；conformance；MAINT 清单：a.e. 共轭模式去重、`angularFourier_conj`/`angular_plancherel`/`angularFrequencyDilation_coeFn` 上提） |
