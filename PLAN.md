...
| 161-A01-b1-time-ladder | 进行中（codex gpt-6-astra low） | 09-15 1834Z | — | — | A01 **B1 时间正则阶梯 rung 1**（HANDOFF P8）：datum 路径的连续 selection（R1）+ 阶梯表 B1_LADDER.md；任务书 collaboration/briefs/161-*.md |
...
| 163-C01-e5-enstrophy | 进行中（codex sol xhigh） | 09-15 1834Z | — | — | C01 **E5 enstrophy 导数**（HANDOFF P2 首环）：vendor wordEnergy_hasDerivWithinAt 在 s=1 减 s=0，分部积分得 −2⟪Δu,∂ₜu⟫，镜像 EnergyDerivative.lean |
| 164-D01-g1-homogeneous-norm | 进行中（codex sol high） | 09-15 1834Z | — | — | D01 **G1 dotHomogeneousENorm**（HANDOFF P4）：datum 形齐次 Ḣ^s 范数定义 + 基本引理 + Contracts/V2 重述与 rfl 桥 + 注册（第 27 条） |
