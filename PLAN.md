...
| 189-A01-pressure-regularity | fix3 运行中（sol high：import 180 的 PressureSupply、删拷贝、A01 构造器管线探针） | 09-16 0045Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
