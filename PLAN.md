...
| 205-A01-coordinate-tame | 进行中（codex astra low；用户确认 codex 恢复后直接启动） | 09-16 0214Z | #— | — | A01 **CylinderCoordinateTame**：逐坐标柱面 Kato–Ponce/Moser 估计——word Leibniz 展开 + 柱面/下降 tame 插值引理 + 光滑逼近到相容有限元；允许 re-cut 到角不变子空间；先证存在常数 |
