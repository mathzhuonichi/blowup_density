...
| 220-R44-s1c-trilinear | 续跑 #2（sol）完成：advection_pairing_le 无条件（trilinearConstJ = 3·C_emb³；物理重组识别用仓库的 Fourier 共轭/降阶对称与 vectorLpReassembly_ae 修好）；审稿中 | 09-16 1339Z | — | — | R44 **S1c**：J 权三线性估计 abs⟪(u·∇)u,Ju⟫ ≤ C₀·Y·(Y²+Z²)（三因子 L³ Hölder + 165/182 临界嵌入 + 164 齐次≤非齐次 + 218 的 Jmul_norm/weight_identity）；静态逐切片；新增 AdvectionJDatum 不改 JWeightDatum |
