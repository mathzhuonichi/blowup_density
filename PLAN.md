...
| 148-C01-e4b-pressure-jets | 进行中 | 09-14 0602Z | — | — | C01 **E4b**（M，≈120–180 行）：喷流 ⇒ order-m datum 路径连续（145 的 ‖A‖² ≤ 16^m·M 给范数控制，需喷流与坐标弱导数的桥）→ 动量残差 datum 路径连续 → lerayComplement（CLM）→ ∇p datum 路径连续 → jetOfDatum_continuous 得 ∇p 喷流连续 → ∂ₜu 的 hB → wordEnergy_hasDerivWithinAt 得 E4；146 的 residualPath 为输入 |
