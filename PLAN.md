...
| 375-T17-U4-force-profile | Opus 完成：五个力轮廓字段全证 + identity（chart 力形式；残余 force_eq_chart = 373 的 force_eq 限制到 chart，留 U12）；def 桥非 rfl（链式法则）→ 排队 codex 审稿（叠在 370 上，370 已合入） | 09-18 1241Z | — | — | T17 U4：力轮廓六字段（rescaledForceProfile = Paper1 forceProfile 桥；smooth/support/Const/nonneg/uniform/identity 含 ε⁻² 重标度） |
