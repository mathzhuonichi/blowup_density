...
| 255-I03-path-measurability | 审稿 ACCEPT-WITH-NOTES（sol；COMPARISON 两行标记已闭，lead 已改）；PR 已开，合并链运行中（wave 77） | 09-16 1714Z | #249 | — | I03 **路径可测性**：证 250 的唯一具名输入 CompactHomogeneousRealization（紧支撑光滑力的 compactHomogeneousPath 在 forceTimeMeasure 下 a.e. 强可测：切片连续 + 齐次实现连续，−3/2<s<0 的低频可积），从而 packet/correctionNegativeHomogeneous 无条件 |
| 256-R46-sobolev-density | 审稿 ACCEPT（sol，无修改）；PR 已开，链排在 #249 之后（wave 78） | 09-16 1714Z | #250 | — | R46 **完备 Sobolev 稠密**（Spec 第一字段）：B01 approxCompact 逼近完备目标 → 252 的 F_c 相对稠密 → 路径可加 + Bochner 数据范数三角不等式 ⇒ CompletedDense q s (breakdownSetIn F_c ν a T) |
