...
| 143-C01-e3-e4-momentum | 审阅中 | 09-14 0416Z | — | — | C01 行 **Ep/E3/E4**（S + S–M + M≈120 行）：载体 B 上为 `ClassicalSolutionR` 交付 `energyIdentity` 装配还欠的三件事——∇p 打包（P2）、动量分裂推到 `Lp`（由 `ClassicalSolutionR.momentum`）、`t ↦ ‖u(t)‖²₂` 的 `HasDerivAt`（`wordEnergy_hasDerivWithinAt` + `Evolution.velocityField_jetLp_continuous` + ∂ₜu 路径的 jetLp 连续性）；之后 Bindings 侧 ≈30 行进 C01 V2 |
