...
| 225-R43-universal | 审稿 ACCEPT（astra，无修改；半径加倍探针）；PR 已开，合并链运行中（wave 56）；Prop. 4.3 两个子句均无条件 | 09-16 1350Z | #228 | — | R43 **universal（一般初值）**：Spec 的 universal 字段逐字——一般 a 的 bootstrap（sqrt_energy_le_primitive′ + continuous_bootstrap，ρ = y(0)+∫b）+ C01 V4 带初值项的 H² 预算 + A04 延拓；若字段有误不 re-cut，写 COMPARISON_UNIVERSAL 给 owner |
