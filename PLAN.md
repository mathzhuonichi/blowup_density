...
| 279-SPEC-t16-spec | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-17 2056Z | — | — | SPEC **T16 定稿**：按 research/T16/RECONCILIATION.md（B 的数据记录 CutoffData + LocalPotentialAPI，A 的引用，T10 词汇）→ research/T16/Spec.lean + COMPARISON.md |
| 280-SPEC-t14-spec | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-17 2058Z | — | — | SPEC **T14 定稿**：按 research/T14/RECONCILIATION.md（B 的 PacketEnergyAPI/PacketImportAPI + A 的族选择器；继承 I01 PacketAPI）→ Spec.lean + COMPARISON.md |
| 281-SPEC-t22-spec | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-17 2058Z | — | — | SPEC **T22 定稿**：按 research/T22/RECONCILIATION.md（B 的 domainSobolevENorm 定义 + 三字段 API）→ Spec.lean + COMPARISON.md |
