...
| 358-T16-assembly | codex REJECT（仅简报符合性：缺 localPotentialData/localPotentialAPI 打包声明、REPORT 已由 lead 补、门禁重跑位置）→ Opus 补齐中；数学已闭合 | 09-18 0956Z | — | — | T16 装配：`localPotential : localPotentialStatement`（347 截断/阈值 + 351 球上势 + 352 格点提升；卸载 correction_fields_of_chart 的 chart 级假设） |
| 359-T13-localization-assembly | 待开：等 353 合入 + 354 落地 | 09-18 0934Z | — | — | T13 `localization` 装配：torus_identity + endpoint_zero + constant_pos_finite + 353 两估计 + 354 核比较 + 348 全空间恒等式 + Parseval-at-0 桥 → `localization` 字段 → LocalizationAPI 装配 + 合同（T02.localization） |
...
| 363-T15-UTB2-parseval-zero | codex sol 完成：periodicSobolevENorm_zero_eq(_of_memLp) / _ne_top（ParsevalZero.lean）→ 排队 codex 审稿 | 09-18 0956Z | — | — | T15 U-TB2：Parseval-at-0 桥 periodicSobolevENorm 0 z = eLpNorm (torusLift z) 2 Haar（parseval_forward + datum_unique）；359 也需要 |
