...
| 198-A01-energy-premises | 审稿 ACCEPT-WITH-NOTES（astra：前提与估计真实、27 公理、符号变异按预期失败；EnvelopeConversion 归为 (b)——需保留耗散的微分路线；三条措辞备注）；lead 应用备注后开 PR | 09-16 0041Z | #— | — | A01 **A3-M2 能量前提**（FiniteMildEnergy 上半）：为柱面 NS 解构造 vendor `mild_majorized_energy_subinterval` 的前提（无散路径、梯度压力、最大正则 Bochner 态 U∈TimeLp(H^{q+2})、源/压力代表元及限制恒等式），恒等度量、全 word 族 N=q−5；导出积分根能量估计，Z 的界与包络转换留给 199 |
