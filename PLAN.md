...
| 321-T11-U10-U11-restart-horizon | 完成（sol：restart + horizon/solution/regularity，唯一输入 PeriodicQuantitativeLocalInput\x27）→ codex 审稿中（rev-321） | 09-18 0503Z | — | — | T11 U10+U11：`restart`（对着具名输入 `PeriodicQuantitativeLocalInput'`，按阶力界 M m := ‖f‖_{L¹H^m}）与 `horizon`/`solution`/`regularity`（M，sol） |
...
| 323-T11-U15-maximal | 完成（sol：maximal_unique 无条件、exists_maximal 依赖 U11 输出的单一输入、lifespan 恒等）→ codex 审稿中（rev-323） | 09-18 0513Z | — | — | T11 U15：`exists_maximal`/`maximal_unique` + `maximalLifespanT = PeriodicLifespan.lifespan`（复用 Paper1 无条件粘合）（M，sol） |
...
| 326-T11-U9d2a-pressure | codex 审稿 REJECT（报告多说的 (I−P)(F−Q) 识别、少导出各阶系数隶属、两条 [propext] 审计行）→ 同一 Opus 子代理修正中，修后重审 | 09-18 0500Z | — | — | T11 U9d2a：mild 解的压力（Leray 补的系数构造、物理场、`pressure_gradient/smooth/gauge/periodic/poisson`）在 `PersistenceInput` 下 |
