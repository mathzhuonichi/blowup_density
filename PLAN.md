...
| 216-R43-critical-datum-path | worker 完成（sol）：六条半阶数据路径 + 四个空间相容字段无条件（Bessel→齐次乘子 abs(ξ)^s(1+abs(ξ)²)^(−s/2)）；装配 exists_criticalDatumPath 与无 hcrit 的 rcritical1_of_classical，条件于 CriticalDatumInputs（仅两项：velocityHalf 时间光滑 + 数据层动量恒等式）；审稿中 | 09-16 0925Z | — | — | R43 **半阶载体 G3/G4**：为经典解构造 CriticalDatumPath（六条 1/2 阶数据路径 + 符号恒等式 + 横向/纵向 + velocityHalf 光滑与动量恒等式，169 的齐次类比）；装配 exists_criticalDatumPath，使 214 的 eq:Rcritical1 去掉 hcrit 绑定；若动量恒等式不闭合则隔离为 CriticalDatumInputs |
...
| 218-R44-s1a-jweight | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 0916Z | — | — | R44 **S1a**：数据层 J=(I−Δ)^{1/2} 乘子（等距 H^s→H^{s−1}）、权恒等式 ‖u‖²_{H^{3/2}} = Y²+Z²（Y=‖u‖_{H^{1/2}}，Z=‖∇u‖_{H^{1/2}}，191 的平移数据）、力对偶 abs⟪f,Ju⟫ ≤ B·sqrt(Y²+Z²)（B=‖f‖_{H^{-1/2}}，需负阶载体或直接定义）；静态、逐切片，不含 PDE |
