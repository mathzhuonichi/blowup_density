...
| 373-T17-U2-transport | Opus 完成（429 中断后恢复）：correctionData（= T16 localPotentialData）、correctionData_correction（rfl）、force_eq（力算子经格点提升传输）、correctionForce_periodic；无缺口 → 排队 codex 审稿 | 09-18 1237Z | — | — | T17 U2：具体 correctionData（= T16 localPotentialData，correction ε = latticeLift(physicalCorrection) 由 rfl）+ 力算子传输 force_eq（局部算子与平移可交换 ⇒ correctionForce = latticeLift(Source.correctionForce)）；U5–U11 全部经此改写 |
...
| 378-SPEC-t19-spec | Opus prover 子代理运行中（statement-only；基座含 T19 RECONCILIATION.md） | 09-18 1237Z | — | — | SPEC **T19 定稿**：按 research/T19/RECONCILIATION.md（四 Prop 结构 3/3/4/3；T18 结论重述不作参数；RelativelyDenseT；注册 alpha；无守卫；引用行号更新） |
...
| 381-T20-canonical | worktree 安装中→自动启动（codex sol xhigh，astra 备用） | 09-18 1247Z | — | — | T20 canonical 模块 Section3/T20/CriticalRegularity.lean（CriticalRegularityTAPI 逐字重述于 T10/T11/T12 规范模块之上）+ probe + H¹ 球核查 research/T20/H1_CHECK.md（对照 H1_GAP.md） |
