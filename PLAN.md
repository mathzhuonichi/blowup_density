...
| 218-R44-s1a-jweight | worker 完成（sol）：Jmul（存储数据上的阶重标；物理乘子 (1+abs(ξ)²)^{1/2}）、weight_identity ‖u‖²_{H^{3/2}}=Y²+Z²、force_pairing_le（B=‖f‖_{H^{-1/2}}，负阶载体已有）、force_pairing_le'；条件于数据束 JWeightDatum；审稿中 | 09-16 0936Z | — | — | R44 **S1a**：数据层 J=(I−Δ)^{1/2} 乘子（等距 H^s→H^{s−1}）、权恒等式 ‖u‖²_{H^{3/2}} = Y²+Z²（Y=‖u‖_{H^{1/2}}，Z=‖∇u‖_{H^{1/2}}，191 的平移数据）、力对偶 abs⟪f,Ju⟫ ≤ B·sqrt(Y²+Z²)（B=‖f‖_{H^{-1/2}}，需负阶载体或直接定义）；静态、逐切片，不含 PDE |
