...
| 120-D01-p2-contract-v3 | 续改中 | — | — | **P2 合同**：`D01.datum_lemmas` V3（`Contracts/V3/DatumLemmas.lean` extends V2）三字段 `solution_slice_pressureGradient_smoothJets` / `…_temporalDerivative_smoothJets` / `…_pressureGradient_exists_datum`（A04 `hP` 形）+ `Bindings.DatumLemmasV3`（经 `uniqueness_toA02`）+ Tests + contracts.json（第 22 条）；照 `REVIEW_SL8_ASSEMBLY.md` §6/附录 C |
