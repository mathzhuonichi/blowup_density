...
| 208-A01-datum-horizon | worker 完成（astra：velocity(0,·)=a 逐点识别、initialClassR→光滑 L² 无散载体桥、总函数 localHorizon 与其正性、localSolution 数据（合同 solution 字段原形）；9 声明公理干净）；codex 审稿中 | 09-16 0506Z | #— | — | A01 **合同注册 1/3**：初值识别 velocity(0,·)=a（连续 + a.e.）、initialClassR 桥、总函数 localHorizon 与 LocalTheoryAPI 的 solution 字段 |
