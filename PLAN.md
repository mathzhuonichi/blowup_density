...
| 234-R41D-small-gaps | worker 完成（sol，2.5 小时）：G2 F_rd ⊆ F_R（实质性：快速衰减切片→Schwartz→全阶 L² 场，控制收敛得连续喷射路径，时间控制函数∈L¹∩L²）、G3 快速类在紧支撑校正下封闭、G4 S_σ ⊆ X_R、G5 ‖0‖=0（任意 q）；14 声明审计；分支已并入集成分支；审稿中 | 09-16 1655Z | — | — | R41D **G2–G5**：F_rd ⊆ F_R、快速类在紧支撑校正下封闭、S_σ ⊆ X_R、‖0‖_{L^qH^s}=0（先核对是否已有，再证；不成立则写 COMPARISON_SMALL_GAPS 给 owner） |
