...
| 366-T12-U1-haar-cube | codex（astra）基础版 REJECT → Opus 在 366 分支补全中（全 p 传输含 0/⊤、标量/梯度张量/tsupport、真实 probe、0 warning） | 09-18 1242Z | — | — | T12 U1：Haar ↔ 立方体 ↔ 全空间的 L^p 传输（周期场；支撑在 Q 内的场；标量/向量；periodicLpENorm 桥） |
...
| 379-T13-contract | codex（sol）完成：合同 T02.localization v1 注册（42 合同）→ 排队 codex 审稿；359 已合入 | 09-18 1254Z | — | — | T13 合同注册 `T02.localization` v1（Contracts/V1/Localization.lean 逐字重述 Spec；Bindings 由 T13.localizationAPI 传输；Tests；注册 → 42 合同；台账 claim T13） |
...
| 381-T20-canonical | codex sol 运行中（astra 备用） | 09-18 1251Z | — | — | T20 canonical 模块 Section3/T20/CriticalRegularity.lean（CriticalRegularityTAPI 逐字重述于 T10/T11/T12 规范模块之上）+ probe + H¹ 球核查 research/T20/H1_CHECK.md（对照 H1_GAP.md） |
| 382-SPEC-t23-spec | Opus prover 子代理运行中（statement-only；基座含 T23 RECONCILIATION.md） | 09-18 1255Z | — | — | SPEC **T23 定稿**：按 research/T23/RECONCILIATION.md（B 为基；去立方体 DomainPlacementData；未周期化 scaledPacket；ClassicalSolutionOmega；IsBoundedBoxOrSmoothDomain；线程 T22 范数层；消费整空间 Scaling/Correction） |
...
| 385-T17-U5U6-derivative-bounds | worktree 安装中→等 codex 空位启动（sol） | 09-18 1303Z | — | — | T17 U5+U6（纯传输）：correction_derivative_bound / force_derivative_bound（correctionData_correction + latticeLift_iteratedFDeriv_eq + Paper1 混合导数界） |
...
| 387-T22-UA4-orderzero-isometry | worktree 安装中→Opus prover 子代理 | 09-18 1304Z | — | — | T22 U-A4：0 阶向量 Plancherel 等距 norm_orderZeroDatum_eq（D01 明示未证的恒等式）供 orderZero |
