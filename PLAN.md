...
| 127-MAINT-hotfix-pressuredrop-import | 进行中 | — | — | 热修：#124（121）在 #123（搬 RealPairing 到 D01）之后合入，`A04/PressureDrop.lean` 仍 `import …A04.RealPairing`，integration 门禁红（wave80 exit=2）；改一行 import 为 `D01.RealPairing`（lead 手改，一行） |
| 其余节点 | 未开始 | — | — | |
