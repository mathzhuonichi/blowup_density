...
| 216-R43-critical-datum-path | 审稿 ACCEPT（sol，无修改；符号变异探针）；PR 已开，合并链运行中（wave 49）；六条半阶路径 + 四个空间字段无条件；装配条件于 CriticalDatumInputs（219 在闭合） | 09-16 0934Z | #221 | — | R43 **半阶载体 G3/G4**：为经典解构造 CriticalDatumPath（六条 1/2 阶数据路径 + 符号恒等式 + 横向/纵向 + velocityHalf 光滑与动量恒等式，169 的齐次类比）；装配 exists_criticalDatumPath，使 214 的 eq:Rcritical1 去掉 hcrit 绑定；若动量恒等式不闭合则隔离为 CriticalDatumInputs |
...
| 219-R43-critical-momentum | worktree 安装中→自动启动（codex astra low，带重试；基于 216 分支） | 09-16 0927Z | — | — | R43 **动量传输**：闭合 216 的 CriticalDatumInputs——半阶速度路径的时间光滑性（215 的整数阶光滑路径 + lowerVectorL + 216 乘子为 CLM）与数据层动量恒等式（局部载体窗口覆盖 + 169 导数=投影残差 + 189/195/197 Leray 识别 + 齐次数据唯一性）；给出无条件 exists_criticalDatumPath 与 rcritical1_of_classical |
