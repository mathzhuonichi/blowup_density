...
| 233-R42-family-from-data | 审稿 ACCEPT（astra，无修改）；分支已并入集成分支；PR 已开，合并链运行中（wave 61） | 09-16 1433Z | #233 | — | R41D **G1**：从原始数据 (ν,T,a∈X_R,g∈F_R, T < T_max) 构造已注册的 R42 插入记录 InsertionLifespanV2API（严格寿命 ⇒ RegularThrough ⇒ 参考解 ⇒ 校正/缩放/族/寿命记录），family.a/g/T 相符；缺的构造器隔离为具名输入并注明归属 lane |
...
| 235-R41D-density | worker 完成（astra，4 分钟）：breakdownDenseR_of_subcritical——**thm:Rmain (i)**：ν,T>0、q∈{1,2}、s<2/q−3/2、任意 a∈X_R ⇒ BreakdownDenseR ν a T q s（两情形 + 233 的记录 + forceConvergence + lifespan=T）；零初值实例 = (ii) 的 if 半；Bindings/DensityFromInsertion.lean 三声明；审稿中 | 09-16 1433Z | — | — | R41D **稠密分支（Y = F_R）**：thm:Rmain (i) 与 (ii) 的"if"半：q∈{1,2}、s < 2/q−3/2、任意 a∈X_R ⇒ BreakdownDenseR ν a T q s（两情形：T_max ≤ T 取 f=g；否则 233 的 R42 记录 + forceConvergence + lifespan = T）；a=0 实例；若 232 可读则给 iff |
