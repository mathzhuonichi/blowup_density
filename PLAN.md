...
| 189-A01-pressure-regularity | 审稿 ACCEPT-WITH-NOTES（astra：装配、端点、管线、公理、非空都过；备注：一处 maxHeartbeats 800000→400000（已改，模块重过）；180 落地后改为 import 其定义）；等 180（PR #190）合入后 rebase 开 PR | 09-16 0042Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
...
| 198-A01-energy-premises | 审稿 ACCEPT-WITH-NOTES；三条措辞备注 lead 已改；PR 已开，合并链运行中（wave 29）→ 落地后 199 自动启动 | 09-16 0042Z | #202 | — | A01 **A3-M2 能量前提**（FiniteMildEnergy 上半）：为柱面 NS 解构造 vendor `mild_majorized_energy_subinterval` 的前提（无散路径、梯度压力、最大正则 Bochner 态 U∈TimeLp(H^{q+2})、源/压力代表元及限制恒等式），恒等度量、全 word 族 N=q−5；导出积分根能量估计，Z 的界与包络转换留给 199 |
| 199-A01-envelope | 等 198 合入后自动建树启动（codex astra low） | 09-16 0042Z | — | — | A01 **A3-M2 包络**（FiniteMildEnergy 下半）：带号正则化能量恒等式保留 −ν 梯度平方 → 张量配对桥 + A03/A04 tame 界 → 极限后 Young 吸收 → 标量比较 ODE y′=αy+b 给处处可微包络 x=y²；同时证 ForcingFamilyBound（换位子族范数界） |
