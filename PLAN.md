...
| 192-A01-wiring-hsob | 审稿 REJECT（分开命名的导出各开各的存在量词，载体不同一；非空例假设了 hb）；fix 运行中（合并导出、真零例、Grönwall 备注改为基阶半径路线） | 09-15 2232Z | #— | — | A01 **接线**：186′+188（公共视界无条件）+187（hfs）+178（全阶 C^j 数据路径）→ 仅由全阶先验界 hb 得到构造器的 hsob/hU/hdiv；记录 A01 管线剩下的唯一分析输入（A3-M2 的 hb，加 190 的 hc3、189 的 hpg） |
| 193-A01-a3-m2-bounds | worktree 安装中→自动启动（codex astra low） | 09-15 2232Z | — | — | A01 **A3-M2 全阶先验界族 hb**（最后一个 A3 分析单元）：基阶 6 的局部视界与半径先定 → 高阶解降阶+唯一性（186/188）等于基阶解 → H² 积分帽用基阶半径 → mild 级 Grönwall 给每个 R q；若 mild 级 Grönwall 缺则单独命名 |
