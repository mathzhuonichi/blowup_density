...
| 137-A01-a3-force-cap | 审阅中 | 09-14 0242Z | — | — | A01 A3 行 **A3-L1·f**（S）：力的一致积分帽 `∫₀ᵗ ‖f(s)‖_{H^m} ds ≤ Bbnd` 对 `t ∈ [0,T₀)`（由 `MemForceR` 经 `Continuity.lean` 的 `continuousOn_sobolevNormAt_force` + 区间可积得到），喂 122 的 `gronwall_bddAbove_Ico` 的 `Bbnd` 槽；顺带 `0 ≤ y_m 0` 无需桥的说明 |
