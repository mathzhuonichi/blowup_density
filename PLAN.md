...
| 123-MAINT-pairing-home | 进行中 | — | — | MAINT：把 `A04/LaplacianPairing.lean`、`A04/RealPairing.lean`（内容全在 `namespace Paper3`，不用任何 A04 声明）搬到与其 import 相符的最低层（Paper3/ 或 D01/），消除 `D01/LerayLowering` → A04 的反向边（115 审稿）；更新所有 importer 与 conformance 文件；陈述逐字不变 |
