...
| 253-R47-flux-cancellation | 审稿 ACCEPT（astra，无修改）；PR 已开，合并链运行中（wave 74）；eq:gridforce 闭合 | 09-16 1654Z | #246 | — | R47 **通量抵消**：证 251 的唯一具名输入 hcompactMomentumIntegral——动量残差之差的空间项（div(u_ε⊗u_ε − v⊗v)、∇δp、−νΔw）都是紧支撑于 cell 内的光滑场的空间导数，在 cell 上积分为零，只剩 ∫∂_t w；力观测相等无条件 |
