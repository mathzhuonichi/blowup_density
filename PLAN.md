...
| 351-T16-ball-potential | Opus 完成：timePotential_contDiffOn_ball / spatialCurl_timePotential_on_ball / exists_potential_on_ball（三个 potential_* 字段，无残余）→ 排队 codex 审稿（叠在 347 上，先合 347） | 09-18 0855Z | — | — | T16 缺口 1：球上径向势 timePotential_contDiffOn_ball / spatialCurl_timePotential_on_ball（空间 bump 截断 + I02 全局引理 + congr_slice）→ exists_potential_on_ball（potential_smooth/formula/curl 三字段） |
| 352-T16-lattice-lift | Opus prover 子代理运行中（基座 = 347 分支） | 09-18 0831Z | — | — | T16 缺口 2：紧支修正的单位周期格点提升 latticeLift（局部有限、光滑、周期、支撑、无散、k=0 公式、eq:bgzero）→ 八个 correction_* 字段 |
...
| 355-T14-packet-contract | codex sol 完成：合同 T01.packet_import v1 注册（40 合同；Contracts/Bindings/Tests + 台账 claim T14；门禁全绿）→ 排队 codex 审稿 | 09-18 0854Z | — | — | T14 合同注册 `T01.packet_import` v1（Contracts/V1/PacketImport.lean 逐字重述 Spec；Bindings 由 Bindings.packet + 346 定理构造 PacketImportFamily；Tests；注册 → 40 合同；台账 claim T14） |
