...
| 189-A01-pressure-regularity | worker 完成（hpg_of_slab：板上 C∞ + 逐时 L² + 对称 Jacobian（非循环 Helmholtz 逆向：Leray 余项 0 阶数据 ⇒ 对称）；输入 hc3/hsob/hdiv/htime/hmom；12 声明公理干净）；codex 审稿中（重点 htime 可满足性） | 09-15 2207Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
