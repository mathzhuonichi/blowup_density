...
| 230-A04-v2-restart-fixed-force | worktree 安装中→自动启动（codex sol xhigh，带重试） | 09-16 1713Z | — | — | A04 **V2 合同**（选项 1）：Restart = RestartFixedForce（固定外力、对 t₀∈[0,S] 与 H⁷ 数据一致）+ HigherOrderBound + 217 形状的三条延拓定理；V1 的 H¹ Restart 留作明确未证；同样的防 reward-hacking 要求 |
