...
| 251-R47-force-cell-integral | worktree 安装中→自动启动（codex astra low，带重试；基于 247 分支） | 09-16 1624Z | — | — | R47 **eq:gridforce**：插入族速度差/力差在含球格子上积分为零（速度：div-free + 支撑；力：两条动量方程之差在格子上积分，时间导数项由速度零均值消去、散度/梯度/Laplace 项由紧支撑消去），并与 247 的局部性引理合出 Spec 的 velocity_/force_observations 字段 |
