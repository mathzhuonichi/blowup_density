...
| 310-T11-U3-galilean-classes | 完成（三字段，无命名输入，624 行）→ codex 审稿中（rev-310） | 09-18 0233Z | — | — | T11 U3：`translation_preserves_sobolev`/`transformed_classes`/`transformed_mean_zero`（M，sol） |
...
| 317-T11-U9c-convolution-bound | worktree 安装中→自动启动（codex **astra** low，sol 备用） | 09-18 0214Z | — | — | T11 U9c：偿还 313 的唯一具名输入 `TorusConvolutionInput`（投影对流卷积的有界双线性 H³×H³→H² 实现：离散 Sobolev 乘积估计，Peetre 不等式 + Cauchy–Schwarz + 格点可和）→ 两空间契约有居民 |
