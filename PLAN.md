...
| 218-R44-s1a-jweight | 审稿 ACCEPT-WITH-NOTES（sol；唯一修改为 G3 措辞，lead 已改）；PR 已开，合并链运行中（wave 50）；S1a 数据层闭合 | 09-16 0947Z | #222 | — | R44 **S1a**：数据层 J=(I−Δ)^{1/2} 乘子（等距 H^s→H^{s−1}）、权恒等式 ‖u‖²_{H^{3/2}} = Y²+Z²（Y=‖u‖_{H^{1/2}}，Z=‖∇u‖_{H^{1/2}}，191 的平移数据）、力对偶 abs⟪f,Ju⟫ ≤ B·sqrt(Y²+Z²)（B=‖f‖_{H^{-1/2}}，需负阶载体或直接定义）；静态、逐切片，不含 PDE |
