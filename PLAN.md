...
| 158-A01-b1-constructor-split | 进行中 | 09-14 1214Z | — | — | A01 **单元 B1/B2（mild ⇒ classical 构造子）拆分 + 首个 S 片**（L 多 lane 的第一条）：目标 CarrierConstructor q ν S（research/A01/REVIEW_SLICE_WIRING.md §3(b) 已 typecheck）；写 research/A01/CONSTRUCTOR_SPLIT.md（c1–c9 字段逐个：来源引理/缺口/大小），证首个 S 片（B2 的字段装配骨架或 c3 velocity_smooth 的 H^m-in-time ⇒ 联合 C^∞ 的最低阶情形）；不碰 A3_SPLIT 之外的既有模块 |
| 159-R43-split | 进行中 | 09-14 1214Z | — | — | R43 **命题 4.3 拆分**（research + 首个 S 行）：照 research/R43/COMPARISON.md §4 的 G1–G8 与 Spec.lean RCritical1API 四字段，写 research/R43/R43_SPLIT.md（每个上游缺口现状：A05.gradient_l6、A04 energy_high V2、C01 V3 已注册；G7 eq:Rcritical1 自有 L 步）；证 S 行（G6 ℕ-pow/rpow 拼写钉、G3 非空洞、G8 常数算术）于 Section4/R43/*.lean |
| 其余节点 | 未开始 | — | — | — | |
