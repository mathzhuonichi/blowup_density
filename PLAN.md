...
| 197-A01-leray-bridge | fix2 完成（rebase；全部证明 ≤400000 heartbeats；fix6 binder 导出；回放落地的 195 消费者）；lead 复核；PR 已开，合并链运行中（wave 28）→ 落地后 189 装配自动启动 | 09-16 0027Z | #201 | — | A01 **P4c 投影桥 hprojected**（195 的唯一命名输入）：柱面 Leray 投影 `leray 1 q` 与数据级 Fourier `lerayComplement 0` 在下降数据上一致——梯度部分纵向、投影部分无散、物理残差数据与下降柱面残差一致（`physicalResidual_datum_eq`，194 也用） |
