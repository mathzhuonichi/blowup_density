...
| 369-T17-U1-lattice-deriv | Opus 补全（6986cd73）：latticeLift_iteratedFDeriv_eq_shift（任意 z、∃k、零情形）+ 全局 ⨆ 推论；k=0 形式保留 → 排队复审 | 09-18 1248Z | — | — | T17 U1：格点提升的迭代导数桥（局部单拷贝 ⇒ iteratedFDeriv 相等；供 U5/U6 传输 eq:derivativebounds） |
...
| 382-SPEC-t23-spec | worktree 安装中→Opus prover 子代理（基座含 research/T23/RECONCILIATION.md） | 09-18 1252Z | — | — | SPEC **T23 定稿**：按 research/T23/RECONCILIATION.md（B 为基；去立方体 DomainPlacementData；未周期化 scaledPacket；ClassicalSolutionOmega；IsBoundedBoxOrSmoothDomain；线程 T22 范数层；消费整空间 Scaling/Correction） |
