...
| 371-T16-contract | codex（astra）注册完成；审稿 REJECT 仅测试交付缺项（Spec 一致性 example、非零常值场测试、门禁输出、措辞）→ 续做简报 fix_371 上 codex | 09-18 1221Z | — | — | T16 合同注册 `T02.local_potential` v1（Contracts/V1/LocalPotential.lean 逐字重述 Spec；CutoffData/LocalPotentialAPI 结构体例外逐字段转换；Bindings 由 T16.localPotential 传输；Tests；注册 → 41 合同；台账 claim T16） |
...
| 378-SPEC-t19-spec | worktree 安装中→Opus prover 子代理（基座含 research/T19/RECONCILIATION.md） | 09-18 1224Z | — | — | SPEC **T19 定稿**：按 research/T19/RECONCILIATION.md（四 Prop 结构 3/3/4/3；T18 结论重述不作参数；RelativelyDenseT；注册 alpha；无守卫；引用行号更新） |
