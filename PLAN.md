...
| 007-I01-contract | review 中 | `I01.packet` V1 已注册（本地全绿） | — | 27 义务全证；合同只能 import Mathlib → 20 个定义内联 + 14 个 rfl 桥 |
| 008-I02-correction-spec | 已合入（ACCEPT-WITH-NOTES） | — | #10 | 76 字段 CorrectionAPI；3.4/3.5 的 R³ 内容本地已基本证完，缺 4 小项 |
| 011-I02-contract | 待 007 review | 目标 `I02.correction` V1 | — | 合同 + 绑定 + 4 个缺口证明；先应用 reviewer 的 Ioo→Ico |
| 009-D01-reconcile | 进行中 | 目标 `Contracts/V1/Data.lean` | — | 归并 A/B + 两份审稿 + 台账需求；产出合同层定义 |
| 010-U05-port | review 中 | — | — | 88 模块全部编过；补丁只在 4 个副本、1 处 have；vendor 零改动 |
| 其余节点 | 未开始 | — | — | |
