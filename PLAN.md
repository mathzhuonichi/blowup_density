...
| 221-R43-force-path | 续跑（astra，4 分钟）完成：G2 精确比较 forceHomogeneousENorm ≤ forceSobolevENormL1、G3 有限性、criticalForceAt 连续/可积/原函数 FTC 单调；critical_bootstrap_zero_datum：a=0 时 y(t) ≤ cν 于 [0,S]（S<T）无具名输入（25 个声明）；审稿中 | 09-16 1319Z | — | — | R43 **G2/G3/G4 时间路径**：b(t)=‖f(t)‖_{Ḣ^{1/2}}（214 的 criticalForceAt）连续、可积、原函数 N 的 FTC 与单调性（匹配 Paper1.critical_norm_bound / criticalNormBound_radius 的假设形状）、G2 时间积分单调性；探针：a=0 的 bootstrap y(t) ≤ cν 仅条件于 ν>0、MemForceR、小性 |
...
| 223-R43-endpoint | worktree 安装中→自动启动（codex astra low，带重试；基于 221 分支） | 09-16 1319Z | — | — | R43 **端点定理 S4/S5/S6**：inhomogeneousAtZero——a=0、‖f‖ < c₀ν ⇒ T_max = ∞：221 的 bootstrap → S3 门 → C01 V4 吸收 eq:RH1 → ∫‖u‖²_{H²} 有限（G5 端点粘合）→ 217 的 lifespanInfiniteOfLocallyFinite_of_memForceR′；陈述逐字取 Spec.lean |
