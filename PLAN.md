...
| 194-A01-complement-path | 审稿 ACCEPT（192/195 探针通过；区间变异按预期失败）；PR 已开，合并链运行中（wave 26） | 09-15 2346Z | #199 | — | A01 **P4a 余项路径**（189 重设计的一半）：残差 νΔu+f−(u·∇)u 的 Leray 余项在每个阶的 C^j_t 数据路径 + 一条物理路径 w + 逐时"w t 是残差 0 阶数据的余项"；经 190 得联合光滑场 G |
