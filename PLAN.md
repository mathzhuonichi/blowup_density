...
| 201-A01-root-comparison | fix 完成（rebase、改名、ha 贯穿、说明）；lead 复核；PR 已开，合并链运行中（wave 33） | 09-16 0131Z | #205 | — | A01 **CylinderRootComparison**（A3-M2 envelope 子行）：带号恒等式 + ForcingFamilyBound（作为假设）⇒ 正则化根的微分不等式 → 沿 198 的最大逼近族取带号积分极限 → 标量 Grönwall 比较（含零根极限与端点）⇒ FiniteMildEnergy 条件于 ForcingFamilyBound |
| 203-A01-signed-limit | 进行中（codex astra low；基于 201 分支） | 09-16 0131Z | #— | — | A01 **CylinderSignedRootLimit**（A3-M2 最后两块之一）：把 199 的带号（保留 −ν 梯度平方）正则化能量不等式沿 198 的最大逼近族取积分极限——vendor 极限引理无耗散槽，需新增带号通道（强 TimeLp 极限经导数 word 映射或弱收敛下半连续）、ε 正则化根分母、先取极限再吸收；与 200 协调正则化族一致界 |
