...
| 209-A01-manuscript-regularity | 审稿 ACCEPT-WITH-NOTES（端点证明、11 公理、符号变异按预期失败；备注：208 的 localSolution 需保留载体见证→211）；PR 已开，合并链运行中（wave 42） | 09-16 0516Z | #214 | — | A01 **合同注册 2/3**：ManuscriptLocalRegularity 四字段（全阶 C∞ 数据路径经 hslice；eq:Rpressure 含 t=0 由 189/194 的 Leray 余项恒等式；projected 由 momentum；径向势规范） |
