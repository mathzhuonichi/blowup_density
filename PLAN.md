...
| 189-A01-pressure-regularity | fix 运行中（astra low）：压力梯度改为残差 Leray 余项路径的联合光滑代表元（190），内点恒等式由 169 得出，无 htime | 09-15 2253Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
