...
| 232-R41-nondensity-both | 审稿 ACCEPT（sol，无修改；阈值放宽探针）；PR 已开，合并链排在 #234 之后（wave 63） | 09-16 1439Z | #235 | — | R41 **nonDensityZero 两个 q 的装配**：按 RMainAPI 骨架（q=1∨q=2，ThresholdAPI 的 exponent q 0 ≤ s）由 224/229 的两条非稠密定理拼出 nonDensityZero_of_q 与 not_breakdownDenseR_zero_of_q，并打包成可 ⟨…⟩ 实例化的结构 |
...
| 235-R41D-density | 审稿 ACCEPT（astra，无修改；阈值探针）；分支已并入集成分支；PR 已开，合并链运行中（wave 62） | 09-16 1438Z | #234 | — | R41D **稠密分支（Y = F_R）**：thm:Rmain (i) 与 (ii) 的"if"半：q∈{1,2}、s < 2/q−3/2、任意 a∈X_R ⇒ BreakdownDenseR ν a T q s（两情形：T_max ≤ T 取 f=g；否则 233 的 R42 记录 + forceConvergence + lifespan = T）；a=0 实例；若 232 可读则给 iff |
...
| 237-SPEC-r41-draft-b | worktree 安装中→自动启动（codex astra low，带重试） | 09-16 1440Z | — | — | SPEC **R41 双盲草案 B**：同上，独立模型；lead 随后比对 A/B 与 STATEMENTS 骨架，定稿后开注册 lane |
