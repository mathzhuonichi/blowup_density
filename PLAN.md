...
| 254-R45-compact-rider | 审稿 ACCEPT（sol，无修改）；PR 已开，合并链运行中（wave 75） | 09-16 1658Z | #247 | — | R45 **F_c 的正则参考 rider**：Spec 的 regularReference 在 Y = forceClassCompact 下（249 的 R41 rider 绑定为模板：233 记录一次调用 + 唯一性对齐 + R42 history/forceConvergence/速率夹逼；ε 形式与任意历史截断 τ 由极限导出；f ∈ F_c 由紧支撑闭包） |
