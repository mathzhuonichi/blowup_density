...
| 249-R41-contract-v1 | 审稿 ACCEPT（astra，无修改；端点与非空洞探针）；分支已并入集成分支；PR 已开，合并链运行中（wave 70）；第 32 个合同 = Theorem 4.1 | 09-16 1625Z | #242 | — | R41 **V1 合同注册**（第 32 个合同 R41.main_thresholds）：MainThresholdsAPI = research/R41/Spec.lean 的 RMainAPI 逐字；Bindings 按绑定计划接 235/232/233 + R42 V2 字段（rider 用同一记录）；若某字段不可绑则注册 partial 变体并报告缺口 |
