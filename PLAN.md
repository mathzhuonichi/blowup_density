...
| 334-T11-U9d2c-classical-assembly | codex ACCEPT-WITH-NOTES（报告行数/措辞）；与去重后 327 的语义冲突（hFI 参数）由 Opus 按 338 的修复移植后合入 | 09-18 0711Z | — | — | T11 U9d2c：全阶时间光滑（mild 方程各阶 + 归纳）+ 装配 `ClassicalSolutionT`/`PeriodicLocalRegularity` → 关闭 U9d 目标（`mild_to_classical`）与 `exists_classical_of_picard` |
