...
| 347-T16-local-potential | Opus 部分交付：T16 规范模块（6 定义 + 2 结构 + 陈述）、Urysohn 截断 θ/η、阈值 ε₀、v=0 的完整 26 字段 API；缺口 = 球上径向势（I02 引理仅对 univ）+ 周期修正格点提升 → 排队 codex 审稿；后续 351/352 | 09-18 0829Z | — | — | T16 证明：`lem:potential` localPotentialStatement（Urysohn 截断 θ/η、ε₀、径向向量势 ∇×A=v、修正 w_ε=−∇×(η_εθ_εA) 光滑无散周期、eq:bgzero）；复用 Section4/I02 Reference/Prescribed |
...
| 351-T16-ball-potential | codex ACCEPT-WITH-NOTES（仅需合并基座）；等 347 合入后合入 | 09-18 0912Z | — | — | T16 缺口 1：球上径向势 timePotential_contDiffOn_ball / spatialCurl_timePotential_on_ball（空间 bump 截断 + I02 全局引理 + congr_slice）→ exists_potential_on_ball（potential_smooth/formula/curl 三字段） |
| 352-T16-lattice-lift | Opus 已修 REJECT（抵消接口改为局部 plateau O ⊆ ball、周期包支撑 ⊆ periodicSet O；去 hθR、0 warning；a6f0e836）→ 复审中；358 已获接口 | 09-18 0926Z | — | — | T16 缺口 2：紧支修正的单位周期格点提升 latticeLift（局部有限、光滑、周期、支撑、无散、k=0 公式、eq:bgzero）→ 八个 correction_* 字段 |
| 353-T13-localization-kernel | Opus 部分交付：tailConst 一致尾和界 + two_r_lt_one + periodicSobolevENorm ≤ Sobolev0 + 齐次 已证；核比较 iTorus_periodize_le 未交付——简报常数 tailConst(2r) 不够（需与 ∂Q 的距离 d 的几何常数）→ 排队 codex 审稿；354 改为几何常数版核比较 | 09-18 0919Z | — | — | T13 `localization` 第 1 部分（核估计）：格点尾和一致界 tailConst、`ITorus s (periodize f) ≤ IReal s f + 4·tailConst·‖f‖₂²`、T³ 上 periodicSobolevENorm ≤ L² + 齐次；装配（需 348）留给 354 |
...
| 360-SPEC-t18-spec | worktree 安装中→Opus prover 子代理（基座含 research/T18/RECONCILIATION.md） | 09-18 0934Z | — | — | SPEC **T18 定稿**：按 research/T18/RECONCILIATION.md（B 为基；参数 PacketImportAPI/PlacementData/ScalingAPI/reference/CorrectionAPI；MemMixedLebesgueT 与 s<0 守卫；periodicSet 支撑；blowup_limsup）；反 stub 条款 |
