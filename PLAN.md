...
| 331-T11-U6-transport | Opus 完成（通用转运构造 14 字段 + 三实例，72 声明；残余：任意 ClassicalSolutionT 的 PeriodicLocalRegularity → 339）→ 排队 codex 审稿 | 09-18 0638Z | — | — | T11 U6：解在光滑变量替换下的转运构造 → `transformed_solution`/`to_unit`/`from_unit`（L，sol） |
...
| 335-T11-U12a-energy-identity | codex 审稿 REJECT（保真：需 Leray 投影形式的系数 ODE；一步桥）→ Opus 补证中，修后合入 | 09-18 0646Z | — | — | T11 U12a：经典解系数路径的时间可导 + datum 形式动量方程 → H^m 能量恒等式（322 的 hRhigh 的 HasDerivAt 部分） |
...
| 337-T11-U14-U16-extends-lifespan | Opus 完成（extendsBeyond/lifespanInfiniteOfLocallyFinite 逐字；323 的输入由 H 推出；整条延拓链只依赖 PeriodicQuantitativeLocalInput\x27 + higherOrderBound）→ 排队 codex 审稿 | 09-18 0619Z | — | — | T11 U14+U16：`extendsBeyond`（H² 判据 → higherOrderBound(m=1) 的 H¹ 轨迹界 → 332 的 restartBeyond）与 `lifespanInfiniteOfLocallyFinite`（反证）；higherOrderBound 陈述作显式假设，唯一具名输入为存在性输入 |
