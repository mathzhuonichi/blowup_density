...
| 192-A01-wiring-hsob | 第三轮审稿 ACCEPT-WITH-NOTES（备注仅探针直接 import 190）；PR 已开，合并链运行中（wave 23） | 09-15 2309Z | #196 | — | A01 **接线**：186′+188（公共视界无条件）+187（hfs）+178（全阶 C^j 数据路径）→ 仅由全阶先验界 hb 得到构造器的 hsob/hU/hdiv；记录 A01 管线剩下的唯一分析输入（A3-M2 的 hb，加 190 的 hc3、189 的 hpg） |
