...
| 222-R44-s1b-energy-identity | 续跑（astra，12 分钟）完成：energy_identity 无条件——(Y²)′ = −2νZ² − 2⟪(u·∇)u,Ju⟫ + 2⟪f,Ju⟫（耗散/压力项为等式；20 个声明）；与 220 的 advectionJPairing 需一个适配引理；审稿中 | 09-16 1144Z | — | — | R44 **S1b**：沿经典解微分 Y²=‖u‖²_{H^{1/2}}（215 光滑整数阶路径经 lowerVectorL 降到 1/2 + DerivNorm），(Y²)′=2⟪∂ₜu,Ju⟫，A04.momentum_datum 二阶动量恒等式降阶，压力项由 Leray 正交消去，耗散项 ⟪νΔu,Ju⟫=−νZ²；得 energy_identity |
| 223-R43-endpoint | worker 完成（astra，8 分钟）：inhomogeneousAtZero_of_memForceR 无条件、陈述逐字同 Spec.lean:243-247（criticalConst 显式）；G5 已在 MaximalEndpoint.lean；含 215/217 两文件的字节复制提交（基线缺）；审稿中 | 09-16 1329Z | — | — | R43 **端点定理 S4/S5/S6**：inhomogeneousAtZero——a=0、‖f‖ < c₀ν ⇒ T_max = ∞：221 的 bootstrap → S3 门 → C01 V4 吸收 eq:RH1 → ∫‖u‖²_{H²} 有限（G5 端点粘合）→ 217 的 lifespanInfiniteOfLocallyFinite_of_memForceR′；陈述逐字取 Spec.lean |
| 224-R41-nondensity-q1 | worktree 安装中→自动启动（codex astra low，带重试；基于 223 分支） | 09-16 1331Z | — | — | R41 **q=1 非稠密子句**：forceSobolevENorm 按阶单调（lowerVectorL 降阶路径）+ 223 端点定理的逆否 ⇒ s ≥ 1/2 时 ¬BreakdownDenseR ν 0 T 1 s（Data.lean 逐字词汇，ρ = criticalConst·ν） |
