...
| 213-A02-A04-wiring | 已合入 #218（wave 46 门禁 OK）；A02 exists_maximal 无条件；A04 重启前提；Restart 定义未居留（H¹/跨力形状不匹配，同 A01 V2 决定）；事后审稿休息后重启 | 09-16 0851Z | #218 | — | A02/A04 **接线**：exists_maximal 用 211 的 localHorizon′/localCarrier 无条件实例化；A04 的 Restart 由捆绑 + 179 的 Grönwall 一致 H^m 界 + horizon_lower_bound_H7_fixedForce 构造 ⇒ restartBeyond/extendsBeyond/lifespanInfiniteOfLocallyFinite 无条件 |
