...
| 250-I03-homogeneous-scaling | worker 完成（astra，25 分钟）：齐次 Fourier 缩放、角频率转换、齐次向量数据的空间/时间缩放、correction force 的齐次分量界；36 声明（Section4/I03/HomogeneousScaling.lean + Bindings/ScalingHomogeneous.lean）；两个 I03 字段条件于唯一具名输入 CompactHomogeneousRealization（D01 的 compactHomogeneousPath 在 forceTimeMeasure 下 a.e. 强可测）；审稿中 | 09-16 1642Z | — | — | I03 **齐次负阶缩放**：Spec 里缺失的 packetNegativeHomogeneous / correctionNegativeHomogeneous（‖F_ε‖_{L^q_tḢ^s} ≤ Cε^β，抛物缩放的 Fourier 齐次权计算，以 IsHomogeneousPath 路径证 forceHomogeneousENorm 上界）——R46/R47 强闭包的 L²_tḢ⁻¹ 项所需 |
| 251-R47-force-cell-integral | worker 完成（astra，12 分钟）：速度差在含球 cell 上积分为零（无条件）、动量方程之差、内点时间导数全空间积分为零、速度/力观测相等（力的 t∈(0,T) 条件于唯一具名输入 hcompactMomentumIntegral = 空间通量抵消：∫_C 残差差 = ∫ ∂_t(u_ε−v)）；t=0 力观测无条件；14 定理；审稿中 | 09-16 1637Z | — | — | R47 **eq:gridforce**：插入族速度差/力差在含球格子上积分为零（速度：div-free + 支撑；力：两条动量方程之差在格子上积分，时间导数项由速度零均值消去、散度/梯度/Laplace 项由紧支撑消去），并与 247 的局部性引理合出 Spec 的 velocity_/force_observations 字段 |
| 252-R45-compact-class | worker 完成（sol，10 分钟）：density_compact、zeroIff_compact（Y = F_c 实例，量词照抄 Spec）+ 紧支撑闭包 + F_c ⊆ F_R；Bindings/CompactClassDensity.lean 四声明；审稿中 | 09-16 1639Z | — | — | R45 **F_c 实例**：Spec 的 density/zeroIff 在 Y = forceClassCompact 下（235 的两情形模板 + forceDifference_compact 保持紧支撑；F_c ⊆ F_R 由 memForceR_insertedForce 取 g=0；非稠密由 breakdownSetIn F_c ⊆ breakdownSetRZero + 232/249 反证） |
| 253-R47-flux-cancellation | worktree 安装中→自动启动（codex astra low，带重试；基于 251 分支） | 09-16 1637Z | — | — | R47 **通量抵消**：证 251 的唯一具名输入 hcompactMomentumIntegral——动量残差之差的空间项（div(u_ε⊗u_ε − v⊗v)、∇δp、−νΔw）都是紧支撑于 cell 内的光滑场的空间导数，在 cell 上积分为零，只剩 ∫∂_t w；力观测相等无条件 |
| 254-R45-compact-rider | worktree 安装中→自动启动（codex sol xhigh，带重试；基于 252 分支） | 09-16 1639Z | — | — | R45 **F_c 的正则参考 rider**：Spec 的 regularReference 在 Y = forceClassCompact 下（249 的 R41 rider 绑定为模板：233 记录一次调用 + 唯一性对齐 + R42 history/forceConvergence/速率夹逼；ε 形式与任意历史截断 τ 由极限导出；f ∈ F_c 由紧支撑闭包） |
