...
| 111-D01-p2-sl8-prep | 续改中 | — | — | P2 SL8 预备：`orderZeroDatum` 可加性/线性（唯一性）、`∂ₜu(t,·)` 与 `∇p(t,·)` 的 `MemLp 2` 与 `ContDiff`（`∂ₜu = h − ∇p`，`h ∈ H^∞`）、0 阶 `(I−P) datum⁰(h) = datum⁰(∇p)` 的表（等 108 的纵向引理）与 bootstrap 路线（085 `isSobolevDatum_lower_iff`），`Section4/D01/OrderZeroAlgebra.lean` + `research/D01/SL8_SPLIT.md` |
