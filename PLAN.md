...
| 195-A01-interior-momentum | fix 完成（hprojected 拆成投影识别 + 残差一致两个诚实输入；194 适配器；192 典范输入探针）；lead 复核（模块零输出、10 公理）；PR 已开，合并链运行中（wave 27） | 09-15 2350Z | #200 | — | A01 **P4b 内点动量恒等式**（189 重设计的另一半）：190 联合代表元的 ∂ₜu = 导数数据的求值（有界求值与 fderiv 交换）+ 代表元线性 ⇒ 在 Ioo 0 S 上 G = residual − ∂ₜu；t=0 不作任何断言 |
