...
| 226-R43-contract-v1 | 审稿 ACCEPT（sol，无修改）；PR 已开，合并链运行中（wave 58）；第 30 个合同 R43.critical_regularity V1 | 09-16 1409Z | #230 | — | R43 **V1 合同注册**（第 30 个合同 R43.critical_regularity）：Contracts/V1/CriticalRegularity.lean = Spec 的 RCritical1API 逐字（dotHomogeneousENorm 用已注册的 V1 定义）、Bindings 见证 {c := criticalConst, universal, inhomogeneousAtZero}（复用 223/225 审计里的 Data 词汇一致性证明与 maximalLifespanR 桥）、Tests、registry |
| 227-R44-endpoint | worker 完成（astra，16 分钟）：rcritical2_endpoint_of_differential——‖f‖_{L²H^{-1/2}} < radius ν S ⇒ ofReal S < maximalLifespanR ν 0 f，唯一具名输入 RCritical2Differential（θ = min(criticalConst, 1/(100(C_emb+1)³))，C₂=2，C₃=4，radius = θ/20·ν^{3/2}·e^{−3νS}；可积性只在闭预奇异窗口 [0,b]，b<T）；27 声明 + Data 词汇一致性；审稿中 | 09-16 1410Z | — | — | R44 **端点装配 S2–S6**：以 S1 微分不等式（拆分表 :56-64 的精确目标）为唯一具名输入 RCritical2Differential，走 166 的 Grönwall/首次逃逸 → L³ 门 → C01 V4 零初值 H² 预算 → G5 端点粘合 → A02/A04 延拓 ⇒ Prop. 4.4 的 a=0 结论（r_{ν,S} 显式）；S1d 之后即无条件 |
...
| 229-R44-prop44 | worktree 安装中→自动启动（codex astra low，带重试；基于 227 分支） | 09-16 1411Z | — | — | R44 **Prop. 4.4 无条件 + R41 q=2**：把 228 的 Absorption.lean 改造（import 227 的 Endpoint、去重、按单调性实例化 227 的 RCritical2Differential）；rcritical2_endpoint_unconditional；R41 q=2 非稠密子句 not_breakdownDenseR_zero_L2（s ≥ −1/2，ρ = radius ν T，224 模板） |
