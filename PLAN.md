...
| 180-A01-b2-assembly | 第四轮审稿 REJECT（构造器与压力腿本身正确；缺陷在供给方量词：管线里 189 前提写成对任意场的全称式被 Lean 证伪；消费者环的 hjoint 没带 hpaths 接不上 190 的真定理）；fix5 运行中（消费者环用落地定理原形；189 义务定义为作用于选定解的 PressureSupply） | 09-15 2315Z | #190 | — | A01 **B2 装配**：用已合入部件（161/162/167/168/169/173 + 153/157）装出有条件的 ClassicalSolutionR 构造子 carrierConstructor_of_localTheory，只留联合光滑性 hc3（B1 R3/R4）与可能的 hcurl 为具名假设；CarrierConstructorFull 形状 + 消费者环探针 |
...
| 189-A01-pressure-regularity | fix 部分完成（astra）；重设计拆成 194（余项路径）+ 195（内点恒等式），189 之后装配 hG_int/hG | 09-15 2303Z | #— | — | A01 **P4 压力梯度正则性**：G = residual − ∂ₜu 在开板 [0,S)×R³ 上 C∞、逐时 L²（Leray 余项）、对称 Jacobian（余项是梯度）——180 的 `hpg`；输入 hc3/hsob/散度/169 的投影动量恒等式 |
...
| 194-A01-complement-path | worker 完成（astra：exists_complement_paths（残差 Leray 余项的全阶 C^j 数据路径 + 物理路径 w + 逐时余项恒等式）、exists_complement_joint_representative；输入仅 192 的柱对合取；40 项公理审计干净）；codex 审稿中 | 09-15 2332Z | #— | — | A01 **P4a 余项路径**（189 重设计的一半）：残差 νΔu+f−(u·∇)u 的 Leray 余项在每个阶的 C^j_t 数据路径 + 一条物理路径 w + 逐时"w t 是残差 0 阶数据的余项"；经 190 得联合光滑场 G |
| 195-A01-interior-momentum | 审稿 REJECT（目标 1/2 正确；目标 3 条件于 hprojected——lead 判定这是设计内的命名输入，由 197 供给）；fix 运行中（说清 hprojected 捆绑了投影识别 + 物理残差一致，拆成两个诚实输入；加 194 导出的适配器） | 09-15 2334Z | #— | — | A01 **P4b 内点动量恒等式**（189 重设计的另一半）：190 联合代表元的 ∂ₜu = 导数数据的求值（有界求值与 fderiv 交换）+ 代表元线性 ⇒ 在 Ioo 0 S 上 G = residual − ∂ₜu；t=0 不作任何断言 |
| 196-A01-mild-gronwall | 审稿 ACCEPT-WITH-NOTES（吸收代数 256→255 变异有核验反例；备注：两个能量谓词加无散假设 ha（允许改 193 模块）、wrapper 阶数句、rebase）；fix 运行中 | 09-15 2336Z | #— | — | A01 **MildGronwall**（A3 最后一个分析输入）：vendor 正则化 mild 能量链 `mild_majorized_energy_subinterval` 特化到常欧氏度量与 q+1 阶有限 word 族，顶阶非线性项走 A03.outerProductTame → A04.outerSobolevNormAt_le → A04.inner_energy_Rhigh；显式 E q, C q |
| 197-A01-leray-bridge | worker 完成（astra，部分：条件 Helmholtz 比较 hprojected_of_cylinder 恰为 195 目标、195 消费探针通过；梯度数据被余项固定、无散数据被余项零化已证；未闭合：未投影残差下降、力相容、物理残差 a.e. 一致（发现 169 的 ordinaryResidualPath 已是投影后的）；9 声明公理干净）；codex 审稿中（要求给出每个命名输入由哪条 lane 供给的映射） | 09-15 2333Z | #— | — | A01 **P4c 投影桥 hprojected**（195 的唯一命名输入）：柱面 Leray 投影 `leray 1 q` 与数据级 Fourier `lerayComplement 0` 在下降数据上一致——梯度部分纵向、投影部分无散、物理残差数据与下降柱面残差一致（`physicalResidual_datum_eq`，194 也用） |
| 198-A01-energy-premises | 等 196 fix 完成后自动建 worktree 并启动（codex astra low；基于 196 分支） | 09-15 2337Z | — | — | A01 **A3-M2 能量前提**（FiniteMildEnergy 上半）：为柱面 NS 解构造 vendor `mild_majorized_energy_subinterval` 的前提（无散路径、梯度压力、最大正则 Bochner 态 U∈TimeLp(H^{q+2})、源/压力代表元及限制恒等式），恒等度量、全 word 族 N=q−5；导出积分根能量估计，Z 的界与包络转换留给 199 |
