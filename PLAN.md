...
| 110-B02-approx-compact | 续改中 | — | — | B02 结论字段 `approxCompactHomogeneous`（M，90–140 行）：跨 realization 的三角不等式粘合 `bochnerDatumENorm q s (separatedPath φ A − separatedPath φ A') ≤ ∑ ‖φ_j‖_{L^q}·‖A_j − A'_j‖ₑ` + `temporalApprox` + `spatialApproxHomogeneous` + `separatedAssembly`（103），`Section4/B02/ApproxCompact.lean` |
