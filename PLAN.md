...
| 180-A01-b2-assembly | 第五轮审稿 REJECT（PressureSupply 只带单阶 u，而已合入的 194 需要同载体全阶 hpairs 族含角不变性；192 主定理返回该族但 wrapper 特化到 q）；fix6 运行中（PressureSupply 带 194 形状的 hpairs + ha/典范力；消费者环用 192 返回族的主定理；真实 194→195→197 管线探针） | 09-15 2359Z | #190 | — | A01 **B2 装配**：用已合入部件（161/162/167/168/169/173 + 153/157）装出有条件的 ClassicalSolutionR 构造子 carrierConstructor_of_localTheory，只留联合光滑性 hc3（B1 R3/R4）与可能的 hcurl 为具名假设；CarrierConstructorFull 形状 + 消费者环探针 |
...
| 197-A01-leray-bridge | fix 完成（astra：柱面 Helmholtz 两个隶属关系已证；194 的物理数据接到 195 且不再需要一致性假设；导出典范桥，消费者探针无 hprojected 闭合；合并了 194 分支）；codex 复审中 | 09-16 0004Z | #— | — | A01 **P4c 投影桥 hprojected**（195 的唯一命名输入）：柱面 Leray 投影 `leray 1 q` 与数据级 Fourier `lerayComplement 0` 在下降数据上一致——梯度部分纵向、投影部分无散、物理残差数据与下降柱面残差一致（`physicalResidual_datum_eq`，194 也用） |
