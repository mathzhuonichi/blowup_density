...
| 144-MAINT-zero-solution | 审稿中 | 09-14 0505Z | — | — | MAINT：把零解见证落地一次（`Section4/A04/ZeroSolution.lean`：`zeroSol : ClassicalSolutionR ν 0 0 T`、`memForceR_zero`、`zero_mem_initialClassR`、`HasSmoothSobolevPath` 见证、`sobolevNormAt_zero`），八次审稿各自重建；之后各 axioms_*.lean 可 import 它 |
