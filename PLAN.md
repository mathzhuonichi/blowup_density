...
| 230-A04-v2-restart-fixed-force | 审稿 ACCEPT-WITH-NOTES（astra 对照论文；三处措辞修正已改）；注册文件并集合并后 PR 已开，合并链运行中（wave 84）；第 35 个合同 | 09-16 1744Z | #256 | — | A04 **V2 合同**（选项 1）：Restart = RestartFixedForce（固定外力、对 t₀∈[0,S] 与 H⁷ 数据一致）+ HigherOrderBound + 217 形状的三条延拓定理；V1 的 H¹ Restart 留作明确未证；同样的防 reward-hacking 要求 |
