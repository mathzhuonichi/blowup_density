# Lane 212-A01-contract-v2 — register the A01 local theory as a V2 contract (OPTION 2: manuscript H¹ wording kept; `horizon_lower_bound` NOT registered yet)
...
**Owner decision (recorded by the lead):** keep the manuscript H¹ wording. Register V2 WITHOUT the `horizon_lower_bound` field (structure `LocalTheoryAPIPartial`: `horizon`, `solution`, `regularity`), and state the H¹ field verbatim as a separate unregistered `def` in the contract file with a docstring "open: needs a forced H¹ local theory (lane TBD)"; do not register the H⁷ variant as the contract field.
