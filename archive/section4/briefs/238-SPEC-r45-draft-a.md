# Lane 238-SPEC-r45-draft-a — double-blind specification of Corollary 4.x "cor:Rclasses" (R45: Theorem 4.1 for the compactly supported and rapidly decaying force classes F_c, F_rd): draft A

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/238-SPEC-r45-draft-a` (git branch `erenup/238-SPEC-r45-draft-a`, based on `origin/erenup/integration`). This is one of two
...
1. `research/R45/DraftA.lean` (create the directory): a Lean file that **elaborates** (`cd verification && lake env lean ../research/R45/DraftA.lean`) containing `structure RClassesAPI`
...
2. `research/R45/COMPARISON_A.md`: paper-clause → Lean field table; choices (parametric `Y` vs copies, casts, how "relative topology" is rendered, what the corollary inherits from Theorem 4.1
   vs. what is new — e.g. that the inserted forces stay in `F_c`/`F_rd`); every ambiguity in the manuscript.
3. Commit on your branch (never push/merge/rebase). Report in four parts and write it to `research/R45/REPORT_238.md`.
