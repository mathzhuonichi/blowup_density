...
- **2026-09-16 0125Z**：180（#190 经典解构造器，六轮审稿）、198（#202 能量前提）合入，170 模块。在跑：189 fix3（import 180 的 `PressureSupply`、A01 构造器管线探针；合入后构造器只条件于 `hb`）、199（`ForcingFamilyBound` + `EnvelopeConversion` ⇒ `FiniteMildEnergy`，astra）。199 若闭合则 `hb` 无条件，A01 经典解构造器在集成分支上无条件成立——届时开一条接线 lane 把 `LocalTheoryAPI`/A01 合同注册为 V2。
