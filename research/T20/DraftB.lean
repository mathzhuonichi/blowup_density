...
namespace BlowupDensity.T20.DraftB
...
def periodicGradientSq (v : SpatialField) : ℝ :=
...
`mean_free_equation` retains `constantTransportT`.  Every occurrence of a
totalized norm is protected collectively by `reduction_regular`, while every
`toReal` use has an explicit `≠ ⊤` conclusion there. -/
structure CriticalRegularityTAPI
    (_localTheory : PeriodicLocalTheoryAPI)
    (_continuation : PeriodicContinuationAPI)
    (_meanReduction : PeriodicMeanReductionAPI)
    (_calculus : MeanZeroSobolevCalculusAPI) : Type where
  ...
  Non-vacuity: `c_pos` below forces a genuinely positive ball. -/
  c : ℝ
  ...
  Non-vacuity: `Ccritical_pos` excludes the zero-constant trap. -/
  Ccritical : ℝ

  /-- `03-torus.tex:426-440`: positivity of `Ccritical`.
  Exact quantifier order: no later parameters.
  Non-vacuity: the bootstrap level `ν/(2*Ccritical)` is meaningful. -/
  Ccritical_pos : 0 < Ccritical

  /-- `03-torus.tex:467-484`: the absolute coefficient of `y‖Δv‖²₂`
  in the `H¹` convection estimate, fixed before all PDE data.
  Exact quantifier order: it is structure data before `ν,g,T,w,t`.
  Non-vacuity: `Censtrophy_pos` below rules out zero. -/
  Censtrophy : ℝ
  ...
  Censtrophy_pos : 0 < Censtrophy

  /-- `03-torus.tex:479-484`: the absolute constant after Young's inequality
  in `eq:H1energy`.
  Exact quantifier order: it is structure data before `ν,g,T,w,t`.
  Non-vacuity: `CHone_pos` below prevents a zero right-hand coefficient. -/
  CHone : ℝ

  /-- `03-torus.tex:479-484`: positivity of the `H¹` energy constant.
  Exact quantifier order: no later parameters.
  Non-vacuity: this constrains the actual constant used in `hOne_energy`. -/
  CHone_pos : 0 < CHone
  ...
  Non-vacuity: `Ccriterion_pos` below forces it to be positive. -/
  Ccriterion : ℝ

  /-- `03-torus.tex:492-500`: positivity of the continuation constant.
  Exact quantifier order: no later parameters.
  Non-vacuity: the finite bound in `continuation_bound` is quantitative. -/
  Ccriterion_pos : 0 < Ccriterion
  ...
  bootstrap regime. -/
  c_lt_critical : c < 1 / (4 * Ccritical)

  /-- `03-torus.tex:477-480`: the second required shrinking
  `c < 1/(4 C₁)`.
  Exact quantifier order: constants were selected first.
  Non-vacuity: this is the stated absorption gate for `eq:H1energy`. -/
  c_lt_enstrophy : c < 1 / (4 * Censtrophy)

  /-- `03-torus.tex:389-392`: the proposition starts the local solution from
  rest, and that fixed datum belongs to the manuscript's periodic initial
  class.
  Exact quantifier order: none; the datum is literally the zero field.
  Non-vacuity: this supplies the `initialClassT` premise needed by T11 rather
  than leaving admissibility of the datum implicit. -/
  zero_initial_admissible : (fun _ : Space ↦ 0) ∈ initialClassT

  /-- `03-torus.tex:392-418,467-488`: regularity and finiteness of every
  mean-free slice on which the displayed norms are evaluated.

  Exact quantifier order: `ν,hν,g,hg,T,w,t,ht`.  The conclusion gives
  mean zero, smooth periodicity, finite homogeneous `1/2` and `3/2` norms,
  finite `H²`, and finiteness of the three `L²` quantities sent through
  `toReal`.
  Non-vacuity: `w` is a full `ClassicalSolutionT`, and every conclusion is a
  concrete T10/T12 membership or norm-finiteness statement. -/
  reduction_regular : ∀ (ν : ℝ), 0 < ν →
  ...
  /-- `03-torus.tex:395-403`: the spatial mean of the actual solution is the
  data-defined path `meanPathT`.
  Exact quantifier order: `ν,hν,g,hg,T,w,t,ht`.
  Non-vacuity: this equates the Haar mean of a concrete classical velocity
  slice with an actual Bochner interval integral. -/
  mean_identification : ∀ (ν : ℝ), 0 < ν →
    ∀ (g : SpaceTimeField), g ∈ forceClassT →
      ∀ (T : ℝ) (w : ClassicalSolutionT ν (fun _ : Space ↦ 0) g T),
        ∀ t ∈ Ico (0 : ℝ) T,
          meanT (fun x ↦ w.velocity (t, x)) = meanPathT g t

  /-- `03-torus.tex:402-405`: `m(0)=0` for the zero initial datum.
  Exact quantifier order: `g,hg`.
  Non-vacuity: the conclusion fixes the actual data-defined mean vector at
  time zero, rather than an auxiliary scalar. -/
  mean_initial : ∀ (g : SpaceTimeField), g ∈ forceClassT →
    meanPathT g 0 = 0

  /-- `03-torus.tex:402-403`: `m'(t)=meanT(g(t,·))` at interior times.
  Exact quantifier order: `g,hg,t` with `0<t`.
  Non-vacuity: `HasDerivAt` asserts the genuine derivative of the concrete
  Bochner-integral mean path. -/
  mean_derivative : ∀ (g : SpaceTimeField), g ∈ forceClassT →
    ∀ t : ℝ, 0 < t → HasDerivAt (meanPathT g) (forceMeanT g t) t

  /-- `03-torus.tex:404-406`, `eq:meanbound`:
  `|m(t)| ≤ ∫₀ᵗ |gbar(s)| ds ≤ ρ`.
  Exact quantifier order: `g,hg,t,ht`; the estimate is independent of a
  chosen solution because `meanPathT` is data-defined.
  Non-vacuity: all three quantities are in `ℝ≥0∞`; failure of integration
  cannot be silently totalized to zero. -/
  mean_bound : ∀ (g : SpaceTimeField), g ∈ forceClassT →
    ∀ t : ℝ, 0 ≤ t →
  ...
  constant_transport_skew : ∀ (m : Space) (v w : SpatialField),
  ...
  constant_transport_commutes_lambda :
  ...
  derivative; `reduction_regular` makes every `toReal` operand finite. -/
  critical_energy : ∀ (ν : ℝ), 0 < ν →
    ∀ (g : SpaceTimeField), g ∈ forceClassT →
      ∀ (T : ℝ) (w : ClassicalSolutionT ν (fun _ : Space ↦ 0) g T),
        ∀ t ∈ Ioo (0 : ℝ) T,
          ∃ E' : ℝ,
  ...
  b_integral : ∀ (g : SpaceTimeField), g ∈ forceClassT →
  ...
  Non-vacuity: `reduction_regular` supplies finiteness for every norm passed
  ...
  /-- `03-torus.tex:490-502` using `eq:criterion`: every finite endpoint at
  or below the maximal lifespan has finite squared `H²` integral.
  Exact quantifier order: `ν,hν,g,hg,hsmall,u,p,hmax,S,hS,hSlife`.
  Non-vacuity: `IsMaximalPeriodicSolution` supplies actual common solution
  fields, and the conclusion is exactly the premise consumed by T11's
  `lifespanInfiniteOfLocallyFinite`. -/
  maximal_criterion_finite : ∀ (ν : ℝ), 0 < ν →
    ∀ (g : SpaceTimeField), g ∈ forceClassT →
      criticalRho g < ENNReal.ofReal (c * ν) →
        ∀ (u : SpaceTimeField) (p : SpaceTimeScalar),
          IsMaximalPeriodicSolution ν (fun _ : Space ↦ 0) g u p →
            ∀ S : ℝ, 0 < S →
              ENNReal.ofReal S ≤
                  maximalLifespanT ν (fun _ : Space ↦ 0) g →
                squaredHTwoIntegralT S u ≠ ⊤

  /-- **Proposition 3.7**, `03-torus.tex:383-390` (`eq:smallcritical`):
  `g∈F_T` and `ρ<cν` imply `T_max^ν(0,g)=∞`.
  Exact quantifier order: the structure's universal `c`, then
  `ν,hν,g,hg,hsmall`.
  Non-vacuity: `c_pos`, `ν>0`, explicit force-class membership, and the
  fail-safe `ℝ≥0∞` force norm make the small ball genuine; the conclusion is
  literal top lifespan. -/
  global_regular : ∀ (ν : ℝ), 0 < ν →
    ∀ (g : SpaceTimeField), g ∈ forceClassT →
      criticalRho g < ENNReal.ofReal (c * ν) →
        maximalLifespanT ν (fun _ : Space ↦ 0) g = ⊤

end BlowupDensity.T20.DraftB
