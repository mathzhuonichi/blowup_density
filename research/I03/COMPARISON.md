...
| `packetNegativeHomogeneous` `‖F_ε‖_{L^q_tḢ^s} ≤ Cε^{β}` | `04-whole-space.tex:70-76`, and at `q=2,s=−1` `:264-272` | **lane 250: exact homogeneous Fourier, angular vector-datum and time scaling proved.** `Section4/I03/HomogeneousScaling.lean`; `Bindings/ScalingHomogeneous.lean` proves the registered exact packet identity and supplies the actual finite profile norm as constant | **conditional, still unregistered:** only `CompactHomogeneousRealization` (a.e. strong time measurability of D01’s explicit compact homogeneous path) remains. Slice validity and norm comparison are proved. See `ATTEMPTS_HOMOGENEOUS.md` and `REPORT_250.md`. | **Closed by lane 255** (`compactHomogeneousPath_continuous` ⇒ `CompactHomogeneousRealization`; the shared correction realization input is discharged).
...
| `correctionNegativeHomogeneous` | `04-whole-space.tex:74-75`, `:271` | **lane 250: actual correction homogeneous component bounds proved**, with the uniform compact profile constants and exponent `β+1`. `homogeneousScaling` assembles both exact existing API fields on the same `Bindings.scaling C th` family and threshold | **conditional on the same single compact-path measurability input**, still unregistered. Uses a genuine homogeneous datum witness, not the inhomogeneous bound. See `ATTEMPTS_HOMOGENEOUS.md`. | **Closed by lane 255** (`compactHomogeneousPath_continuous` ⇒ `CompactHomogeneousRealization`; the shared correction realization input is discharged).
