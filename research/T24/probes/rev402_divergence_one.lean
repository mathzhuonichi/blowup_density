...
point. -/
theorem rev402_divergence_one {U : VelocityField} (c : Space) (r τ₀ τ₁ : ℝ)
    ...
    ∀ b : VelocityField, AffineAdmissible c r τ₀ τ₁ b →
      ∀ t ∈ Ico (0 : ℝ) 1, ∀ x : Space,
        spatialDivergence (affineVelocity U b) t x = 1 := by
  intro b hb t ht x
  ...
  change spatialDivergence (fun z ↦ U z + b z) t x = 1
