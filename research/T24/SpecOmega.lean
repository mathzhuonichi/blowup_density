...
namespace BlowupDensity.T24.MultipleOmega
...
structure MultipleRegionsOmegaAPI {ν : ℝ}
  ...
  -- TORUS→Ω: see COMPARISON_OMEGA.md for this domain field.
  placement : Fin N → DomainPlacementData P.velocity P.pressure P.force P.carrier
  ...
  component : ∀ j : Fin N,
    ClassicalSolutionOmega ν Ω (0 : SpatialField)
      (scaledForce P.force (placement j).x₀ T (ε j)) T
  ...
  component_pin : ∀ j : Fin N,
    (component j).velocity = scaledVelocity P.velocity (placement j).x₀ T (ε j) ∧
      (component j).pressure = domainNormalizePressure Ω (scaledPressure P.pressure (placement j).x₀ T (ε j))
  ...
  component_force_support : ∀ j : Fin N, ∀ t : ℝ, ∀ x : Space,
    x ∉ Metric.ball (regionCenter j) (regionRadius j) →
      scaledForce P.force (placement j).x₀ T (ε j) (t, x) = 0
  ...
  assembled_force_formula :
    assembled_force =
      finiteForceSum (fun j ↦ scaledForce P.force (placement j).x₀ T (ε j))
...
end BlowupDensity.T24.MultipleOmega
