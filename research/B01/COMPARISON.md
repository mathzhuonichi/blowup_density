...
`BlowupDensity.B01.Draft`, structure `BochnerApproxAPI` (422 lines, typechecks
...
`SeparatedCompactDense`, `bochnerApproxStatement`; plus the assembly helper
`schwartzVector`.
...
`:231-260`; the ledger's `B01` export,
...
(`04-whole-space.tex:241-249`, node `B02`), the singular-force half
(`04-whole-space.tex:262`, node `R41D`/`R45`), and the four simultaneous
...
* `CFA  = formalization/NSFormalization/Paper3/CompactForceAdmissibility.lean`
* `CAF  = formalization/NSFormalization/Paper3/CompactAdmissibleForce.lean`
* `AF   = formalization/NSFormalization/Paper3/AdmissibleForce.lean`
...
* **vector-ness**: real Euclidean three-vector throughout;
  ...
  the manuscript's "take real parts" step (`04-whole-space.tex:249`) is already
  discharged by the carrier.
* **`s` and `q` ranges**: unrestricted real `s`, and `1 ≤ q < ∞`.  Matches the
  manuscript exactly (`04-whole-space.tex:231,239,251`).  **No threshold.**
...
| `compactSubsetForceR` | `04:183` ("two subclasses of `F_R`"), presupposed by cor:Rclasses `04:194-196` (quote at `:195`), against `02-prelim:17` eq:Rclasses | **Largely present at the complex-cycles level.**  `CAF:46` `admissibleForce_compactForceDistribution` proves, for *every* compact smooth `F : VelocityField`, the **exact shape** of `MemForceR`'s second conjunct: `AF:30` `AdmissibleForce g = ∀ m, ∃ G, (∀ t ≥ 0, realization = g t) ∧ ContDiffOn ℝ ∞ G (Ici 0) ∧ MemLp G 1 positiveTimeMeasure ∧ MemLp G 2 positiveTimeMeasure`, with `AF:16` `ForceDatum m = Fin 3 → SobolevHilbert m`.  `CFA:14` `contDiff_compactVectorFourierLp` already performs the `contDiff_piLp` **vector** transport, and `CFA:52` `compact_vector_force_sobolev_regular` bundles it with `MemLp … 1`/`2` at every integer `m`.  Scalar ancestors `CST:18,39,47` (built on `SCT:16,37,49`) and `RPD:86`.  At the angular real-vector level `ARVB:64` `memLp_angularRealVectorSlice` already gives both `MemLp`s at **every** `q`, so `q = 1, 2` are immediate. | **The remaining gap is narrower than a first reading suggests, but it is real.**  Missing is only the `ContDiff` transport across the *reality* and *convention* maps: no `ContDiff` statement exists for `realCompactSobolevTimeSlice` (`RPD:54`), `realVectorSlice` (`RVPD:46`) or `angularRealVectorSlice` (`ARVB:47`), each a composition with `RPD:21` `realProjectionTo` (a CLM) or `ARVB:15` `cyclesToAngularRealVector` (a CLE).  The `PiLp` step is **not** missing (`CFA:14`) and the target shape is **not** unprecedented (`CAF:46`).  Also needed: the components `fun z => (f z).ofLp i` of a given `f` with `MemForceCompact f` — but `CAF:34` `compactForceDatum` with `coordinateForce_smooth`/`coordinateForce_compact` already does exactly that at the cycles level. |
...
1. **Does `exists_angular_real_vector_positive_physical_approx` already give the
   ...
   enters `prop:Renergy` only at `04-whole-space.tex:262`, where
...
* **Angular versus cycles is a bounded equivalence, not an isometry.**
  `ARVB:24` `cyclesToAngularRealVector_norm_le` bounds it by
...
| 4 | `contDiff_angularRealVectorSlice` | S–M | `ContDiff ℝ ∞ (realCompactSobolevTimeSlice s F hF hc)`, then `… (realVectorSlice s F hF hc)`, then `… (angularRealVectorSlice s F hF hc)`. | **Written template:** `CFA:14` `contDiff_compactVectorFourierLp` is the same proof one convention over (`(contDiff_piLp 2).mpr` + `CST:18` per component); copy it and insert `(realProjectionTo s).contDiff.comp` (`RPD:21`) and `(cyclesToAngularRealVector s).contDiff.comp` (`ARVB:15`). |
| 5 | `compactSubsetForceR` | S–M | `forceClassCompact ⊆ forceClassR`. | Follow `CAF:46` `admissibleForce_compactForceDistribution`, which is `MemForceR`'s exact shape (`AF:30`) at the cycles level, line for line: components by `CAF:34`'s `coordinateForce_smooth`/`coordinateForce_compact`; then unit 1 at order `(m : ℝ)`, unit 4 with `.contDiffOn`, and `ARVB:64` at `q = 1, 2` (`CFA:52` is the bundled cycles statement). |
...
| 9 | `schwartzApprox` + `cutoffApprox` | M | The two fidelity-only fields. | `SD:43` `denseRange_weightedFourierLp` and `SD:51`/`SCA:169`/`SCA:188`, componentwise, then `schwartzVector`.  Both fields now carry `ψ : Fin 3 → SchwartzMap Space ℝ`, so **no** `SchwartzMap` product assembly is needed — that bundling was the only reason this unit was `L`.  **Fidelity-only:** `R46` consumes neither field and unit 8 does not depend on them; both fields (and this unit) are deletable if the lane prefers a minimal contract. |
| 10 | `BochnerApproxAPI` term + `SeparatedCompactDense` + registration | M | `χ := CC:29 baseCutoff` (five fields discharged by `CC:40,46,50,42,44`, checked); assemble units 2–9; derive `SeparatedCompactDense q s` from `temporalApprox` + `spatialApprox` + `separatedAssembly` (the ledger export shape, `STATEMENTS.md:825-830`); promote to `verification/Contracts/V1/BochnerApprox.lean`, add the typed `Bindings` entry and the `#print axioms` test. | Packaging pattern of `verification/Contracts/V1/Thresholds.lean` and `verification/contracts.json`. |
...
`B02` (homogeneous `Ḣ^{-1}`, `04-whole-space.tex:241-249`) shares this lane's
**stage 4 verbatim** and nothing else.  Concretely:

* **Shared, unchanged.**  `SeparatedTemporalDense q s` (`Spec.lean`, and field
  `temporalApprox`).  `04-whole-space.tex:251` writes the Bochner step for
...
* **Not shared.**  The spatial stage.  `B02`'s is the annular construction of
  `04-whole-space.tex:241-249` (`G_n ∈ C_c^∞(R³∖{0})`, `ĥ_n = |ξ|G_n`) followed
  by the low/high split `eq:Rnegative-cutoff`
  (`‖k‖²_{Ḣ^{-1}} ≤ C‖k‖₁² + ‖k‖₂²`, `04-whole-space.tex:242-248`) in place of
...
and no `-j`.  Re-run after applying `research/B01/REVIEW.md` issues 1-5.
...
3. `cd verification && lake env lean ../research/B01/Spec.lean` — exit 0, no
   output (3.7 s before the review edits, 3.6 s after).
...
5. After the review edits, re-checked: `#print axioms` on `BochnerApproxAPI`,
   `SeparatedCompactDense`, `SeparatedTemporalDense`, `schwartzVector` →
   `[propext, Classical.choice, Quot.sound]`; `schwartzVector ψ x i = ψ i x` by
   `rfl`.  Every renumbered paper anchor was re-read in
   `paper/sections/04-whole-space.tex` (176-200, 228-266) and
   `paper/sections/02-preliminaries.tex` (58-75); the reviewer's
   `CompactAdmissibleForce.lean:44` is the declaration at **`:46`** (`:44` is
   the last line of the preceding proof), and `CompactForceAdmissibility.lean:14`
   and `:52` are exact.
6. `make check` — all four checks pass.
