...
/-! Reviewer negative probe: the canonical coefficient `2` is mutated to `1`.
...
example (ν : ℝ) {v : SpaceTimeField}
    ...
    ∀ ε ∈ Ioc (0 : ℝ) (correctionData v x₀ T θ η O θR ε₀).ε₀,
      tsupport (correctionForce ν v (correctionData v x₀ T θ η O θR ε₀) ε) ⊆
        Ioo (T - ε ^ 2) (T + ε ^ 2) ×ˢ
          periodicSet (ball x₀
            (ε * (correctionData v x₀ T θ η O θR ε₀).θRadius)) := by
  exact force_support ν hv x₀ T δ r hvper O θR ε₀ hθ hη hθc hηc
