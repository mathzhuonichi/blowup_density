...
/-!
# T17 reconciled specification: bounds for the background correction

Statement-only reconciliation of `lem:correction`, `paper/sections/03-torus.tex:218-285`,
following `research/T17/RECONCILIATION.md`.  The copied vocabulary below is
limited to still-unregistered declarations; no field is a placeholder.
-/
...
namespace BlowupDensity.T17.CorrectionCanonicalProbe.Spec
...
end BlowupDensity.T17.CorrectionCanonicalProbe.Spec

namespace BlowupDensity.T17.CorrectionCanonicalProbe
...
/-! ## Structure-exception adapters for the three upstream records -/

def placementOfSpec {ν : ℝ} {P : PacketAPI ν}
...
def placementToSpec {ν : ℝ} {P : PacketAPI ν}
    (place : NSFormalization.Section3.T15.PlacementData
      P.velocity P.pressure P.force P.carrier) :
    BlowupDensity.T15.Spec.PlacementData P :=
  { T := place.T
    time_pos := place.time_pos
    chartCenter := place.chartCenter
    chartRadius := place.chartRadius
...
theorem placementToSpec_ofSpec {ν : ℝ} {P : PacketAPI ν}
    (place : BlowupDensity.T15.Spec.PlacementData P) :
    placementToSpec (placementOfSpec place) = place := rfl
...
def cutoffOfSpec (D : BlowupDensity.T16.Draft.CutoffData) :
...
def cutoffToSpec (D : NSFormalization.Section3.T16.CutoffData) :
    BlowupDensity.T16.Draft.CutoffData :=
  { θ := D.θ
    η := D.η
    plateau := D.plateau
    θRadius := D.θRadius
...
theorem cutoffToSpec_ofSpec (D : BlowupDensity.T16.Draft.CutoffData) :
    cutoffToSpec (cutoffOfSpec D) = D := rfl

theorem cutoffOfSpec_toSpec (D : NSFormalization.Section3.T16.CutoffData) :
    cutoffOfSpec (cutoffToSpec D) = D := rfl

theorem localPotentialOfSpec {v U : SpaceTimeField} {K : Set Space}
    {x₀ : Space} {r T δ : ℝ} {D : BlowupDensity.T16.Draft.CutoffData}
    (h : BlowupDensity.T16.Draft.LocalPotentialAPI v U K x₀ r T δ D) :
    NSFormalization.Section3.T16.LocalPotentialAPI
      v U K x₀ r T δ (cutoffOfSpec D) :=
...
theorem localPotentialToSpec {v U : SpaceTimeField} {K : Set Space}
    {x₀ : Space} {r T δ : ℝ} {D : NSFormalization.Section3.T16.CutoffData}
    (h : NSFormalization.Section3.T16.LocalPotentialAPI v U K x₀ r T δ D) :
    BlowupDensity.T16.Draft.LocalPotentialAPI
      v U K x₀ r T δ (cutoffToSpec D) :=
  { theta_smooth := h.theta_smooth
    theta_compactSupport := h.theta_compactSupport
    theta_range := h.theta_range
    plateau_open := h.plateau_open
...
theorem localizationOfSpec (h : BlowupDensity.T13.Spec.LocalizationAPI) :
...
theorem localizationToSpec (h : NSFormalization.Section3.T13.LocalizationAPI) :
    BlowupDensity.T13.Spec.LocalizationAPI :=
  { constant_pos_finite := h.constant_pos_finite
    wholeSpace_identity := h.wholeSpace_identity
    torus_identity := h.torus_identity
    localization := h.localization
...
/-! ## Definitional vocabulary bridges -/

theorem fixedProfileCylinder_eq (D : BlowupDensity.T16.Draft.CutoffData) :
    Spec.fixedProfileCylinder D =
      NSFormalization.Section3.T17.fixedProfileCylinder (cutoffOfSpec D) := rfl

theorem correctionChartPoint_eq {ν : ℝ} {P : PacketAPI ν}
    (place : BlowupDensity.T15.Spec.PlacementData P) (ε : ℝ) (z : SpaceTime) :
    Spec.correctionChartPoint place ε z =
      NSFormalization.Section3.T17.correctionChartPoint
        (placementOfSpec place).x₀ (placementOfSpec place).T ε z := rfl

theorem rescaledReference_eq {ν : ℝ} {P : PacketAPI ν}
    (v : SpaceTimeField) (place : BlowupDensity.T15.Spec.PlacementData P) (ε : ℝ) :
    Spec.rescaledReference v place ε =
      NSFormalization.Section3.T17.rescaledReference
        v (placementOfSpec place).x₀ (placementOfSpec place).T ε := rfl

theorem rescaledPotential_eq {ν : ℝ} {P : PacketAPI ν}
    (v : SpaceTimeField) (place : BlowupDensity.T15.Spec.PlacementData P) (ε : ℝ) :
    Spec.rescaledPotential v place ε =
      NSFormalization.Section3.T17.rescaledPotential
        v (placementOfSpec place).x₀ (placementOfSpec place).T ε := rfl

theorem rescaledCorrectionProfile_eq {ν : ℝ} {P : PacketAPI ν}
    (v : SpaceTimeField) (place : BlowupDensity.T15.Spec.PlacementData P)
    (ε : ℝ) (D : BlowupDensity.T16.Draft.CutoffData) :
    Spec.rescaledCorrectionProfile v place ε D =
      NSFormalization.Section3.T17.rescaledCorrectionProfile
        v (placementOfSpec place).x₀ (placementOfSpec place).T ε (cutoffOfSpec D) := rfl

theorem rescaledForceProfile_eq {ν : ℝ} {P : PacketAPI ν}
    (v : SpaceTimeField) (place : BlowupDensity.T15.Spec.PlacementData P)
    (ε : ℝ) (D : BlowupDensity.T16.Draft.CutoffData) :
    Spec.rescaledForceProfile ν v place ε D =
      NSFormalization.Section3.T17.rescaledForceProfile
        ν v (placementOfSpec place).x₀ (placementOfSpec place).T ε (cutoffOfSpec D) := rfl

theorem correctionForce_eq (ν : ℝ) (v : SpaceTimeField)
    (D : BlowupDensity.T16.Draft.CutoffData) (ε : ℝ) :
    Spec.correctionForce ν v D ε =
      NSFormalization.Section3.T17.correctionForce ν v (cutoffOfSpec D) ε := rfl

theorem torusSpaceTimeLift_eq (f : SpaceTimeField) :
    Spec.torusSpaceTimeLift f = NSFormalization.Section3.T17.torusSpaceTimeLift f := rfl

theorem torusSpatialSupport_eq (f : SpaceTimeField) :
    Spec.torusSpatialSupport f = NSFormalization.Section3.T17.torusSpatialSupport f := rfl
...
def ofSpec {ν : ℝ} {P : PacketAPI ν}
    {place : BlowupDensity.T15.Spec.PlacementData P}
    {v : SpaceTimeField} {r δ : ℝ} {D : BlowupDensity.T16.Draft.CutoffData}
    (h : Spec.CorrectionAPI ν place v r δ D) :
    NSFormalization.Section3.T17.CorrectionAPI ν
      (placementOfSpec place) v r δ (cutoffOfSpec D) :=
  { potential := localPotentialOfSpec h.potential
...
def toSpec {ν : ℝ} {P : PacketAPI ν}
    ...
    Spec.CorrectionAPI ν (placementToSpec place) v r δ (cutoffToSpec D) :=
  { potential := localPotentialToSpec h.potential
    localization := localizationToSpec h.localization
    viscosity_pos := h.viscosity_pos
    radius_pos := h.radius_pos
    ball_in_chart := h.ball_in_chart
    eps_le_placement := h.eps_le_placement
...
theorem toSpec_ofSpec {ν : ℝ} {P : PacketAPI ν}
    {place : BlowupDensity.T15.Spec.PlacementData P}
    {v : SpaceTimeField} {r δ : ℝ} {D : BlowupDensity.T16.Draft.CutoffData}
    (h : Spec.CorrectionAPI ν place v r δ D) :
    toSpec (ofSpec h) = h := by cases h; rfl

theorem ofSpec_toSpec {ν : ℝ} {P : PacketAPI ν}
    {place : NSFormalization.Section3.T15.PlacementData
      P.velocity P.pressure P.force P.carrier}
    {v : SpaceTimeField} {r δ : ℝ} {D : NSFormalization.Section3.T16.CutoffData}
    (h : NSFormalization.Section3.T17.CorrectionAPI ν place v r δ D) :
    ofSpec (toSpec h) = h := by cases h; rfl

/-! ## Exact checks against the landed U3--U6 theorem units -/

section ProvedFields

open NSFormalization.Paper1.CorrectionProfile (physicalCorrection)

variable {ν : ℝ} {v U : SpaceTimeField} {K : Set Space}
  {x₀ : Space} {r T δ : ℝ} {D : NSFormalization.Section3.T16.CutoffData}

theorem closes_correction_profile_smooth (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ContDiffOn ℝ ∞
        (NSFormalization.Section3.T17.rescaledCorrectionProfile v x₀ T ε D)
        (NSFormalization.Section3.T17.fixedProfileCylinder D) := by
  exact NSFormalization.Section3.T17.correction_profile_smooth hv x₀ T D hθ hη

theorem closes_correction_profile_support (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθsupp : tsupport D.θ ⊆ Metric.ball (0 : Space) D.θRadius)
    (hηsupp : tsupport D.η ⊆ Ioo (-2 : ℝ) 2) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      tsupport (NSFormalization.Section3.T17.rescaledCorrectionProfile v x₀ T ε D) ⊆
        NSFormalization.Section3.T17.fixedProfileCylinder D := by
  exact NSFormalization.Section3.T17.correction_profile_support
    hv x₀ T D hθ hη hθsupp hηsupp

def provedCorrectionProfileConst (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η) : ℕ → ℝ :=
  NSFormalization.Section3.T17.correctionProfileConst hv x₀ T hθ hη hθc hηc

theorem closes_correctionProfileConst_nonneg (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η) :
    ∀ k, 0 ≤ provedCorrectionProfileConst (x₀ := x₀) (T := T) hv hθ hη hθc hηc k := by
  exact NSFormalization.Section3.T17.correctionProfileConst_nonneg
    hv x₀ T hθ hη hθc hηc

theorem closes_correction_profile_uniform (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η)
    (hε₀ : D.ε₀ ≤ 1) :
    ∀ k : ℕ, ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ∀ z ∈ NSFormalization.Section3.T17.fixedProfileCylinder D,
        ‖iteratedFDeriv ℝ k
            (NSFormalization.Section3.T17.rescaledCorrectionProfile v x₀ T ε D) z‖ ≤
          provedCorrectionProfileConst (x₀ := x₀) (T := T) hv hθ hη hθc hηc k := by
  exact NSFormalization.Section3.T17.correction_profile_uniform
    hv x₀ T D hθ hη hθc hηc hε₀

theorem closes_correction_profile_identity (hv : ContDiff ℝ ∞ v)
    (hpot : NSFormalization.Section3.T16.LocalPotentialAPI v U K x₀ r T δ D) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ∀ z ∈ NSFormalization.Section3.T17.fixedProfileCylinder D,
        D.correction ε
            (NSFormalization.Section3.T17.correctionChartPoint x₀ T ε z) =
          NSFormalization.Section3.T17.rescaledCorrectionProfile v x₀ T ε D z := by
  exact NSFormalization.Section3.T17.correction_profile_identity hv hpot

theorem closes_force_profile_smooth (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ContDiffOn ℝ ∞
        (NSFormalization.Section3.T17.rescaledForceProfile ν v x₀ T ε D)
        (NSFormalization.Section3.T17.fixedProfileCylinder D) := by
  exact NSFormalization.Section3.T17.force_profile_smooth ν hv x₀ T D hθ hη

theorem closes_force_profile_support (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθsupp : tsupport D.θ ⊆ Metric.ball (0 : Space) D.θRadius)
    (hηsupp : tsupport D.η ⊆ Ioo (-2 : ℝ) 2) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      tsupport (NSFormalization.Section3.T17.rescaledForceProfile ν v x₀ T ε D) ⊆
        NSFormalization.Section3.T17.fixedProfileCylinder D := by
  exact NSFormalization.Section3.T17.force_profile_support
    ν hv x₀ T D hθ hη hθsupp hηsupp

def provedForceProfileConst (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η) : ℕ → ℝ :=
  NSFormalization.Section3.T17.forceProfileConst ν hv x₀ T hθ hη hθc hηc

theorem closes_forceProfileConst_nonneg (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η) :
    ∀ k, 0 ≤ provedForceProfileConst (ν := ν) (x₀ := x₀) (T := T)
      hv hθ hη hθc hηc k := by
  exact NSFormalization.Section3.T17.forceProfileConst_nonneg
    ν hv x₀ T hθ hη hθc hηc

theorem closes_force_profile_uniform (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η)
    (hθc : HasCompactSupport D.θ) (hηc : HasCompactSupport D.η)
    (hε₀ : D.ε₀ ≤ 1) :
    ∀ k : ℕ, ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ∀ z ∈ NSFormalization.Section3.T17.fixedProfileCylinder D,
        ‖iteratedFDeriv ℝ k
            (NSFormalization.Section3.T17.rescaledForceProfile ν v x₀ T ε D) z‖ ≤
          provedForceProfileConst (ν := ν) (x₀ := x₀) (T := T)
            hv hθ hη hθc hηc k := by
  exact NSFormalization.Section3.T17.force_profile_uniform
    ν hv x₀ T D hθ hη hθc hηc hε₀

/- The landed U4 theorem has the chart-force form; the U2 lattice transport is
   deliberately not reproved in this statement-restatement lane. -/
theorem closes_force_profile_identity_chart (hv : ContDiff ℝ ∞ v)
    (hθ : ContDiff ℝ ∞ D.θ) (hη : ContDiff ℝ ∞ D.η) :
    ∀ ε ∈ Ioc (0 : ℝ) D.ε₀,
      ∀ z ∈ NSFormalization.Section3.T17.fixedProfileCylinder D,
        NSFormalization.Source.correctionForce ν v
            (physicalCorrection v x₀ T D.θ D.η ε)
            (NSFormalization.Section3.T17.correctionChartPoint x₀ T ε z) =
          (ε ^ 2)⁻¹ •
            NSFormalization.Section3.T17.rescaledForceProfile ν v x₀ T ε D z := by
  exact NSFormalization.Section3.T17.physicalForce_eq_rescaledForceProfile
    ν hv x₀ T D hθ hη

theorem closes_correction_derivative_bound (hv : ContDiff ℝ ∞ v)
    ...
    ∀ j m : ℕ,
      ∀ ε ∈ Ioc (0 : ℝ)
        (NSFormalization.Section3.T17.correctionData v x₀ T θ η O θR ε₀).ε₀,
        ∀ z : SpaceTime, ∀ u : Fin m → Space, (∀ i, ‖u i‖ ≤ 1) →
          ‖iteratedFDeriv ℝ (j + m)
              ((NSFormalization.Section3.T17.correctionData
                v x₀ T θ η O θR ε₀).correction ε) z
              (Fin.append (fun _ : Fin j => ((1 : ℝ), (0 : Space)))
                (fun i => ((0 : ℝ), u i)))‖ ≤
            NSFormalization.Section3.T17.correctionDerivConst
                hv x₀ T hθ hη hθc hηc j m *
              (ε⁻¹) ^ (2 * j + m) := by
  exact NSFormalization.Section3.T17.correction_derivative_bound
    hv x₀ T O θR ε₀ r hθ hη hθc hηc hθsupp hηsupp hr2 hε₀ hεspace

theorem closes_force_derivative_bound (hv : ContDiff ℝ ∞ v)
    (hvper : NSFormalization.Section3.T10.IsPeriodicOn univ v)
    {θ : Space → ℝ} {η : ℝ → ℝ}
    (O : Set Space) (θR ε₀ r δ : ℝ)
    (hθ : ContDiff ℝ ∞ θ) (hη : ContDiff ℝ ∞ η)
    (hθc : HasCompactSupport θ) (hηc : HasCompactSupport η)
    (hθsupp : tsupport θ ⊆ Metric.ball (0 : Space) θR)
    (hηsupp : tsupport η ⊆ Ioo (-2 : ℝ) 2)
    ...
    ∀ m : ℕ,
      ∀ ε ∈ Ioc (0 : ℝ)
        ...
        ∀ z : SpaceTime, ∀ u : Fin m → Space, (∀ i, ‖u i‖ ≤ 1) →
          ‖iteratedFDeriv ℝ m
              (NSFormalization.Section3.T17.correctionForce ν v
                (NSFormalization.Section3.T17.correctionData
                  v x₀ T θ η O θR ε₀) ε) z
              (fun i => ((0 : ℝ), u i))‖ ≤
            NSFormalization.Section3.T17.forceDerivConst
                ν hv x₀ T hθ hη hθc hηc m *
              (ε⁻¹) ^ (2 + m) := by
  exact NSFormalization.Section3.T17.force_derivative_bound
    ν hv x₀ T δ r hvper O θR ε₀ hθ hη hθc hηc hθsupp hηsupp
      hr2 hε₀ hεtime hεspace

end ProvedFields

end BlowupDensity.T17.CorrectionCanonicalProbe
