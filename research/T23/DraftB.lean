...
/-!
# T23 draft B specification: Corollary `cor:boundary` (interior no-slip insertion)

Statement-only draft (rule 2 of `CLAUDE.md`) for
...
* **Imported registered contracts** (used by name): `Contracts.V1.Data`
  ...
  registered vocabulary namespace is available).  The whole-space rescaled
  packet is the registered `scaledPacket`/`scaledPressure`/`scaledForce`, so the
  un-periodized T15.Draft `scaledVelocity`/`scaledPressure`/`scaledForce`
  wrappers (which are `rfl`-equal to those, `research/T18/Spec.lean:344-361`) are
  not copied.
* **Copied verbatim** (T13 copy policy — same names, namespaces, provenance
  ...
  `fundamentalCube` (T13.Spec), `PlacementData` (T15.Draft), `CutoffData` and
  `correctedBackground` (T16.Draft), `correctionForce` (T17.Spec); from
  `research/T22/Spec.lean` the reconciled bounded-domain norm layer
  `DomainTest`, `DomainFunctional`, `restrictDatum`, `domainSobolevENorm`,
...
1. **No periodization.**  The domain uses the single whole-space rescaled packet
   ...
   periodization layer) is therefore not threaded; `prop:scaling` enters as the
   whole-space packet energy/force rates, whose constants appear as fields.
2. **Domain norms.**  Energy in `L²(Ω)` and force in `L¹(0,∞;H^s(Ω))` use the
   ...
   norms; `BoundedDomainNormAPI` (T22) is threaded as the norm-layer input.
...
-- copied verbatim from research/T18/Spec.lean:52-66 (T13.Spec)
namespace BlowupDensity.T13.Spec

open BlowupDensity.Contracts.V1

/-- `03-torus.tex:22-23`: the closed fundamental cube of the unit-torus lift,
...
-- copied verbatim from research/T18/Spec.lean:454-548 (T15.Draft)
...
/-! # T23 draft B: the bounded-domain layer and `cor:boundary` -/

namespace BlowupDensity.T23.DraftB
...
/-- `03-torus.tex:632-633`: `Ω ⊂ R³` is a bounded box or bounded smooth domain.
Only the open / bounded / nonempty / measurable structure common to both cases,
and used by the corollary's statement, is encoded.  The box-vs-smooth-boundary
distinction governs only the *assumed* existence of the compatible reference
(elliptic regularity), which the corollary takes as a hypothesis (`:648`), so it
is not separately encoded; see `research/T23/COMPARISON_B.md`.

Non-vacuity: a genuine conjunction of four honest topological/measure
predicates on `Ω`; it is neither `True` nor an unfolding. -/
def BoundedDomain (Ω : Set Space) : Prop :=
  IsOpen Ω ∧ Bornology.IsBounded Ω ∧ Ω.Nonempty ∧ MeasurableSet Ω
...
structure BoundaryInsertionAPI (ν : ℝ) (P : PacketImportAPI ν)
  ...
  Quantifier order: none.  Non-vacuity: `BoundedDomain` is a real conjunction of
  open/bounded/nonempty/measurable. -/
  domain : BoundedDomain Ω
...
`ν > 0`, packet `P`, placement `place`, bounded domain `Ω`, bounded-domain norm
layer `norms`, initial datum `a`, reference force `g`, radii `r`, margin `δ`,
cutoff data `D`, and bounded-domain no-slip reference regular through `T+δ`, with
`Ω` a bounded domain, `δ > 0`, `g ∈ 𝓕(Ω)`, `a ∈ 𝓧(Ω)`, and the chart ball's
closure strictly inside `Ω`, there is an inserted family: `Nonempty` of the API,
which carries `ε₀` as a field.  Introducing this definition asserts nothing. -/
def boundaryInsertionStatement : Prop :=
  ∀ (ν : ℝ), 0 < ν → ∀ (P : PacketImportAPI ν)
    ...
    BoundedDomain Ω → 0 < δ → g ∈ forceClassOmega Ω → a ∈ initialClassOmega Ω →
      closure (Metric.ball place.chartCenter place.chartRadius) ⊆ Ω →
        Nonempty
          (BoundaryInsertionAPI ν P place Ω norms a g r δ D reference)

end BlowupDensity.T23.DraftB
