...
namespace BoundaryCanonicalProbe
open Set MeasureTheory Filter Topology
...
def correctionTo (b : CorrectionAPI ν P.toPacketAPI) : NSFormalization.Section3.T23.WholeSpaceCorrectionAPI ν P.velocity P.carrier where
  T := b.T
  time_pos := b.time_pos
  δ := b.δ
...
def correctionFrom (b : NSFormalization.Section3.T23.WholeSpaceCorrectionAPI ν P.velocity P.carrier) : CorrectionAPI ν P.toPacketAPI where
  T := b.T
  time_pos := b.time_pos
  δ := b.δ
  margin_pos := b.margin_pos
...
example (b : DomainPlacementData P.toPacketAPI) : placeFrom (placeTo b) = b := rfl
example (b : DomainPlacementData P.toPacketAPI) : placeTo (placeFrom (placeTo b)) = placeTo b := rfl
example (b : CutoffData) : cutoffFrom (cutoffTo b) = b := rfl
example (b : CutoffData) : cutoffTo (cutoffFrom (cutoffTo b)) = cutoffTo b := rfl
example (b : ClassicalSolutionOmega ν Ω a g T) : solutionFrom (solutionTo b) = b := rfl
example (b : ClassicalSolutionOmega ν Ω a g T) : solutionTo (solutionFrom (solutionTo b)) = solutionTo b := rfl
example (b : CorrectionAPI ν P.toPacketAPI) : correctionFrom (correctionTo b) = b := rfl
example (b : CorrectionAPI ν P.toPacketAPI) : correctionTo (correctionFrom (correctionTo b)) = correctionTo b := rfl

-- Reverse round trips on arbitrary canonical records, including all data fields.
example (b : NSFormalization.Section3.T23.DomainPlacementData P.velocity P.pressure P.force P.carrier) : placeTo (placeFrom (P := P) b) = b := rfl
example (b : NSFormalization.Section3.T23.CutoffData) : cutoffTo (cutoffFrom b) = b := rfl
example (b : NSFormalization.Section3.T23.ClassicalSolutionOmega ν Ω a g T) : solutionTo (solutionFrom b) = b := rfl
example (b : NSFormalization.Section3.T23.WholeSpaceCorrectionAPI ν P.velocity P.carrier) : correctionTo (correctionFrom (P := P) b) = b := rfl
...
-- Each requested auxiliary geometry lemma has a one-line consumer.
example (c : Space) (R : ℝ) : IsCompact (Metric.closedBall c R) := NSFormalization.Section3.T23.prescribed_closedBall_compact c R
example {c x : Space} {R : ℝ} (hx : x ∈ Metric.ball c R) : ∃ r : ℝ, 0 < r ∧ Metric.closedBall x r ⊆ Metric.ball c R := NSFormalization.Section3.T23.exists_inner_closedBall hx
example (hΩ : IsOpen Ω) {c : Space} {R : ℝ} (h : Metric.closedBall c R ⊆ Ω) : Disjoint (Metric.closedBall c R) (frontier Ω) := NSFormalization.Section3.T23.prescribed_closedBall_disjoint_frontier hΩ h
end BoundaryCanonicalProbe
...
example : boundaryInsertionStatement = (∀ (ν : ℝ), 0 < ν → ∀ (u : VelocityField) (p : PressureField) (f : VelocityField)
    (K : Set Space) (M E : ℝ)
    ...
    IsBoundedBoxOrSmoothDomain Ω → 0 < δ → g ∈ forceClassOmega Ω →
      ...
      closure (Metric.ball place.chartCenter place.chartRadius) ⊆ Ω →
        Nonempty
          (BoundaryInsertionAPI ν u p f K M E place Ω norms a g r δ D reference)) := rfl
...
example : boundaryInsertionStatement' = (∀ (ν : ℝ), 0 < ν → ∀ (u : VelocityField) (p : PressureField) (f : VelocityField)
    ...
    IsBoundedBoxOrSmoothDomain Ω → 0 < δ → 0 < r →
      ...
      ∃ (C : WholeSpaceCorrectionAPI ν u K) (D : CutoffData),
        ...
        D.θRadius = C.θRadius ∧ D.ε₀ = C.ε₀ ∧
        D.potential = C.potential ∧ D.correction = C.correction ∧
        Nonempty (BoundaryInsertionAPI ν u p f K M E place Ω norms a g r δ D reference)) := rfl
...
open BlowupDensity.Contracts.V1 BlowupDensity.Contracts.V1.Data
open BlowupDensity.T23.Spec BlowupDensity.T16.Draft BlowupDensity.T22.Draft
-- The repaired conclusion chooses exactly the same supplier and all seven cutoff fields.
example {ν : ℝ} {P : PacketImportAPI ν} {place : DomainPlacementData P.toPacketAPI}
    {Ω : Set Space} {norms : BoundedDomainNormAPI} {a : SpatialField} {g : SpaceTimeField}
    {r δ : ℝ} {reference : ClassicalSolutionOmega ν Ω a g (place.T + δ)} :
    (∃ (C : CorrectionAPI ν P.toPacketAPI) (D : CutoffData),
        C.T = place.T ∧ C.δ = δ ∧ C.x₀ = place.x₀ ∧ C.r = r ∧
        (∀ t ∈ Ioo (0 : ℝ) (place.T + δ), ∀ x ∈ Metric.ball place.x₀ r,
          C.v (t, x) = reference.velocity (t, x)) ∧
        ...
        D.potential = C.potential ∧ D.correction = C.correction ∧
        Nonempty (BoundaryInsertionAPI ν P place Ω norms a g r δ D reference)) ↔
    (∃ (C : NSFormalization.Section3.T23.WholeSpaceCorrectionAPI ν P.velocity P.carrier) (D : NSFormalization.Section3.T23.CutoffData),
        C.T = place.T ∧ C.δ = δ ∧ C.x₀ = place.x₀ ∧ C.r = r ∧
        (∀ t ∈ Ioo (0 : ℝ) (place.T + δ), ∀ x ∈ Metric.ball place.x₀ r,
          C.v (t, x) = (solutionTo reference).velocity (t, x)) ∧
        D.θ = C.θ ∧ D.η = C.η ∧ D.plateau = C.plateau ∧
        D.θRadius = C.θRadius ∧ D.ε₀ = C.ε₀ ∧
        D.potential = C.potential ∧ D.correction = C.correction ∧
        Nonempty (NSFormalization.Section3.T23.BoundaryInsertionAPI ν P.velocity P.pressure P.force P.carrier P.energyBound P.dissipationBound (placeTo place) Ω (normsTo norms) a g r δ D (solutionTo reference))) := by
  constructor
  · rintro ⟨C, D, hT, hd, hx, hr, hv, hθ, hη, hO, hR, hε, hA, hw, ⟨B⟩⟩
    exact ⟨correctionTo C, cutoffTo D, hT, hd, hx, hr, hv, hθ, hη, hO, hR, hε, hA, hw,
      ⟨apiTo B⟩⟩
  · rintro ⟨C, D, hT, hd, hx, hr, hv, hθ, hη, hO, hR, hε, hA, hw, ⟨B⟩⟩
    exact ⟨correctionFrom C, cutoffFrom D, hT, hd, hx, hr, hv, hθ, hη, hO, hR, hε, hA, hw,
      ⟨apiFrom (norms := norms) (D := cutoffFrom D) B⟩⟩
end BoundaryCanonicalProbe
