...
/-! Existing U2 local-construction checks, extended below with registered suppliers. -/
namespace T23U2bPriorChecks
...
namespace T23U2b.FieldChecks
open Set
open NavierStokes NavierStokes.ProblemStatement
open NSFormalization.Section3.T23
open scoped ContDiff
variable {ν : ℝ} {u v : VelocityField} {K : Set Space}
  (C : WholeSpaceCorrectionAPI ν u K) (hK : IsCompact K)
  (hv : ContDiffOn ℝ ∞ v (Ioo (0 : ℝ) (C.T + C.δ) ×ˢ Metric.ball C.x₀ C.r))
  (hd : ∀ t ∈ Ioo (0 : ℝ) (C.T + C.δ), ∀ x ∈ Metric.ball C.x₀ C.r,
    spatialDivergence v t x = 0)
  (hu : ∀ t ∈ Ioo (0 : ℝ) 1, tsupport (fun x => u (t, x)) ⊆ K)
  {e : ℝ} (he : e ≤ C.ε₀)

include hK hv hd hu he

-- Literal Boundary.lean U2 expressions, at the threaded cutoff.
theorem canonical_background_advects_packet :
  ...
  intro D ε hε t ht x
  exact (C.local_crossTransport hK hv hd hu he ε hε t ht x).1

theorem canonical_packet_advects_background :
    let D := localCorrectionData v C.x₀ C.T C.θ C.η C.plateau C.θRadius e
    ∀ ε ∈ Ioc (0 : ℝ) e, ∀ t ∈ Ico (0 : ℝ) C.T, ∀ x : Space,
  ...
  intro D ε hε t ht x
  exact (C.local_crossTransport hK hv hd hu he ε hε t ht x).2

-- Literal Spec.lean U2 expressions: the registered scaledPacket and copied
-- correctedBackground must be definitionally the very fields proved above.
theorem spec_background_advects_packet :
    let D := localCorrectionData v C.x₀ C.T C.θ C.η C.plateau C.θRadius e
    ∀ ε ∈ Ioc (0 : ℝ) e, ∀ t ∈ Ico (0 : ℝ) C.T, ∀ x : Space,
      spatialDerivative (BlowupDensity.Contracts.V1.scaledPacket u C.x₀ C.T ε) t x
        (BlowupDensity.T16.Draft.correctedBackground v D.correction ε (t, x)) = 0 := by
  exact canonical_background_advects_packet C hK hv hd hu he

theorem spec_packet_advects_background :
    let D := localCorrectionData v C.x₀ C.T C.θ C.η C.plateau C.θRadius e
    ∀ ε ∈ Ioc (0 : ℝ) e, ∀ t ∈ Ico (0 : ℝ) C.T, ∀ x : Space,
      spatialDerivative (BlowupDensity.T16.Draft.correctedBackground v D.correction ε) t x
