...
| **D-b-transport** | **the only genuinely new work:** transport `db_cycles_full` from the cycles variable to the raw *angular* variable — conclude `∀ i j, MemLp (ξ ↦ (ξ j)·(A i)(ξ)) 2 volume` | **M** (~50 lines) | template `transverse_of_divergence_free`/`transverse_symm_of_divergence_free` (`Transverse.lean:68-205`, same expression `ξⱼ·(A i)(ξ)` in the angular variable); `angularFrequencyDilation_coeFn` (`AngularFourierDilation.lean:248`); `angularWeightEquiv_coeFn` (`AngularSobolevCoordinates.lean:284`); `Measure.map_addHaar_smul` | **gap (M)** — the angular-convention transport of the in-tree raw multiplier is not yet written; `Transverse.lean:177-205` is the worked template.  Feeds `raisableWitness_of_memLp_smul` (D-d2) |
| **D-euler-pairing** | Euler side owes the **Schwartz-pairing form** of its weak derivatives: `∫ψ·(∂ⱼz)_i = ∫(-∂ⱼψ)·z_i` for each descended `L²` word | S–M | C1b-m-E of `C1B_SPLIT.md` (`word_hasDerivAt` gives *strong `L²` translation derivatives*, not the Schwartz pairing directly) | **gap** — small obligation currently recorded nowhere; also add to `C1B_SPLIT.md` row C1b-m-E |
| **D-euler-coord** | Euler side supplies each `(ξ j)·(A i) ∈ L²` from the descended `L²` derivative words | M | D-euler-pairing + D-b-transport (feed `C j` = datum of the word into `db_cycles_full`, transport, get coordinate `L²`) | **gap** — awaits D-b-transport + D-euler-pairing |
