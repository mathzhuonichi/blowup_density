...
| **D-quant** | **the quantitative constructor (lane 145):** order-0 bound `‖orderZeroDatum hz‖ ≤ ‖z‖_{L²}` (`c₀ = 1`); raising bound `‖A'‖² ≤ 4·(‖A‖² + ∑ⱼ‖C j‖²)`; and `‖A‖² ≤ 16^m·M` for the order-`m` datum of `z` under the tracked-`L²` hypothesis `HasWeakDerivsL2Bound z M m` (`m = 2` instance: `‖A‖² ≤ 256·M`) | **M** | **D1**: the order-0 realization pipeline is an isometry chain — `cyclesToAngularRealVector_norm_le` at `s=0` (`AngularRealVectorBochner.lean:24`, `frequencyUnit^{|0|}=1`), `Lp.norm_fourier_eq` (Plancherel), `realProjectionTo_norm_le` (`RealPositiveDensity.lean:41`), plus the Euclidean-valued `L²` Pythagoras `‖z‖²_{L²} = ∑ᵢ ‖z·ᵢ‖²_{L²}` **proved here** (`OrderZeroDatum.lean:40-53` flagged it absent). **D2**: symbol bound `norm_raiseIntegrand_le` + the a.e. identity `(2πi)·(ξⱼ·(A i)) =ᵐ frequencyUnit·(C j i)` (`coord_smul_deriv_ae`, re-derived from `db_cycles_full`, giving `eLpNorm(ξⱼ·A i)=eLpNorm(C j i)` since `frequencyUnit=2π=‖2πi‖`), four-term Cauchy–Schwarz. **D3**: induction (D1 base, D2 step, `×16` per order) + `isSobolevDatum_unique` to transfer to any order-`m` datum | **DONE (lane 145)** — `FiniteOrderNorm.lean`: `norm_orderZeroDatum_le`, `norm_raise_le`, `exists_isSobolevDatum_norm_le`, `norm_isSobolevDatum_le_of_memLp_derivs`, `norm_isSobolevDatum_le_two`; non-vacuity `exists_hasWeakDerivsL2Bound_smooth` (every `SmoothL2Field` has a uniform bound to every order). Supplies the vector-norm bound `OrderZeroDatum.lean:40-53` / row C1b-c8-m flagged absent; the `m=2` instance feeds A01's `Kbnd`. `#print axioms` = standard (`research/D01/axioms_finite_order_norm.lean`) |
...
* **Where the vector order-`m` Plancherel isometry would be needed, and why the constructor avoids
  ...
  **Lane 145 update (row D-quant): the norm bound is now proved separately** in
  `Section4/D01/FiniteOrderNorm.lean`, *without* the full vector Plancherel isometry — the order-0
  step uses the isometry chain `cyclesToAngularRealVector` (`s=0`) ∘ `𝓕` ∘ `realProjectionTo` plus
  the Euclidean `L²` Pythagoras identity (proved there), and higher orders are controlled by the
  raising bound (`ξⱼ`-multiple = derivative datum in `L²`, via `coord_smul_deriv_ae`) rather than a
  Plancherel identity at order `m`.  So the "absent isometry" of `OrderZeroDatum.lean:40-53` is not
  needed for the datum-norm bound after all; the constant grows as `16^m` (`256` at `m=2`).
