...
| **D-euler-coord** | Euler side supplies each `(ξ j)·(A i) ∈ L²` from the descended `L²` derivative words | M | D-euler-pairing + D-b-transport (feed `C j` = datum of the word into `db_cycles_full`, transport, get coordinate `L²`) | **D01 half DONE (lane 132)** — `memLp_coord_smul_datum` *is* the "feed `C j` into `db_cycles_full`, transport, get coordinate `L²`" step, packaged to take the family `C j` + Schwartz pairings.  Awaits only D-euler-pairing to supply that family. |
