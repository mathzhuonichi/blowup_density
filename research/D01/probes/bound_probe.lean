...
-- norm^2 of a Euclidean vector as sum of squares of coords
example (ξ : Space) : ‖ξ‖ ^ 2 = ∑ j : Fin 3, (ξ j) ^ 2 := by
  rw [EuclideanSpace.norm_eq]
  rw [Real.sq_sqrt (by positivity)]
  congr 1; funext j; rw [Real.norm_eq_abs, sq_abs]

-- sum of squares ≤ (sum of abs)^2
example (ξ : Space) : ∑ j : Fin 3, (ξ j) ^ 2 ≤ (∑ j : Fin 3, |ξ j|) ^ 2 := by
  rw [Fin.sum_univ_three, Fin.sum_univ_three]
  nlinarith [abs_nonneg (ξ 0), abs_nonneg (ξ 1), abs_nonneg (ξ 2),
    sq_abs (ξ 0), sq_abs (ξ 1), sq_abs (ξ 2),
    mul_nonneg (abs_nonneg (ξ 0)) (abs_nonneg (ξ 1)),
...
theorem norm_sobolevBesselWeight_one (ξ : Space) :
  ...
  rw [sobolevBesselWeight, Complex.norm_real, Real.norm_eq_abs,
    abs_of_nonneg (by positivity), Real.rpow_natCast]  -- probe: will adjust
  sorry
