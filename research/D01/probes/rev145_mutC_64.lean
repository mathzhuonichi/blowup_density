-- REVIEW PROBE (lane 145, mutation C): the m=2 constant 256 replaced by 64.
-- Expected: norm_isSobolevDatum_le_two fails.  Read-only copy; not in the build.
...
(`c₂ = 64`) that A01's order-2 cap `Kbnd` consumes. -/
...
theorem exists_isSobolevDatum_norm_le : ∀ (m : ℕ) (z : Space → Space) (M : ℝ),
    HasWeakDerivsL2Bound z M m →
      ∃ A : RealVectorSobolev (m : ℝ), IsSobolevDatum (m : ℝ) z A ∧ ‖A‖ ^ 2 ≤ (16 : ℝ) ^ m * M
  | 0, z, M, h => by
  ...
  | (m + 1), z, M, h => by
      ...
      have hCex : ∀ j, ∃ Cj : RealVectorSobolev (m : ℝ),
          IsSobolevDatum (m : ℝ) (w j) Cj ∧ ‖Cj‖ ^ 2 ≤ (16 : ℝ) ^ m * M :=
      ...
      calc ‖(WithLp.toLp 2 (fun i => raiseHilbert (A i) (hg i)) : RealVectorSobolev ((m:ℝ)+1))‖ ^ 2
        ...
        _ ≤ 4 * ((16 : ℝ) ^ m * M + ∑ _j : Fin 3, (16 : ℝ) ^ m * M) := by
...
theorem norm_isSobolevDatum_le_of_memLp_derivs (m : ℕ) (z : Space → Space) (M : ℝ)
    ...
    (hA : IsSobolevDatum (m : ℝ) z A) : ‖A‖ ^ 2 ≤ (16 : ℝ) ^ m * M := by
...
theorem norm_isSobolevDatum_le_two (z : Space → Space) (M : ℝ)
    ...
    (hA : IsSobolevDatum ((2 : ℕ) : ℝ) z A) : ‖A‖ ^ 2 ≤ 64 * M := by
  have hb := norm_isSobolevDatum_le_of_memLp_derivs 2 z M h A hA
  rwa [show (16 : ℝ) ^ (2 : ℕ) = 64 from by norm_num] at hb
