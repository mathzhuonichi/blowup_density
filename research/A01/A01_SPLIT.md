...
| A2b | order-`m` continuation (bounded trajectory extends) **+ cross-order agreement** | **OpenAI/local route (recommended): `EulerBoundedMildContinuation.exists_global_mild_of_bound` (`vendor/.../Euler/BoundedMildContinuation.lean:39`)** on exactly `exists_local`'s carrier + `Coefficients` (composability probe compiles, lane 122 `research/A01/probes/a2b_continuation_probe.lean`); div-free via `CorrectionContinuation.lean:17`, angle-inv via `ForcedCylinderInvariant.lean:30`. HeliCorgi order-3 fallback `FormalPatched/R3MildContinuation` `:93`/`:143` **not needed** | **M** (was L) | A2, F1, A02.uniqueness | **not blocked on C1c** — corrected lane 122 (F5): the OpenAI/local layer *does* have forced-path continuation from an a-priori bound; the revision-1 "Euler/local layer has neither" was a false negative. See `A3_SPLIT.md` rows A2b-a′/A2b-a and §3b |
