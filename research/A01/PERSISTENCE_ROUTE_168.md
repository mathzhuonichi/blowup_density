...
| Consistency of nonlinear spatial maps | `SobolevNonlinearCompatibility.restrict_asymmetricTransport`; `CorrectionSourceRestriction.truncate_transport`; `SmoothFieldSobolevTime.restrict_sobolev` | Specialize to the actual forced source P(f−advection), including the W from lane 166. |
| Higher nonlinear source in time L2 | `TimeSobolevTransport.transportTime`, `transportTime_ae`; `TimeCorrectionSource.restrict_transport_state` | Wire the actual velocity, force and Leray map, not a generic correction-data assumption. This is this lane's selected bridge. |
...
| Continuous higher-order persistence on the low-order interval | `SobolevViscousEnergy.finite_sobolev_viscous_energy` is genuinely finite-Sobolev, not a classical-solution theorem | Supply finite-word time derivatives, divergence and pressure constraints, differentiated equation and controlled commutator forcing; integrate estimates and obtain the high-order trace/continuity. |
...
| Maximal continuation from finite H2 time integral | `A01/Horizon.localTheory_on_prescribed_horizon` and vendor bounded continuation | Their a priori bound input must be proved for finite-order mild solutions without assuming ClassicalSolutionR. |
...
Construct the actual projected source `P(f−advection(u,W))` in TimeLp H(q+1), and prove its restriction equals the original Hq nonlinear source a.e. on exactly the lane-166 horizon. Retain the true physical force slice identification in a consumer. This supplies the missing high-order source term for a subsequent finite-mild energy/trace argument, rather than inventing a continuous high-order path.
...
No Lean or lake command was run during this source audit. The table records actual supplier hypotheses, not completed obligations.
