...
| **C1b-rep** | **the B1 hand-off:** `velocity t =ᵐ[volume] ⇑(U t)` for `t ∈ Ico 0 T`, and `velocity (0,·) = a.field` pointwise | **L** | `ClassicalSolutionR.velocity` (`Contracts/V1/Data.lean:624`), C1b-cong | (produced by B1; **consumed** here) | **owned by B1** — given it, every datum row transfers to `velocity` by `IsSobolevDatum.congr_field`.  This is the *pointwise* half of c5 and the reason no datum row discharges c5 by itself.  **Transport machinery ready (lane 140):** `Section4/A01/EulerPairing.lean:exists_isSobolevDatum_m_of_ae` moves the order-`m` datum of `⇑U` onto any `v =ᵐ ⇑U` — one `exact` once B1 supplies the a.e. identity |
| **C1b-m-E** | Euler side (`t>0`, no smoothness): each derivative word descends to an ordinary `L²` field — `{q n} (hq : n+3 ≤ q) (u : SobolevSpace 1 q) (hinv) (w : Fin n → Fin 4) → ∃ z, ordinaryLift z = word 1 u _ w` | M | abstract array `u t : SobolevSpace 1 (q+1)` + `word`/`word_hasDerivAt`/`word_has_jet`/`ofJet` (`Euler/CylinderSobolevSpace.lean:66,70,91,78`), `exists_ordinary_value` (`Source/OrdinaryCylinderDescent.lean:29`), clause 7 angle invariance (`Source/OrdinaryForcedLocal.lean:47`) | real analysis (Euler) | **DONE (lane 140)** — `Section4/A01/EulerPairing.lean`: `exists_descend` (word ↦ ordinary `L²` via `exists_ordinary_value`; angle-invariance of `ofJet` collapses to the word by `value_injective`, so **no `jet_word_eq`** needed), `hasWeakDerivsL2_of_word` (order induction), `hasWeakDerivsL2_of_cylinder` (from `exists_local`'s `u`,`U`).  `sobolevTranslation` **is** coordinatewise (rfl, reviewer §6).  The strong→Schwartz-pairing step is `weakDeriv_pairing_of_translation_hasDerivAt` (row D-euler-pairing); the lift→mean pullback is `hasDerivAt_meanTranslation_of_lift` (via `ordinaryLift_translation` + `LinearIsometry.adjoint_comp_self`) |
| **C1b-m-D** | D01 side — **the missing finite-order constructor:** `{m} (hz : MemLp z 2 volume) (L² derivs of z up to order m) → ∃ A : RealVectorSobolev m, IsSobolevDatum m z A` | **M–L** | parallel to `orderZeroDatum` (`D01/OrderZeroDatum.lean:96`); `isSobolevDatum_partialDeriv` (`D01/DerivativeDatum.lean:245`) is `SmoothL2Field`-only, so does not apply; `memLp_of_isSobolevDatum` (`D01/DatumToJets.lean:267`) | **real analysis (the actual blocker)** | **gap** — D01 jumps from order 0 (`orderZeroDatum`, bare `MemLp`) to all orders (`smoothAngularDatum`, `SmoothL2Field`) with nothing in between; the one new analytic input is the homogeneous↔inhomogeneous comparison `(1+‖ξ‖²)^m ≤ c_m(1+∑_{|α|=m}|ξ^α|²)` that lands the weighted transform in `L²`.  **Split + order-raising step done (lane 125): see `research/D01/FINITE_ORDER_SPLIT.md`; `Section4/D01/FiniteOrderDatum.lean:isSobolevDatum_raise` raises order `s→s+1` given the `L²` witness; residual blocker = row D-b (raw-frequency `𝓕(∂ⱼz)=iξⱼ𝓕z`). **Lane 132: CLOSED on the D01 side** — `Section4/D01/FiniteOrderConstructor.lean:exists_isSobolevDatum_of_memLp_derivs` takes `HasWeakDerivsL2 z m` (`MemLp z 2` + iterated `L²` weak-derivative Schwartz pairings up to order `m`) to `∃ A : RealVectorSobolev m, IsSobolevDatum m z A`; the one new lemma is `memLp_coord_smul_datum` (row D-b-transport). The only remaining external obligation was row **D-euler-pairing** = the Euler side's Schwartz-pairing form of C1b-m-E's `word_hasDerivAt`; **now CLOSED (lane 140)**, `Section4/A01/EulerPairing.lean:weakDeriv_pairing_of_translation_hasDerivAt`.  **C1b-m-D fully closed:** `exists_isSobolevDatum_m_ordinaryL2` (from `HasWeakDerivsL2 (⇑U) m`) and `exists_isSobolevDatum_m_of_cylinder` (directly from the `exists_local` cylinder solution, `m ≤ q−2`).** |
