...
def rev207_controlConstant (q : ℕ) : ℝ :=
...
theorem rev207_controlTame {q : ℕ} (hq : 6 ≤ q) :
    SmoothCylinderCoordinateTame q hq (rev207_controlConstant q) := by
  ...
  simpa only [Finset.sum_const, Finset.card_univ, nsmul_eq_mul,
    rev207_controlConstant, mul_assoc] using hsum
...
theorem rev207_controlCommutator_ae {q : ℕ} (hq : 6 ≤ q)
