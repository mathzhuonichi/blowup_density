...
example {ν : ℝ} {a : A02.SpatialField} {f : A02.SpaceTimeField}
    (hν : 0 < ν) (ha : a ∈ A02.initialClassR) (hf : D01.MemForceR f) :
    ManuscriptLocalRegularity ν a f (localHorizon ν a f) (localSolution ν a f hν ha hf) := by
  apply manuscriptLocalRegularity_of_pipeline (localSolution ν a f hν ha hf) hf
