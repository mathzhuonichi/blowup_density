...
-- Deliberately ask the mild theorem for the required full consumer conclusion.
example {q : ℕ} (hq : 6 ≤ q) {ν S R : ℝ}
    (hν : 0 < ν) (hS : 0 < S) (hR : 0 ≤ R)
    (a : SmoothL2Field Space) (_ha : ∀ x, EulerSmoothLimit.divergence a.field x = 0)
  ...
  exact forced_global_mild_of_boundInv hq hν hS hR a F hF hu₀ hbound
