...
| A3-M2 · forcing bound | quantitative limiting word forcing | tame tensor/word identification | OPEN, lane 199 | `ForcingFamilyBound` | Exact full-gradient pairing target in `MildEnergyPremises.lean`; no general-data constants proved. |
| A3-M2 · envelope | everywhere-interior differentiable squared-energy majorant | integrated estimate plus tame pairing, with dissipation recovery | OPEN, lane 199 | `EnvelopeConversion` | Vendor root estimate has dropped dissipation; this is a substantive obligation, not just differentiating an integral. |
