...
| A3-M2 · forcing bound | quantitative limiting word forcing | exact Z identification, source/pressure cancellation, force bound and conditional assembly (lane 200) | OPEN: one finite spatial commutator estimate | `CylinderCommutatorBound q hq` → `ForcingFamilyBound` | `E=mildNormConstant`; explicit candidate `A=E*(1+outerTameConst(q+1))` remains conditional. See `REPORT_200.md`, `ATTEMPTS_FORCING_BOUND.md`. |
