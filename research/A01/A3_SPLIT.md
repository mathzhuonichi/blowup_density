...
| **A3-U common horizon** | one ordinary `U` on `[0,S]` realizes canonical mild solutions at every `q ≥ 6` | M | per-order bounds + `hfs` | **Uniqueness input discharged (188)**: `mildUniqueness : MildUniqueness` on the entire `[0,S]`; all three primed common-carrier exports are unconditional in that input. `CommonHorizon.lean`, `MildUniqueness.lean`, `ATTEMPTS_MILD_UNIQUENESS.md` |
