...
/-! Literal canonical-field conformance and a nonzero compact packet instance. -/

noncomputable section

namespace NSFormalization.Section3.T15.ConvergenceTwoProbe
...
/-! ## Literal ScalingAPI.forceConvergence field conformance -/
...
example {u f : VelocityField} {p : PressureField} {K : Set Space}
  ...
  exact forceConvergence hf hc place
...
theorem nonzero_packet_converges :
    force (1 / 2, 0) ≠ 0 ∧
      ∀ q : ℝ≥0∞, (q = 1 ∨ q = 2) → ∀ s : ℝ, s < criticalOrder q.toReal →
        Tendsto
          (fun ε : ℝ ↦ forceSobolevENormT q s
  ...
  · exact forceConvergence force_contDiff force_compactPositiveTime placement

end NSFormalization.Section3.T15.ConvergenceTwoProbe
