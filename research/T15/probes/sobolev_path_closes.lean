...
namespace NSFormalization.Section3.T15.SobolevProbe
...
theorem emPres_extension_smooth :
    ContDiffOn ℝ ∞ (zeroPastField emPres) (Iio (1 : ℝ) ×ˢ (univ : Set Space)) := by
  have heq : zeroPastField emPres = emPres := by
    funext z
    by_cases h : 0 < z.1
    · exact zeroPastField_of_pos emPres h z.2
    · rw [zeroPastField_of_nonpos emPres (not_lt.1 h)]
      simp [emPres, emTimeBump_eq_zero (not_lt.1 h)]
  rw [heq]
  exact ((emTimeBump.contDiff.comp contDiff_fst).mul
    (emSpaceBump.contDiff.comp contDiff_snd)).contDiffOn

theorem emPres_support : ∀ t ∈ Ico (0 : ℝ) 1,
    tsupport (fun x : Space => emPres (t, x)) ⊆ emCarrier := by
  intro t _
  have hs : Function.support (fun x => emPres (t, x)) ⊆ Function.support (⇑emSpaceBump) := by
    intro x hx hzero
    exact hx (by simp [emPres, hzero])
  have h := closure_mono hs
  change tsupport (fun x => emPres (t, x)) ⊆ tsupport (⇑emSpaceBump) at h
  rwa [emSpaceBump.tsupport_eq] at h

theorem sobolev_path_closes :
    (∀ m : ℕ, ∃ G : ℝ → PeriodicSobolev (m : ℝ),
      ContinuousOn G (Ico (0 : ℝ) emPlacement.T) ∧
        ∀ t ∈ Ico (0 : ℝ) emPlacement.T,
          IsPeriodicDatum (m : ℝ)
            (fun x => periodizedScaledVelocity emVel emPlacement.x₀ emPlacement.T (1/2) (t,x)) (G t)) ∧
    (∀ t ∈ Ico (0 : ℝ) emPlacement.T,
      MemLp (torusLift (fun x => pressureGradient
        (normalizedScaledPressure emPres emPlacement.x₀ emPlacement.T (1/2)) t x)) 2
          periodicTorusMeasure) := by
  have hε : (1/2 : ℝ) ∈ Ioc (0 : ℝ) emPlacement.ε₀ := by
    change (1/2 : ℝ) ∈ Ioc (0 : ℝ) (1/2)
    norm_num
  exact ⟨periodized_sobolev emVel_extension_smooth (isCompact_closedBall _ _)
      emVel_support emPlacement (1/2) hε,
    periodized_pressure_gradient emPres_extension_smooth (isCompact_closedBall _ _)
      emPres_support emPlacement (1/2) hε⟩

example : emVel (1/2, 0) ≠ 0 := by
...
example : emPres (1/2, 0) = 1 := by
  have hb : emSpaceBump (0 : Space) = 1 :=
    emSpaceBump.one_of_mem_closedBall (by norm_num [emSpaceBump])
  have ht : emTimeBump (1/2 : ℝ) = 1 :=
    emTimeBump.one_of_mem_closedBall (by norm_num [emTimeBump])
  change emTimeBump (1/2 : ℝ) * emSpaceBump (0 : Space) = 1
  rw [ht, hb, one_mul]

end NSFormalization.Section3.T15.SobolevProbe
