...
/-! Reviewer negative checks for lane 150.  Each theorem below is the lane's statement
with ONE value mutated and the lane's proof script copied verbatim.  All three must fail. -/
...
/- ### M1 — factor 2 dropped from row E4's derivative value -/
theorem M1_energyDerivative_noTwo
...
/- ### M3 — `∂ₜu` replaced by `f` in row E4's pairing -/
theorem M3_energyDerivative_forceInsteadOfTemporal
    (w : ClassicalSolutionR ν a f T) (hf : MemForceR f)
    (hc : 0 < c) (hcS : c ≤ S) (hST : S < T)
    {r : ℝ} (hr : r ∈ Ioo (0 : ℝ) (S - c)) :
    HasDerivAt
      ...
      (2 * ⟪(velocitySliceField w (Ioo_subset_Ico_self
                (mem_Ioo_of_mem_Icc hc hST (show r + c ∈ Icc c S from
                  ⟨by linarith [hr.1], by linarith [hr.2]⟩)))).toLp,
            (forceSliceField hf
                (le_of_lt (mem_Ioo_of_mem_Icc hc hST (show r + c ∈ Icc c S from
                  ⟨by linarith [hr.1], by linarith [hr.2]⟩)).1)).toLp⟫)
      r := by
  have hT' : (0 : ℝ) ≤ S - c := sub_nonneg.mpr hcS
  let ιvel : Icc (0 : ℝ) (S - c) → Icc (0 : ℝ) S := fun ρ =>
    ⟨ρ.1 + c, ⟨by have := ρ.2.1; linarith, by have := ρ.2.2; linarith⟩⟩
...
/- ### M2 — viscous term sign flipped in the unconditional energy identity -/
theorem M2_energyIdentity_signFlipped
    (w : ClassicalSolutionR ν a f T) (hf : MemForceR f)
    {t : ℝ} (ht : t ∈ Ioo (0 : ℝ) T) :
    HasDerivAt
      (fun s : ℝ =>
