...
from the budget (`energyBudget a f t = l2Norm a`).  Proof of `l2Bound` copied verbatim,
with `energyBudget` replaced by `energyBudget` throughout.
...
theorem l2Bound_MUT_C (w : ClassicalSolutionR ν a f T) (hf : MemForceR f) (hν : 0 < ν) {t : ℝ}
    (ht : t ∈ Icc (0 : ℝ) T) :
