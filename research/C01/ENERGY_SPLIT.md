...
| E4b | jet-continuity in time of `∇p(s,·)` = `(I−P) h`, hence of `∂ₜu(s,·)` (the `hB` of `wordEnergy_hasDerivWithinAt`) | **M** | E4a `residualPath_jetLp_continuous`; `Leray.lerayComplement` CLM (`LerayDatum.lean:255`); `pin_pressureGradient_datum` (`PressureJets.lean:112`); `jetOfDatum_continuous` (`Evolution.lean:147`) | **OPEN.** Missing piece is the *jets ⟹ order-m datum* direction: an order-`m` analogue of `A01.orderZeroDatumCLM` (`DatumPathContinuity.lean:103`, order 0 only), i.e. the jet-norm control `‖smoothAngularDatum m m _ A‖ ≤ C_m·∑_{j≤m}‖A.jetLp j‖` — **lane 145 `FiniteOrderNorm.lean`**. Then `lerayComplement`∘it (continuous) gives `∇p`s datum path, `jetOfDatum_continuous`∘that gives `∇p` jets, and `∂ₜu = residual − ∇p` gives `hB`. See `research/C01/ATTEMPTS_JET_PATHS.md`. |
