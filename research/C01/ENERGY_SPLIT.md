...
| **energyIdentity** (:344) | `HasDerivAt (fun s => l2Sq (slice u s)) (−2ν·gradientSq(slice u t)+2·pairing(slice u t)(slice f t)) t` | **M** (assembly) | E0(`_deriv`)+E1+Ep+E2+E3+E4; `advection_inner_zero`, `laplacian_pairing`, `gradient_pairing_zero` for E0's `hnl`/`hlap`/`hpr` | **payoff DONE modulo E4 (lane 143, `energyIdentity_classical`, std 3 axioms).** Given the E4 value fact `hd : d = 2⟪u,∂ₜu⟫`, `energyIdentity_of_carrierB` yields `d = −2ν·gradientSq(u) + 2·pairing(u,f)` (raw-integral form) from Ep+E3+`hdiv`/`hgrad`/`hp`. **E4 is the single remaining input and is blocked** (see E4 row). Bindings lane: bridge the raw integrals to spec `l2Sq`/`gradientSq`/`pairing` (the `verification`-side `PiLp.norm_sq_eq_of_L2` step, `Vocabulary.lean` header), and — once E4 lands — wrap in the `HasDerivAt`. **E4 now landed (lane 150).** `energyIdentity_classical_unconditional` (`Section4/C01/EnergyDerivative.lean`, std 3 axioms) is the full `HasDerivAt` with the raw-integral value, at every interior `t ∈ Ioo 0 T`, obtained by feeding `energyDerivative_hasDerivAt` (window `c=t/2`, `S=(t+T)/2`, so `r+c=t`) into `energyIdentity_classical` and undoing the shift by the `−(t/2)` chain rule. The energy function is `s ↦ ‖(velocityField w _ (projIcc 0 ((t+T)/2) _ s)).toLp‖²` (the `projIcc` is the identity near `t`). **Remaining bridge to `Spec.lean`'s `energyIdentity` (contract at :344): the single `gradientSq`/`pairing` vocabulary step** — the raw integrands `∫∑ᵢ‖fderiv u · (axis i)‖²`, `∫⟪u,f⟫` are definitionally `gradientSq`/`pairing` on `u.field`; the `= ‖gradientTensor·‖²` identification is the `verification`-side `PiLp.norm_sq_eq_of_L2` binding noted in row E2 / `REVIEW_E3E4.md`. That plus the `l2Sq` LHS spelling is a `verification` Bindings lane, not a `formalization` proof. **REGISTERED V2 (lane 152, `C01.energy_absorption_partial_v2`).** The clamp-free `l2Sq`/`slice`/`pairing` form is now `NSFormalization.Section4.C01.energyIdentity_l2Sq` (`Section4/C01/EnergySpec.lean`, std 3 axioms), and the `verification`-side V2 contract `Contracts/V2/EnergyAbsorptionPartial.lean` (`extends` the V1 API + the field `energyIdentity` token-for-token from `Spec.lean:344-350`) is bound via the single non-`rfl` `gradientSq` bridge (`PiLp.norm_sq_eq_of_L2`+`integral_congr_ae`) and the `pairing` `rfl` bridge; `Tests.EnergyAbsorptionPartialV2` = std 3 axioms. |
| **energyDifferentialBound** (:364) | `∀E', HasDerivAt … E' t → E'+2ν·gradientSq ≤ 2·l2Norm(f)·l2Norm(u)` | **S** (after E2) | `energyIdentity` (`HasDerivAt.unique`); **E2a in the form `l2Norm (slice z t) = ‖(Z t).toLp‖`** (finding 8); Cauchy–Schwarz `real_inner_le_norm` | **DONE (lane 154, `Section4/C01/EnergyBounds.lean`, std 3 axioms).** `energyDifferentialBound`: `hderiv.unique (energyIdentity_l2Sq …)` pins `E'`; the `−2ν·gradientSq` of the identity and the `+2ν·gradientSq` here cancel to `2·pairing` (`ring`); Cauchy–Schwarz is `pairing_le_l2Norm_mul` = `pairing_eq_inner`+`real_inner_le_norm`+`norm_toLp_sq_eq_l2Sq` (finding 8 via `l2Norm_eq_norm_toLp_{velocity,force}`). Local raw-integral `gradientSq z = ∫∑ᵢ‖fderiv z · (coordinateVector i)‖²`; `0<ν`/`a∈initialClassR` unused. **REGISTERED V3 (lane 156, `C01.energy_absorption_partial_v3`).** `Contracts/V3/EnergyAbsorptionPartial.lean` (`extends EnergyAbsorptionPartialV2API` + the field token-for-token from `Spec.lean:364-370`); bound through the single existing V2 `gradientSq` bridge `energyAbsorptionPartialV2_gradientSq_eq` + `uniqueness_toA02`; `Tests.EnergyAbsorptionPartialV3` = std 3 axioms. |
| **l2Bound** (**eq:RL2**, :383) | `l2Norm (slice u t) ≤ energyBudget a f t` | **M** | `energyDifferentialBound` (dissipation discarded); generalize `Paper1.sqrt_energy_le_primitive` (`ScalarEnergy.lean:22`, currently needs `E 0 = 0 ∧ N 0 = 0`) to `√(E 0) ≤ N 0` | **DONE (lane 154, `Section4/C01/EnergyBounds.lean`, std 3 axioms).** The generalization `sqrt_energy_le_primitive'` (copy of the `ScalarEnergy` proof; only the endpoint bound `G 0 ≤ δ` changes, using `E 0 ≤ (N 0)²` from `√(E 0) ≤ N 0`) applied on `[0,t]`. Inputs: `energyDifferentialBound` + `gradientSq_nonneg` (drop dissipation, needs `0<ν`); energy continuity `velocityL2Sq_continuousOn` (velocity analogue of `forceTimeRegularity`, endpoint 0 via `sobolev`'s order-0 datum path + `continuous_jetOfDatum_zero`/`l2Norm_eq_norm_jetOfDatum`); budget continuity `intervalIntegral.continuousOn_primitive_interval` + FTC `integral_hasDerivAt_right` (from `forceTimeRegularity`); `√(E 0)=‖a‖₂=K(0)` via `ClassicalSolutionR.initial` (equality). **REGISTERED V3 (lane 156, `C01.energy_absorption_partial_v3`).** Same V3 record; `l2Bound` field token-for-token from `Spec.lean:383-387`, bound with **no bridge** (`energyBudget`/`l2Norm`/`slice` defeq to `NSFormalization.Section4.C01`'s) via `uniqueness_toA02`; the two new spec-local defs `forcePrimitive` (`Spec.lean:218-219`) / `energyBudget` (`:224-225`) restated in the contract, each with a `rfl` bridge; std 3 axioms. |
| E5 | enstrophy time derivative: `d/dt ∑ᵢ‖∂ᵢu‖²_{Lp} = 2∑ᵢ⟪∂ᵢu,∂ᵢ∂ₜu⟫` then `= −2⟪Δu,∂ₜu⟫` | **M** | `wordEnergy_hasDerivWithinAt` at `s=1` minus `s=0`; `field_directional_ibp` | **DONE (lane 163, `Section4/C01/Enstrophy.lean`, std 3 axioms).** `wordEnergy_one` / `wordInner_sum_one` collapse the order-one word layer via `(Fin 1 → Fin 3) ≃ Fin 3`; `enstrophyDerivative_hasDerivAt` subtracts the vendor identities on E4's translated window; `directional_pairing_sum_eq_neg_laplacian` sums `field_directional_inner`; `enstrophyDerivative_eq_neg_laplacian` gives the requested value equality; `enstrophyDerivative_classical_unconditional` removes the shift/clamp at every `t ∈ Ioo 0 T` in raw-integral form. E5 has no residual; E6/E7 remain separate rows. |
| E6 | enstrophy pressure drop `⟨Δu, ∇p⟩ = 0` (physical) | **M** | `field_directional_ibp` twice + solenoidality, or `gradient_pairing_zero` at the higher jet with `∇p`'s P2 packaging | **DONE (lane 170, `Section4/C01/EnstrophyIdentity.lean`, std 3 axioms).** `laplacianField_divergence_zero` gets `div(Δu)=0` from `curl curl u = ∇ div u − Δu` and `div curl=0`; `laplacian_pressure_pairing_zero` feeds it to the existing `gradient_pairing_zero` with the P2 pressure packaging. |
| E7 | enstrophy arithmetic core `½·d_grad + ν·‖Δu‖² = ⟪N,L⟫ − ⟪F,L⟫`, from `d_grad = −2⟪L,Gt⟫`, `hmom`, `⟪L,L⟫=‖Δu‖²`, `⟪P,L⟫=0` | **S** | pure algebra (pair the equation against `L=Δu`, nonlinear **kept**) | **DONE (lane 170, `inner_enstrophy_identity_deriv`, std 3 axioms).** Carrier-independent identity `d = 2⟪N,L⟫ − 2ν‖L‖² − 2⟪F,L⟫`; `enstrophyDerivative_value_classical` instantiates it with `momentum_split_toLp`. |
| **enstrophyIdentity** (:487) | `HasDerivAt (fun s => gradientSq (slice u s)) (2·advectionWork(u t) − 2ν·laplacianSq(u t) − 2·pairing(f t)(laplacian(u t))) t` | **M** (assembly) | E5+E6+E7+E3+E2 (extended to `laplacianSq`, `advectionWork`) | **DONE (lane 170, std 3 axioms).** `enstrophyIdentity_classical` is the requested raw-integral form at every `t∈Ioo 0 T`; `enstrophyIdentity_gradientSq` is the local spec-vocabulary form, using whole-field Laplacian/advection bridges. The nonlinear `advectionWork` term survives. |
...
| **h2TimeIntegral** (:576), **…ZeroDatum** (:599) | `∫⁻₀^S ‖u(t)‖²_{H²} ≤ ENNReal.ofReal (CSK² + Cν⁻¹‖∇a‖₂² + Cν⁻²∫‖f‖₂²)` | **M** (assembly) | `l2Bound` + `enstrophyIntegralBound` + `sobolevTwoFourier` + `forceTimeRegularity` (registered); zero-datum sets `a=0` | **PARTIAL (lane 170).** `h2TimeIntegral_strict` proves the C01 rpow lower integral is `≠⊤` for every `0<S<T`, directly from the continuous order-two datum path; `squaredHTwoIntegral_strict` gives A04's npow spelling via lane 159's pin; `h2TimeIntegral_of_absorption` carries the exact registered gate (slack on strict subintervals). Exact residual: the quantitative RHS and `S=T` remain blocked by the still-unproved `enstrophyIntegralBound` and `sobolevTwoFourier`; consequently the zero-datum field also remains open. Future C01 V4 extends V3; no registration in lane 170. |
