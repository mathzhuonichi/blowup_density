...
- **U6 / lane 331 status:** partial, honest. New `Section3/T11/Transport.lean` (72 declarations, all three standard axioms, no named input, no `maxHeartbeats` override) proves the **general transport constructor** `classicalSolutionT_transport` for `(t,x) ↦ (α t, x + X t)` with `c = X'`, `d = X''`, `ν ↦ α ν`, `α T' = T` — all 14 `ClassicalSolutionT` fields, including the full momentum chain rule (`navierStokesResidual_transport`: the `−Ẋ(t)` transport term cancels the extra advection term, which is exactly why the transformed force is `galileanForceT`), the datum path (translation character × amplitude × zero-mode constant, with `ℓ²` strong continuity of the translation family via Tannery) and the gauge (Haar translation invariance). Three instances: `classicalSolutionT_galilean`, `classicalSolutionT_toUnit`, `classicalSolutionT_fromUnit`, each with `velocity`/`pressure` equal to the target fields by `rfl`. `to_unit`/`from_unit` close **verbatim from the given solution's own `PeriodicLocalRegularity`** (`periodicLocalRegularity_toUnit/_fromUnit`); `transformed_solution` closes verbatim **minus its `PeriodicLocalRegularity` conjunct**, of which `pressure_poisson` (via the second-derivative symmetry lemma `spatialDivergence_directional_eq_zero`) and `projected` are also proved. Residual = exactly `∀ ν > 0, ∀ a ∈ initialClassT, ∀ f ∈ forceClassT, ∀ T (w : ClassicalSolutionT ν a f T), PeriodicLocalRegularity ν a f T w` (a `ClassicalSolutionT` carries only a `ContinuousOn` datum path; `sobolev_smooth` needs a `C^∞` one — U9/U9d work), plus, for the Galilean case only, non-differentiability of the translation family in `H^m`. See `REPORT_331.md`, `ATTEMPTS_TRANSPORT.md`.
...
- **U12b / lane 336 status:** complete, unconditional — new `Section3/T11/PairingBound.lean` proves the tame pairing bound `|⟪Gm,Nm⟫_{H^m}| ≤ 15·4^{m/2}(∑ₖW(k)^{-2})^{1/2}·‖u‖_{H²}‖u‖_{H^m}‖∇u‖_{H^m}` with `Nm` the order-`m` datum of the **physical** advection `convectionFieldT u`, i.e. lane 335's `hpair` **verbatim** (`torusPairingBound_classical`), hence `higherOrderBound` of `PeriodicContinuationAPI` **unconditionally** (`torusHigherOrderBound = higherOrderBound_of_pairingBound _ torusPairingBound_classical`; probe copies both targets verbatim). New ingredients: `torusInverseWeight_summable` (`∑ W^{-r} < ∞` for every real `r > 3/2`; the tree only had `r = 3`, and `r = 2` is what makes the `H²` low factor legitimate), `torusTrilinearConvolution` (`∑_{k,l}X(k)Y(l)β(k-l) ≤ ‖X‖₂‖Y‖₂‖β‖₁`), `torusWeightPeetre_grad` (Peetre **with one derivative** — the region split `|l| ≤ |k-l|` vs `>`; the additive Peetre inequality alone only gives `‖u‖_{H^{m+1}}`, which is **not** interchangeable with `‖∇u‖_{H^m}` and does not discharge `hpair`), and `velocityCoeffT_advection` (the advection coefficient is the convolution, from `MildPressure.periodicFourierCoeff_mul` + `T10.FourierCalculus.periodicFourierCoeff_fderiv`). Also proved: the companion `‖u‖_{H^{m+1}}` bound for lane 328's canonical convection datum with physical-norm bridges, and the Leray-projected form against a solenoidal partner. 33 named declarations, each printing exactly the three standard axioms; no `sorry`, no axiom, no named `Prop` input; see `REPORT_336.md`, `ATTEMPTS_PAIRING_BOUND.md`.
- **U12a / lane 335 status:** complete, no named input — new `Section3/T11/EnergyIdentity.lean` proves lane 322's **missing fact A** outright for a genuine `ClassicalSolutionT`: `hasDerivAt_velocityCoeffT` (time differentiation of the Fourier coefficient integral under the integral sign, from `velocity_smooth` alone, via `Paper1.periodicFourierCoeff_eq_cube` + `PeriodicIntegration.hasDerivAt_cubeIntegral_of_contDiffOn`), `velocityDerivCoeffT_momentum` (the pressure-explicit coefficient form `d/dt û ᵢ(k) = −ν|2πk|²û ᵢ(k) + (f̂ ᵢ(k) − Q̂ ᵢ(k)) − 2πikᵢp̂(k)`) and `velocityDerivCoeffT_momentum_projected` (the manuscript's projected form `d/dt û(t)(k) = −ν·4π²|k|²û(t)(k) + (P̂(f̂(t) − Q̂(t)))(k)`, `P̂` = `T10.periodicLeray` at each `k`, via `solenoidal_velocityDerivCoeffT` + `lerayAt_of_solenoidal` + `lerayAt_gradient`), `solenoidal_velocityCoeffT` + `torusPressureSymbol_drop_raw` (the pressure drops at every frequency), `hasDerivAt_torusSobolevNormAt_sq` (termwise differentiation of `∑ₖ W(k)^m∑ᵢ|û ᵢ|²`, dominated by `6MD·W(k)⁻²` from the **continuous order-`2m+4` datum path** of `ClassicalSolutionT.sobolev` plus one sup bound of `∂ₜu` on a compact slab piece — so no `ContDiffOn ℝ ∞ G` regularity predicate and no `PeriodicQuantitativeLocalInput'` is needed), and `energyIdentity_of_classical`: `d/dt‖u(t)‖²_{H^m} = −2ν‖∇u‖²_{H^m} − 2⟪(u·∇)u,u⟫_{H^m} + 2⟪f,u⟫_{H^m}` with `‖∇u‖_{H^m} = torusGradientNormAt m u t`. `hRhigh_of_pairingBound` then takes **U12b's pairing estimate as an explicit theorem argument** and returns `hRhigh` in lane 322's exact binder (`g := torusGradientNormAt m u t`), and `higherOrderBound_of_pairingBound` composes with 322 so that `PeriodicContinuationAPI.higherOrderBound` is now conditional on **missing fact B alone**. 55 module declarations, each printing exactly the three standard axioms; non-vacuity in `research/T11/probes/energy_identity_closes.lean` at the nonzero forced solution `u(t,x)=eᵗc`, where the identity's right-hand side is forced to `2e^{2t}‖K‖² > 0`. See `REPORT_335.md`, `ATTEMPTS_ENERGY_IDENTITY.md`.
- **U9d2c / lane 334 status:** complete — `Section3/T11/MildClassical.lean` proves `mild_to_classical` (the U9d existential target verbatim, **no named input**: `PersistenceInput` is discharged from lane 330), including the two residual joint-slab smoothness fields `velocity_smooth`/`pressure_smooth` and `PeriodicLocalRegularity.sobolev_smooth`, plus the unconditional corollary `exists_classical_of_picard`; probe, non-vacuity (affine constant force, nonzero velocity) and exact three-axiom audit of all 77 declarations pass. See `REPORT_334.md`, `ATTEMPTS_MILD_CLASSICAL.md`.
- **U12 / lane 322 status:** partial, honest — new `Section3/T11/HighOrder.lean` proves the exact `higherOrderBound` field **verbatim** from ONE written-out hypothesis, the periodic `eq:Rhigh` on a `ClassicalSolutionT` (`higherOrderBound_of_energyInequality`; no `def … : Prop` named input, because two independent facts are missing and naming one would misrepresent the gap). Unconditional: order descent `‖z‖_{H^r} ≤ ‖z‖_{H^s}` with constant 1, the horizon-uniform running `H²` cap from `squaredHTwoIntegralT S u ≠ ⊤`, the `L¹_tH^m` forcing cap for `f ∈ F_T`, the **pressure drop** by coefficient-side solenoidality (exhibited in the probe at a nonzero velocity datum and a nonzero pressure-gradient datum), the Laplacian pairing value `−|2πk|²∑|û|²`, force Cauchy–Schwarz, Young's absorption (`C_{m,ν} = C_m²/(4ν)`) and the full `ζ↓0` + Grönwall chain reusing `Section4.A04.sqrt_le_primitive_linear` / `Section4.A01.gronwall_bddAbove_Ico`. Residual = eq:Rhigh, needing (a) time differentiability of the coefficient path + momentum in datum form (Section 4's SL1/SL2; blocked on lane 321's `regularity`, hence on `PeriodicQuantitativeLocalInput'`) and (b) the torus tame product at the **pairing** level (T12's `tameProduct` is a scalar physical-field bound; lane 328's real-order convection CLM is the starting point). Suggested follow-ups **U12a** (energy identity) and **U12b** (nonlinear pairing bound); their conjunction is exactly the hypothesis. 24 module declarations, each printing exactly the three standard axioms (the shear-mode witness lives in the probe); see `REPORT_322.md`, `ATTEMPTS_HIGH_ORDER.md`.

- **U12a / lane 335 status:** complete, no named input — new `Section3/T11/EnergyIdentity.lean` proves lane 322's **missing fact A** outright for a genuine `ClassicalSolutionT`: `hasDerivAt_velocityCoeffT` (time differentiation of the Fourier coefficient integral under the integral sign, from `velocity_smooth` alone, via `Paper1.periodicFourierCoeff_eq_cube` + `PeriodicIntegration.hasDerivAt_cubeIntegral_of_contDiffOn`), `velocityDerivCoeffT_momentum` (the coefficient form `d/dt û ᵢ(k) = −ν|2πk|²û ᵢ(k) + (f̂ ᵢ(k) − Q̂ ᵢ(k)) − 2πikᵢp̂(k)`), `solenoidal_velocityCoeffT` + `torusPressureSymbol_drop_raw` (the pressure drops at every frequency), `hasDerivAt_torusSobolevNormAt_sq` (termwise differentiation of `∑ₖ W(k)^m∑ᵢ|û ᵢ|²`, dominated by `6MD·W(k)⁻²` from the **continuous order-`2m+4` datum path** of `ClassicalSolutionT.sobolev` plus one sup bound of `∂ₜu` on a compact slab piece — so no `ContDiffOn ℝ ∞ G` regularity predicate and no `PeriodicQuantitativeLocalInput'` is needed), and `energyIdentity_of_classical`: `d/dt‖u(t)‖²_{H^m} = −2ν‖∇u‖²_{H^m} − 2⟪(u·∇)u,u⟫_{H^m} + 2⟪f,u⟫_{H^m}` with `‖∇u‖_{H^m} = torusGradientNormAt m u t`. `hRhigh_of_pairingBound` then takes **U12b's pairing estimate as an explicit theorem argument** and returns `hRhigh` in lane 322's exact binder (`g := torusGradientNormAt m u t`), and `higherOrderBound_of_pairingBound` composes with 322 so that `PeriodicContinuationAPI.higherOrderBound` is now conditional on **missing fact B alone**. 51 module declarations, each printing exactly the three standard axioms; non-vacuity in `research/T11/probes/energy_identity_closes.lean` at the nonzero forced solution `u(t,x)=eᵗc`, where the identity's right-hand side is forced to `2e^{2t}‖K‖² > 0`. See `REPORT_335.md`, `ATTEMPTS_ENERGY_IDENTITY.md`.
- **U6 / lane 331 status:** partial, honest. New `Section3/T11/Transport.lean` (72 declarations, all three standard axioms, no named input, no `maxHeartbeats` override) proves the **general transport constructor** `classicalSolutionT_transport` for `(t,x) ↦ (α t, x + X t)` with `c = X'`, `d = X''`, `ν ↦ α ν`, `α T' = T` — all 14 `ClassicalSolutionT` fields, including the full momentum chain rule (`navierStokesResidual_transport`: the `−Ẋ(t)` transport term cancels the extra advection term, which is exactly why the transformed force is `galileanForceT`), the datum path (translation character × amplitude × zero-mode constant, with `ℓ²` strong continuity of the translation family via Tannery) and the gauge (Haar translation invariance). Three instances: `classicalSolutionT_galilean`, `classicalSolutionT_toUnit`, `classicalSolutionT_fromUnit`, each with `velocity`/`pressure` equal to the target fields by `rfl`. `to_unit`/`from_unit` close **verbatim from the given solution's own `PeriodicLocalRegularity`** (`periodicLocalRegularity_toUnit/_fromUnit`); `transformed_solution` closes verbatim **minus its `PeriodicLocalRegularity` conjunct**, of which `pressure_poisson` (via the second-derivative symmetry lemma `spatialDivergence_directional_eq_zero`) and `projected` are also proved. Residual = exactly `∀ ν > 0, ∀ a ∈ initialClassT, ∀ f ∈ forceClassT, ∀ T (w : ClassicalSolutionT ν a f T), PeriodicLocalRegularity ν a f T w` (a `ClassicalSolutionT` carries only a `ContinuousOn` datum path; `sobolev_smooth` needs a `C^∞` one — U9/U9d work), plus, for the Galilean case only, non-differentiability of the translation family in `H^m`. See `REPORT_331.md`, `ATTEMPTS_TRANSPORT.md`.
- **U6b / lane 339 status:** complete, no named input — new `Section3/T11/ClassicalRegularity.lean` (82 declarations, all three standard axioms, no `maxHeartbeats` override) proves `periodicLocalRegularity_of_classical`: **every** `ClassicalSolutionT` satisfies `PeriodicLocalRegularity`, i.e. exactly the residual hypothesis of lane 331 (`REPORT_331.md` §3) and of lane 321's `regularity`. `sobolev_smooth` is the new analysis: a Bernstein bound `W(k)^N‖ĝ(k)‖ ≤ 2^N(sup_cube|g| + sup_cube|Δ^N g|)` (iterated Laplacian symbol + `(1+A)^N ≤ 2^N(1+A^N)`), the slab derivative fields `stPartial` (frozen-time spatial) and `stTime` (`fderivWithin` on `Ico 0 T ×ˢ univ`, hence defined and smooth **up to `t = 0`**), one uniform weighted coefficient bound on each compact time window (compactness of `J ×ˢ cube`), a Tannery argument (`tendsto_tsum_of_dominated_convergence`, dominating weight `W(k)⁻²`) turning coefficientwise convergence into `ℓ²` convergence, lane 335's differentiation under the integral sign plus the mean value inequality for the interior derivative, `hasDerivWithinAt_Ici_of_tendsto_deriv` for the one-sided derivative at `t = 0`, and induction on the order of differentiation via `contDiffOn_succ_iff_derivWithin` (the `j`-th derivative of the datum path is the datum path of `stTime^[j] u`). `pressure_poisson` is the divergence of the projected equation: `div ∂ₜu = 0` by `ContDiffAt.isSymmSndFDerivAt` on the open slab, `div Δu = 0` by using lane 331's `spatialDivergence_directional_eq_zero` twice, `div ∇p = Δp`; the initial time is reached by continuity of all three terms in `t` on the closed-at-zero slab. Consequently `to_unit`, `from_unit`, `transformed_solution` and `PeriodicLocalTheoryAPI.regularity` (`regularity_of_solution`) are **unconditional**; lane 331's second residual (non-differentiability of the translation family in `H^m`) is bypassed, since the Galilean transform of a classical solution is itself a `ClassicalSolutionT`. Non-vacuity in `research/T11/probes/classical_regularity_closes.lean` at the nonzero constant flow, whose order-`m` datum path is nonzero at `t = 0`. See `REPORT_339.md`, `ATTEMPTS_CLASSICAL_REGULARITY.md`.
