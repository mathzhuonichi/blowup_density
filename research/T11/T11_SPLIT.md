...
- **U9e / lane 338 status:** complete — new `Section3/T11/ExistenceInputH3.lean` proves `periodicQuantitativeLocalInputH3` (amendment 2's `H³`-ball version of `PeriodicQuantitativeLocalInput'`, every other quantifier identical) **outright, no named input**, with the horizon given explicitly as `picardHorizon ν K M = torusKernelTime ν (torusPicardThreshold ‖bilinear‖ (K.toReal + (M 3).toReal))` — a function of `ν`, `K` and `M 3` alone (`exists_classical_on_picardHorizon`). Key new step: the force hypothesis is an `L¹_t H^m` bound, not a supremum, so lane 313's `torus_forcedLinear_bound` is replaced by `torus_forcedLinear_bound_L1` (heat contraction ⟹ `‖L(t)‖ ≤ ‖A‖ + ∫₀^T‖F‖`), and the infimum defining `forceSobolevENormT` is shown **attained** at every admissible path (`forceSobolevENormT_eq_of_path`, from `datum_unique`); exactly one force order, `M 3`, is consumed. Re-instantiations with the input discharged: `restartH3` (321), `restartBeyondH3` (332), `extendsBeyondH3` / `lifespanInfiniteOfLocallyFiniteH3` (337, using `hHigh` at `m = 3` instead of `m = 1`; `hHigh` is the only binder left anywhere), `periodicMaximalExistenceInput_unconditional` / `exists_maximal_unconditional` (323). No `set_option maxHeartbeats` anywhere; all 24 declarations print the three standard axioms. The `H¹` ball stays open — exact residual statements, the Fujita–Kato reason and the U17 consumer checklist (T20's `Spec.lean:402,437` is the one place to re-read) are in `research/T11/H1_GAP.md`. **The lane base did not compile**: 334's `MildClassical.lean` vs 327's post-`2572e51b` `MildMomentum.lean` (`hFI` argument removed); three lines of `MildClassical.lean` repaired, see `ATTEMPTS_EXISTENCE_INPUT_H3.md` §0 — needs a `logs/LESSONS.md` line from the lead. See `REPORT_338.md`.
