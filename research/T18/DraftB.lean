...
/-!
# T18 draft B: the Lean statement of `thm:insertion` (exact local insertion on `T³`)

Statement-only draft (double-blind draft B) of Theorem `thm:insertion`,
`paper/sections/03-torus.tex:287-311`, proof `:312-346`.  No proofs; one
`Type`-valued structure `PeriodicInsertionAPI` whose fields are the clauses of
the theorem, plus `periodicInsertionStatement : Prop`.

Shape mirrors Section 4's registered `InsertionFamilyAPI`
(`Contracts/V1/InsertionFamily.lean`, R42) and its lifespan extension
`InsertionLifespanV2API` (`Contracts/V2/InsertionLifespan.lean`), differing only
where the torus proof differs (torus norms/classes, the `∫_{T³}p=0` gauge, the
periodized packet, the two cross-transport identities, and the `0≤s<1/2` range
of `eq:Hsclose`); those differences are recorded field by field in
`research/T18/COMPARISON_B.md`.

The T13/T15/T16/T17 vocabulary the statement is written in is copied verbatim
from `research/T17/Spec.lean:17-987` (block delimited below).  The registered
`Contracts.V1.TorusLocalTheory` energy / force-Sobolev norms, `ClassicalSolutionT`,
`maximalLifespanT`, `IsMaximalPeriodicSolution`, `forceClassT`, `initialClassT`,
`normalizePressureT` are imported and used by name; drift of the copied block's
overlapping `T10.Draft` restatements against those registered names is pinned by
`example … := rfl` immediately after the copied block.
-/

noncomputable section

/- ============================================================================
   BEGIN vocabulary copied verbatim from research/T17/Spec.lean:17-987
   (T10.Draft energy/force-norm restatements, T13.Spec LocalizationAPI,
    T16.Draft CutoffData/LocalPotentialAPI, T15.Spec mixed norms/PlacementData,
    T17.Spec CorrectionAPI).  No token changed.
   ============================================================================ -/

/- copied verbatim from research/T10/Spec.lean:255-268,393-411;
   registered `Contracts.V1.TorusData` supplies the torus vocabulary. -/
...
`L∞(0,T;L²(T³))` norm on the open time interval. -/
...
/- copied verbatim from research/T13/Spec.lean:168-312; dependencies resolve
   through registered `Contracts.V1.TorusData`. -/
...
/- copied verbatim from research/T16/Spec.lean:53-336; only the T16 helpers,
   `CutoffData`, and `LocalPotentialAPI` are retained. -/
...
/- END verbatim copy from research/T17/Spec.lean:17-987 -/

/-! ## Drift checks: the copied `T10.Draft` restatements equal the registered
`Contracts.V1.TorusLocalTheory` names this file uses.  Each is `rfl`, pinning
that the copied vocabulary block did not drift from the registered spelling. -/

open scoped ENNReal

/-- `energyENormT` drift check against the registered `TorusLocalTheory` spelling. -/
example (T : ℝ) (z : BlowupDensity.Contracts.V1.Data.SpaceTimeField) :
    BlowupDensity.T10.Draft.energyENormT T z
      = BlowupDensity.Contracts.V1.TorusLocalTheory.energyENormT T z := rfl

/-- `energyEssSupT` drift check. -/
example (T : ℝ) (z : BlowupDensity.Contracts.V1.Data.SpaceTimeField) :
    BlowupDensity.T10.Draft.energyEssSupT T z
      = BlowupDensity.Contracts.V1.TorusLocalTheory.energyEssSupT T z := rfl

/-- `energyGradientT` drift check. -/
example (T : ℝ) (z : BlowupDensity.Contracts.V1.Data.SpaceTimeField) :
    BlowupDensity.T10.Draft.energyGradientT T z
      = BlowupDensity.Contracts.V1.TorusLocalTheory.energyGradientT T z := rfl

/-- `forceSobolevENormT` drift check. -/
example (q : ℝ≥0∞) (s : ℝ) (f : BlowupDensity.Contracts.V1.Data.SpaceTimeField) :
    BlowupDensity.T10.Draft.forceSobolevENormT q s f
      = BlowupDensity.Contracts.V1.TorusLocalTheory.forceSobolevENormT q s f := rfl

/-- `IsPeriodicSobolevPath` drift check. -/
example (s : ℝ) (f : BlowupDensity.Contracts.V1.Data.SpaceTimeField)
    (G : ℝ → BlowupDensity.Contracts.V1.TorusData.PeriodicSobolev s) :
    BlowupDensity.T10.Draft.IsPeriodicSobolevPath s f G
      = BlowupDensity.Contracts.V1.TorusLocalTheory.IsPeriodicSobolevPath s f G := rfl

namespace BlowupDensity.T18.DraftB
...
/-! ## Periodized rescaled pressure and force

`03-torus.tex:112-120,314`: "Place the rescaled whole-space solution in `B` and
then periodize it."  The velocity periodization `periodicScaledPacket` is copied
above (T16.Draft); these are its two companions for `P_ε` and `F_ε`, built from
the registered whole-space `scaledPressure` / `scaledForce`
(`Contracts.V1.Scaling`) and the copied lattice vector.  They are the only new
definitions this file introduces; both are literal lattice sums, not stubs. -/

/-- `P_ε` on `T³`: the lattice periodization of the rescaled whole-space packet
pressure `scaledPressure Pr x₀ T ε`; `03-torus.tex:113-115,120,314`. -/
def periodicScaledPressure (Pr : PressureField) (x₀ : Space) (T ε : ℝ) : PressureField :=
  fun z => ∑' k : PeriodicFrequency,
    scaledPressure Pr x₀ T ε (z.1, z.2 - BlowupDensity.T16.Draft.latticeVector k)

/-- `F_ε` on `T³`: the lattice periodization of the rescaled whole-space packet
force `scaledForce F x₀ T ε`; `03-torus.tex:116-117,120,314`. -/
def periodicScaledForce (F : VelocityField) (x₀ : Space) (T ε : ℝ) : VelocityField :=
  fun z => ∑' k : PeriodicFrequency,
    scaledForce F x₀ T ε (z.1, z.2 - BlowupDensity.T16.Draft.latticeVector k)

/-- **Theorem `thm:insertion` (exact local insertion on `T³`),
`paper/sections/03-torus.tex:287-311`, proof `:312-346`, for one `ε`-family.**

Parameters are the objects the proof takes as given (`:287-289,312-314`):
`ν`, the fixed whole-space packet `P` (`prop:scaling` rescales it), the
placement `place` of `lem:localization`/`prop:scaling`, the initial velocity `a`
and reference force `g`, the coordinate-ball radius `r` and regularity margin
`δ`, the cutoff data `cutoff` of `lem:potential`, the reference solution
`reference` regular through `T+δ`, and the `lem:correction` record `correction`
(`w_ε`, `H_ε` and every bound of `eq:wE`, `eq:Hmixed`, `eq:HHs`).  This mirrors
`InsertionFamilyAPI (ν) (P)` (`Contracts/V1/InsertionFamily.lean`), whose one
`scaling : ScalingAPI ν P` field bundles the ambient data; here that bundle is
split across the placement and the `CorrectionAPI` parameters because the torus
scaling/correction records are not (yet) a single registered contract.

Fields are the clauses of the theorem; every quantitative field carries the
integrability/class guard that makes its Bochner norm honest.  Differences from
the `R42`/`R42.v2` template are marked `TORUS:` and tabulated in
`research/T18/COMPARISON_B.md`. -/
structure PeriodicInsertionAPI (ν : ℝ) (P : PacketAPI ν)
    (place : BlowupDensity.T15.Spec.PlacementData P)
    (a : SpatialField) (g : SpaceTimeField) (r δ : ℝ)
    (cutoff : BlowupDensity.T16.Draft.CutoffData)
    (reference : ClassicalSolutionT ν a g (place.T + δ))
    (correction :
      BlowupDensity.T17.Spec.CorrectionAPI ν place reference.velocity r δ cutoff) :
    Type where
  ...
  Quantifier order: none.  Non-vacuity: this is a strict inequality on the real
  margin used in `reference`'s horizon `place.T + δ`. -/
  delta_pos : 0 < δ
  ...
  eps_pos : 0 < ε₀
  /-- `03-torus.tex:290,312`: the inserted family is a sub-family of the
  correction family, so every `lem:correction` bound holds on `(0,ε₀]`.
  Quantifier order: none.  Non-vacuity: a genuine `≤` between the two thresholds. -/
  eps_le_cutoff : ε₀ ≤ cutoff.ε₀
  ...
  with `v = reference.velocity`, `w_ε = cutoff.correction ε` (`lem:potential`),
  and `U_ε` the periodized rescaled packet (`prop:scaling`).
  Quantifier order: `∀ ε, ∀ z`.  Non-vacuity: a pointwise field equation fixing
  `velocity` on all spacetime, not an existential shadow. -/
  velocity_formula : ∀ ε : ℝ, ∀ z : SpaceTime,
    velocity ε z = reference.velocity z + cutoff.correction ε z +
      BlowupDensity.T16.Draft.periodicScaledPacket P.velocity place.x₀ place.T ε z
  /-- `03-torus.tex:314,318-320` `eq:insertion`, second display: `p_ε = π + P_ε`
  normalized by subtracting its torus mean ("Initially use the compact
  representative `P_ε`; subtract its spatial mean to normalize `p_ε`",
  `:318-320`).  TORUS: the `∫_{T³}p=0` gauge (`normalizePressureT`) is imposed;
  the `R42` twin keeps the compact gauge `π + P_ε` (`InsertionFamily.lean:183`).
  Quantifier order: `∀ ε`.  Non-vacuity: fixes `pressure ε` as a concrete
  normalized field. -/
  pressure_formula : ∀ ε : ℝ,
    pressure ε = normalizePressureT
      (fun z => reference.pressure z + periodicScaledPressure P.pressure place.x₀ place.T ε z)
  /-- `03-torus.tex:314` `eq:insertion`, third display: `g_ε = g + H_ε + F_ε`,
  with `H_ε = correctionForce ν v cutoff ε` (`lem:correction`, `eq:H`) and `F_ε`
  the periodized rescaled packet force (`prop:scaling`).
  Quantifier order: `∀ ε, ∀ z`.  Non-vacuity: a pointwise field equation. -/
  force_formula : ∀ ε : ℝ, ∀ z : SpaceTime,
    force ε z = g z + BlowupDensity.T17.Spec.correctionForce ν reference.velocity cutoff ε z +
      periodicScaledForce P.force place.x₀ place.T ε z
  ...
  identically zero, where `b_ε = v + w_ε` is the corrected background and `U_ε`
  the periodized packet.  TORUS: this explicit identity is the periodic form of
  the whole-space cancellation used in Theorem 4.2's proof.
  Quantifier order: `∀ ε ∈ Ioc 0 ε₀, ∀ t ∈ Ico 0 T, ∀ x`.  Non-vacuity: the
  directional (Fréchet) derivative of the actual packet in the actual background
  direction vanishes pointwise. -/
  crossTransport_background_advects_packet : ∀ ε ∈ Ioc (0 : ℝ) ε₀,
    ∀ t ∈ Ico (0 : ℝ) place.T, ∀ x : Space,
      spatialDerivative
          (BlowupDensity.T16.Draft.periodicScaledPacket P.velocity place.x₀ place.T ε) t x
          (BlowupDensity.T16.Draft.correctedBackground reference.velocity cutoff.correction ε
            (t, x)) = 0
  ...
  crossTransport_packet_advects_background : ∀ ε ∈ Ioc (0 : ℝ) ε₀,
    ∀ t ∈ Ico (0 : ℝ) place.T, ∀ x : Space,
      spatialDerivative
          (BlowupDensity.T16.Draft.correctedBackground reference.velocity cutoff.correction ε) t x
          (BlowupDensity.T16.Draft.periodicScaledPacket P.velocity place.x₀ place.T ε (t, x)) = 0
  ...
  support, via `periodicSet`, rather than the whole-space ball of `R42`.
  ...
  velocityDifference_support : ∀ ε ∈ Ioc (0 : ℝ) ε₀,
    ∀ t ∈ Ico (0 : ℝ) place.T,
      tsupport (fun x : Space => velocity ε (t, x) - reference.velocity (t, x)) ⊆
        BlowupDensity.T16.Draft.periodicSet (Metric.ball place.x₀ (ε * diffSupportRadius))
  ...
  and `C = correction.energyConst` (`eq:wE`).  The norm is the registered torus
  `energyENormT`, so the bound cannot be met vacuously.
  Quantifier order: `∀ ε ∈ Ioc 0 ε₀`.  Non-vacuity: an `ℝ≥0∞` inequality on the
  actual energy norm of the actual difference. -/
  ...
  /-- `03-torus.tex:303-304`: honest Haar-`L^p` slice representative of `g_ε - g`,
  making the Bochner infimum in `mixedLebesgueENormT` honest.  Quantifier order:
  `∀ p [Fact (1≤p)], ∀ ε ∈ Ioc 0 ε₀, ∀ t`.  Non-vacuity: the actual difference
  slice has an `L^p(T³)` representative. -/
  forceDifference_spatial_memLp : ∀ (p : ℝ≥0∞) [Fact (1 ≤ p)],
    ∀ ε ∈ Ioc (0 : ℝ) ε₀, ∀ t : ℝ,
      MemLp (torusLift (fun x => force ε (t, x) - g (t, x))) p periodicTorusMeasure
  /-- `03-torus.tex:303-304` `eq:Fclose`: `‖g_ε - g‖_{L^q_tL^p_x} ≤
  C_{p,q}(ε^{α(p,q)} + ε^{α(p,q)+1})`, `α(p,q) = -3+3/p+2/q`.  The `ε^{α}` term is
  `F_ε` (`eq:packetFscale`), the `ε^{α+1}` term is `H_ε` (`eq:Hmixed`); force
  norms over `(0,∞)`.  Quantifier order: `∀ p q [Fact (1≤p)], 1 ≤ q,
  ∀ ε ∈ Ioc 0 ε₀`.  Non-vacuity: the previous field guards honesty; the norm is
  the copied `mixedLebesgueENormT`. -/
  forceDifference_mixed_bound : ∀ (p q : ℝ≥0∞) [Fact (1 ≤ p)], 1 ≤ q →
    ∀ ε ∈ Ioc (0 : ℝ) ε₀,
      BlowupDensity.T15.Spec.mixedLebesgueENormT q p (fun z => force ε z - g z) ≤
  ...
  forceDifference_sobolev_memLp : ∀ s : ℝ, 0 ≤ s → s < 1 / 2 →
    ∀ ε ∈ Ioc (0 : ℝ) ε₀,
      BlowupDensity.T15.Spec.MemForceSobolevT 1 s (fun z => force ε z - g z)
  ...
  actual difference to `0` as `ε ↓ 0`, which also asserts eventual finiteness. -/
...
for every viscosity `ν > 0`, every whole-space packet `P`, placement, initial
datum `a`, reference force `g`, radii `r`, margin `δ`, cutoff, reference
solution regular through `T+δ`, and correction record, with `δ > 0`, `g ∈ 𝓕`,
`a ∈ 𝓧`, there is (an `ε₀ > 0` and) an inserted family: `Nonempty` of the API,
which carries the threshold `ε₀` as a field.  Introducing this definition
asserts nothing. -/
def periodicInsertionStatement : Prop :=
  ∀ (ν : ℝ), 0 < ν → ∀ (P : PacketAPI ν)
    ...
    0 < δ → g ∈ forceClassT → a ∈ initialClassT →
      Nonempty (PeriodicInsertionAPI ν P place a g r δ cutoff reference correction)

end BlowupDensity.T18.DraftB
