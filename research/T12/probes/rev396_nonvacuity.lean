...
theorem rev396_probeMZ_finite : periodicHomogeneousENorm (1 / 2) probeMZ < ⊤ := by
  exact NSFormalization.Section3.T13.periodicHomogeneousENorm_lt_top
    (by norm_num) probeZ_periodic probeZ_contDiff

example : MemPeriodicHomogeneous (1 / 2) probeMZ :=
  ⟨probeMZ_smoothPeriodic.2, memLp_torusLift_vector probeMZ_smoothPeriodic.1.continuous 2,
    probeMZ_meanZero, rev396_probeMZ_finite.ne⟩
