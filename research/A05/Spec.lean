...
`paper/sections/appendix-b-embeddings.tex:11-38`, **restricted to `R³`** and in
...
(`appendix-b-embeddings.tex:17`, second inequality) is **out of scope**: no
...
* Homogeneous realization: `eq:homogeneous-realization`
  ...
  (`appendix-b-embeddings.tex:55-70`) are the single predicate
...
* Fields: `MemHInfty` (`02-preliminaries.tex:12` eq:Rinitial) is the
  "smooth `H^∞` fields" class of `appendix-b-embeddings.tex:36-37,71-72`.
...
* Every norm is `ℝ≥0∞`-valued and no norm is routed through `.toReal`, so the
  ...
  (`appendix-b-embeddings.tex:26-27`) is the fail-safe `⊤` on the right-hand side
...
`appendix-b-embeddings.tex:100-102` refuses `Ḣ^{3/2}` as a space, so `Λ^{3/2}u`
...
No `𝓢' → 𝓢'` multiplier by `|ξ|` is introduced here, and there is none in tree:
`Paper3/HomogeneousRealization.lean:7` states only that a full `L² → 𝓢'`
homogeneous multiplier "is intentionally not introduced here", giving no reason.
The reason recorded below is this contract's own, not that file's: at `s = 3/2`
there is no completed domain for such a map to act on.
-/
...
/-- `appendix-b-embeddings.tex:26-27` `‖v‖_{Ḣ^s}` **for a physical field**, the
...
/-- `appendix-b-embeddings.tex:12` `p_a = 6/(3-2a)`, the critical exponent of
...
/-- Lemma B.1 (`lem:critical-embeddings`, `appendix-b-embeddings.tex:11-38`) on
`R³`, in the form Propositions 4.3 and 4.4 consume it.

All constants are structure fields, hence quantified **outside** every field,
every viscosity and every solution: `appendix-b-embeddings.tex:109-110`, "All
...
structure CriticalEmbeddingAPI where
  /-- `appendix-b-embeddings.tex:14-15` `C_a` of eq:critical-embedding-pair, the
  ...
  C : ℝ → ℝ
  /-- `appendix-b-embeddings.tex:12-13` "there are finite constants". -/
  C_pos : ∀ a : ℝ, 0 < C a
  /-- `appendix-b-embeddings.tex:30-31` eq:critical-derived, second line: the one
  ...
  /-- `appendix-b-embeddings.tex:100-101` "The Euclidean statements use the
  ...
  /-- `appendix-b-embeddings.tex:69-70` "Thus this construction gives a unique
  ...
  `dotHomogeneousENorm` is attained rather than a strict infimum.

  The range is `-3/2 < s ≤ 3/2`: one step past the in-tree convention
  `-3/2 < s < 3/2` of `Contracts/V1/Data.lean:328`, because **the endpoint is
  where Proposition 4.3 needs it** — `04-whole-space.tex:91` sets
  `z = ‖Λ^{3/2}u‖₂`, and `rieszPowerNorm` and `derivativeCriticalL3` are stated
  at `a = 3/2`, so without the endpoint nothing in this contract says their
  right-hand side `dotHomogeneousENorm (3/2) v` is attained.  Uniqueness does
  hold at `s = 3/2`: if `G` and `K` are both order-`s` data of `z` then
  `|ξ|^{-s}(G-K)` is integrable against every Schwartz test (that is the
  `Integrable` clause of `Data.IsHomogeneousDatum`, satisfied by both) and pairs
  to zero, hence vanishes a.e., hence `G = K` a.e.  What fails at `s = 3/2` is
  only *surjectivity* of `G ↦ z` onto `L²`, i.e. the existence of a completion —
  exactly what `appendix-b-embeddings.tex:100-102` declines to build, and what no
  clause of this contract asserts.  The two paper citations are for the content;
  neither displays the range. -/
  ...
  homogeneous datum, which is `appendix-b-embeddings.tex:71-72` "It includes all
  ...
  /-- `appendix-b-embeddings.tex:98-99` "Plancherel and `∑_j|ξ_j|² = |ξ|²`",
  ...
  /-- `appendix-b-embeddings.tex:69-70`: the completion's unique `L^{p_a}`
  ...
  `appendix-b-embeddings.tex:44`; `a = 3/2` is included as well because
  `‖Λ^{3/2}u‖₂` is a quantity the manuscript writes on `H^∞` fields even though
  `appendix-b-embeddings.tex:100-102` builds no `Ḣ^{3/2}` space. -/
  rieszPowerExists :
    ∀ a : ℝ, 0 < a → a ≤ 3 / 2 →
      ∀ v : SpatialField, MemHInfty v → dotHomogeneousENorm a v ≠ ⊤ →
        ∃ w : SpatialField, IsRieszPower a v w ∧ MemLp w 2 volume

  /-- `04-whole-space.tex:91` and `appendix-b-embeddings.tex:30-31`:
  ...
  /-- `appendix-b-embeddings.tex:22-24` "Componentwise application gives the same
  ...
  **tensor** follows from the `L^p` membership of its three vector columns, with
  ...
  /-- `appendix-b-embeddings.tex:14-15` eq:critical-embedding-pair, Euclidean half,
  ...
  The `a = 1` clause is kept for fidelity to `eq:critical-embedding-pair` but is
  **not consumed inside this contract**: `gradientLSix` is an independent field
  with its own constant, discharged by the Fourier-free vendor bound instead of
  by the paper's stated route through the `Ḣ^1 → L^6` case.  Dropping `a = 1`
  would lose nothing downstream; it is retained so that the contract states
  Lemma B.1 rather than only the part R43/R44 happen to use. -/
  embeddingPair :
    ∀ a : ℝ, a = 1 / 2 ∨ a = 1 →
      ∀ v : SpatialField, MemHInfty v →
  ...
  `appendix-b-embeddings.tex:22-24`, and the right-hand side is
  `Proposition 4.3`'s `z` by `rieszPowerNorm` at `a = 3/2`.

  On the right-hand side `s = 3/2` sits at the endpoint that
  `appendix-b-embeddings.tex:100-102` refuses to complete into a space.  The
  ...
  (`appendix-b-embeddings.tex:100-102`). -/
