...
Constants: `appendix-b-embeddings.tex:109-110`, "All constants depend only on
...
solution, or on frequency support; `research/section4/STATEMENTS.md:434`, `:508`
and `:1288` make `ν`-freedom of `c, C₀, C₁` load-bearing for Theorem 4.1's
converse.
...
wherever finite." `appendix-b-embeddings.tex:49-51` repeats it: "Tao's Fourier
...
The same holds for the torus half (`appendix-b-embeddings.tex:83-85`), where the
...
`RieszFrequencyCutoffs.symbol a ξ = ‖ξ‖^{-a}` in the `𝓕` variable
(`Source/RieszFrequencyCutoffs.lean:13`, consumed by
`Source/RieszSingularMultiplier.lean:17` `def multiplier`);
...
* **`s = 3/2` is inside `Data.IsHomogeneousDatum`'s stated collapse range.**
  ...
  asserted, per `appendix-b-embeddings.tex:100-102`.
...
1. **Not as a `𝓢' → 𝓢'` multiplier.** `Paper3/HomogeneousRealization.lean:7`
   states that a full `L² → 𝓢'` homogeneous multiplier "is intentionally not
   introduced here". It gives no reason, and `HasTemperateGrowth` does not occur
   in that file; earlier drafts of this document attributed a reason to it that
   it does not contain. The reason offered here is this document's own: the
   order that Section 4 most needs, `a = 3/2`, has no completed domain (point 2),
   so a total multiplier would not help even if one were built.
   `research/D01/RECONCILIATION.md:192-197` reaches the same conclusion from the
   manuscript side and hands `Λ`, `J` to A03/A05/R43/R44. `Spec.lean` builds no
   such multiplier.
2. **Not as a total operator on a completed space either**, at the order that
   matters most: `appendix-b-embeddings.tex:100-102` refuses `Ḣ^{3/2}` as a space,
...
Three deliberate narrowings, recorded so review can accept or reject them
explicitly rather than discover them later.

1. **`embeddingPair` at `a = 1` is stated but never consumed.** Inside the
   contract, `gradientLSix` is an independent field with its own constant
   `Csix`, discharged by the Fourier-free vendor bound
   (`vendor/.../R3/SmoothSobolevL6.lean:72`). The paper
   (`04-whole-space.tex:112`) instead derives `‖∇u‖₆ ≤ C‖Δu‖₂` *from* the
   `Ḣ^1 → L^6` case of Lemma B.1, so the contract's route is not the paper's.
   **Kept, not dropped**, for two reasons: `eq:critical-embedding-pair`
   (`appendix-b-embeddings.tex:12-15`) is stated for `a ∈ {1/2, 1}` and a
   contract that dropped `a = 1` would state less than Lemma B.1; and it is the
   fallback if the `‖D²v‖₂ = ‖Δv‖₂` half of U9 turns out harder than the vendor
   route makes it look. Cost of keeping it: one extra proof obligation in U7 that
   nothing downstream needs. R43/R44 review may drop it at no loss.
2. **Everything is stated for `MemHInfty` fields only**, whereas
   `formalization/blueprint/DEPENDENCY_GRAPH.md:199` asks for the "full stated
   homogeneous completion and smooth H-infinity fields". The completion itself —
   the abstract closure of `{v : v̂ ∈ C_c^∞(R³∖{0})}` in `‖Λ^a v‖₂`
   (`appendix-b-embeddings.tex:51-53`) — is **not** built here; only its
   realization predicate `Data.IsHomogeneousDatum` and the statement that every
   `H^∞` field with a finite homogeneous norm satisfies it
   (`homogeneousLeSobolev`, `criticalRepresentative`). The narrowing is licensed
   by `appendix-b-embeddings.tex:34-37` ("in particular they apply to all the
   smooth `H^∞` fields used in the article") and by
   `research/section4/STATEMENTS.md:467` ("Applied only to smooth `H^∞`
   fields"), and no Section 4 statement quantifies over a completion element
   that is not an `H^∞` field. If a later reviewer reads
   `DEPENDENCY_GRAPH.md:199` as demanding the abstract completion as an object,
   that is an eleventh unit (build the closure, prove it equals the realization
   set via the annular density of `appendix-b-embeddings.tex:67-69`) and should
   be booked as such, not folded into U6/U7.
3. **The torus half and Appendix A's `eq:embeddings`** remain out of scope, as
   already stated in `Spec.lean`'s header and §5 item 6.
