...
| `spatialApproxHomogeneous` | :539 | `Section4/B02/AnnularReal.lean:225` (requires `SplitRange s`, matching the field at `Spec.lean:539`; the earlier "(unconditional)" annotation was wrong — finding 5-B) |
...
| 8 | `approxCompactHomogeneous` | :607 | M-L (largest) | `SeparatedCompactHomogeneousDense` glue: `temporalApprox` + `spatialApproxHomogeneous` + `separatedAssembly` (below) | depends on row 6; and `B01`'s `approxCompact` is **monolithic** (not this glue), so cannot be reused/parametrized | **DONE (lane 110): proved as `B02.approxCompactHomogeneous` and the export `B02.SeparatedCompactHomogeneousDense` in the new module `Section4/B02/ApproxCompact.lean`, on `1 ≤ q`, `q ≠ ⊤`, `SplitRange s` — definitionally equal to `Spec.lean:607` (NOT literally token-identical: 3 defeq token diffs — `CompletedDenseHomogeneous` abbrev, `SplitRange`, `forceClassCompact` restatements; `rfl`-checked in the conformance file `research/B02/axioms_approx_compact.lean`, axioms `[propext, Classical.choice, Quot.sound]`). The one new lemma is the realization-independent gluing `bochnerDatumENorm_separatedPath_sub_le` (`04-whole-space.tex:253-255`, content line 254 `Σ|E_j|^{1/q}‖b_j−h_j‖`, NOT `:257`); the three inputs (rows 5,6 + `spatialApproxHomogeneous`) fed in directly, `separatedAssembly`'s `-3/2 < s` via `SplitRange.1`. Route + `ℝ≥0∞` bookkeeping + the `add_le_add_right`-orientation `whnf` loop in `research/B02/ATTEMPTS_APPROX_COMPACT.md`.** |
...
All three inputs exist. The one new piece is the realization-independent triangle-inequality gluing next to `separatedPath`: `bochnerDatumENorm q s (separatedPath φ A - separatedPath φ A') ≤ ∑ j, eLpNorm (φ j) q forceTimeMeasure * ‖A j - A' j‖ₑ` (the manuscript's `04-whole-space.tex:253-255`, content line 254 `Σ|E_j|^{1/q}‖b_j−h_j‖`; the earlier `:257` pointer was to line 258's differing-time-factor variant — finding 2-C), from `eLpNorm_sum_le` and a short `eLpNorm (fun t => φ j t • v) q = ‖v‖ₑ * eLpNorm (φ j) q`; `CompletedDenseVia` does not require `MemBochnerDatum q s D`. Size 90–140 lines; the difficulty is `ℝ≥0∞` bookkeeping (`⊤`/`0`, `J = 0`), not analysis. B01's `approxCompact` cannot be reused. **[DONE lane 110 — see row 8.]**
