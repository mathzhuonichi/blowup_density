...
| 7 | `low_high_split` | **M** | `lowHighSplit`, on `SplitRange`, for `k ∈ L¹ ∩ L²`, in `ℝ≥0∞` and in the angular convention. | `HT:14` `homogeneous_energy_le_bound_add_L2` is the scalar core but splits at *cycles* radius `1`; restate it at angular radius `1` by re-running its two-line majorant argument (`SW:54` for the low half, `Real.rpow_le_one_of_one_le_of_nonpos` for the high half) against `FC:23`; then sum over `Fin 3`, feed unit 4 for `C`, and move `Real.sqrt`/`ℝ` to `ℝ≥0∞`.  **Lane 059 correction:** widening to `L¹ ∩ L²` is *not* free — `Integrable ‖φ‖²` is the Plancherel statement itself; the lane built `angular_plancherel` (`Section4/B02/LowHigh.lean`) and re-ran the majorant in `ℝ≥0∞`. |
