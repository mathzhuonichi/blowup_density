...
**All current nodes are Closed.** Each status belongs to the exact clause written in its node. Closed requires a Lean kernel-checked proof with all auxiliary results formally proved and instantiated. It introduces no assumptions beyond those explicitly stated in the article (for example, positive viscosity). Only the standard logical axioms `propext`, `Classical.choice` and `Quot.sound` are permitted.

**Solid arrows** are dependencies of the proved argument. Repeated nodes in different panels denote the same result.

The periodic argument splits into a construction/density branch and a critical regularity/non-density branch. The whole-space argument has the same structure, with separate low-frequency estimates and two critical norms. Proposition 2.1 also includes the fixed-force H1-uniform restart and endpoint clauses on both domains.
...
flowchart TD
  ...
  L21_LOCAL_FORCE["Proposition 2.1<br/>local smooth forcing<br/>Closed"]
  B11 --> B11_FULL
  CMP --> B11_FULL
  B11 --> E22
  L21T --> L21_H1
  ...
  L32 --> C35_T
  C35_T --> C35_FULL
  L21T --> L21_LOCAL_FORCE
  L21R --> L21_LOCAL_FORCE
  classDef closed fill:#dcfce7,stroke:#15803d,color:#14532d;
  classDef mainResult stroke-width:4px;
  class B11,CMP,B11_FULL,E22,L21T,L21R,L21_H1,V34,C35_LOCAL,L32,C35_T,C35_FULL,L21_LOCAL_FORCE closed;
```
...
All 26 numbered statements and the numbered remark are Closed. There are no split or unfinished article entries in the current inventory.
...
| Proposition 2.1: local smooth forcing | Closed | Local existence with full regularity and pressure recovery, maximal development, uniqueness and squared-H2 continuation beyond the density force classes. Whole-space forces have smooth future Sobolev paths; periodic forces are restrictions of smooth periodic representatives on each finite closed future slab. No global time integrability, temporal support or negative-time regularity of the original force is required. [LocalForce.lean](../../formalization/NSFormalization/Section4/A01/LocalForce.lean); [LocalForceTheory.lean](../../formalization/NSFormalization/Section4/A01/LocalForceTheory.lean); [LocalForceBounds.lean](../../formalization/NSFormalization/Section3/T11/LocalForceBounds.lean); [LocalForceRestart.lean](../../formalization/NSFormalization/Section3/T11/LocalForceRestart.lean); [LocalForceTheory.lean](../../formalization/NSFormalization/Section3/T11/LocalForceTheory.lean); [LocalForceTheory.lean](../../verification/Bindings/LocalForceTheory.lean) |
...
| Proposition 3.17: bounded-domain case | Closed | For every potential smooth on a neighborhood of [0,T) times the closure of an arbitrary nonempty bounded open domain with homogeneous no-slip, the force-velocity pairing vanishes; at positive viscosity every classical solution from rest has zero velocity throughout its lifespan. Neither boundary regularity nor a globally smooth potential is required. [ConservativeOmega.lean](../../formalization/NSFormalization/Section3/T24/ConservativeOmega.lean); [ConservativeForcingV2.lean](../../verification/Bindings/ConservativeForcingV2.lean) |
...
Run `python3 experiments/check_formalization_plan.py` to regenerate this file; `python3 experiments/check_formalization_plan.py --check` verifies coverage agreement, acyclicity, displayed edges, source paths and the rule that a Closed proof cannot depend on an unfinished node.
