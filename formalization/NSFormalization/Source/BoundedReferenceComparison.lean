import NSFormalization.Source.BoundedReferenceFlux

/-! Whole-space comparison for a uniformly bounded noncompact reference. -/
noncomputable section

open Set MeasureTheory
open scoped ContDiff BigOperators InnerProductSpace

namespace NSFormalization.Source.BoundedReferenceComparison

open NavierStokesR3 ProblemStatement Comparison ComparisonCutoffs
...
theorem eq_of_pressure_flux_bound {T M G CP U : ℝ}
    {u v : VelocityField} {p q : PressureField}
    (hT : 0 ≤ T) (hM0 : 0 ≤ M) (hG0 : 0 ≤ G) (hCP0 : 0 ≤ CP)
    (hU0 : 0 ≤ U) (hU : ∀ t ∈ Icc (0 : ℝ) T, ∀ x, ‖u (t,x)‖ ≤ U)
  ...
  have hderiv : 0 ≤ derivativeConstant 1 := (derivativeConstant_pos 1).le
  let Cextra : ℝ := 8 * derivativeConstant 1 * U * M ^ 2
  have hCextra : 0 ≤ Cextra := by dsimp [Cextra]; positivity
  have hwzero : ∀ t ∈ Icc (0 : ℝ) T, ∀ x, (u - v) (t, x) = 0 := by
    apply WholeSpaceEnergyLimit.eq_zero_of_weighted_rate_bound (K := 2 * G) (C := D + Cextra) (R₀ := 0)
      hT (mul_nonneg (by norm_num) hG0) (add_nonneg hD hCextra) (hu.sub hv)
    · intro t ht
    ...
    · intro R hR t ht
      ...
      have hl' : |∫ x : Space, ‖(u - v) (t, x)‖ ^ 2 *
        ...
        calc
          ...
          _ = _ := by ring
      have htflux := (BoundedReferenceFlux.transport_flux_bound
        ((cutoff_smooth R).of_le (by simp)) (cutoff_hasCompactSupport hRpos)
      ...
      have htflux' : |∫ x : Space, ‖(u - v) (t, x)‖ ^ 2 *
          ...
          (8 * derivativeConstant 1 * M ^ (3 / 2 : ℝ)) / R * B ^ (3 / 2 : ℝ) + Cextra / R := by
        apply htflux.trans
        have hfirst : (8 * (derivativeConstant 1 / R)) *
            comparisonLpNorm 2 (fun x => (u-v) (t,x)) ^ (3/2 : ℝ) * B ^ (3/2 : ℝ) ≤
            (8 * (derivativeConstant 1 / R)) * M ^ (3/2 : ℝ) * B ^ (3/2 : ℝ) := by
          apply mul_le_mul_of_nonneg_right _ (Real.rpow_nonneg hB _)
          exact mul_le_mul_of_nonneg_left
            (Real.rpow_le_rpow hm (hM t htcc) (by norm_num)) (by positivity)
        have hsecond : (8 * (derivativeConstant 1 / R) * U) *
            comparisonLpNorm 2 (fun x => (u-v) (t,x)) ^ 2 ≤
            (8 * (derivativeConstant 1 / R) * U) * M ^ 2 :=
          mul_le_mul_of_nonneg_left (pow_le_pow_left₀ hm (hM t htcc) 2) (by positivity)
        exact (add_le_add hfirst hsecond).trans_eq (by dsimp [Cextra]; ring)
      ...
      have hshift := hrate R hR1 A hA B hB hSob
        (weightedEnergy (weight R) (u - v) t)
        (weightedEnergyRate (weight R) (u - v) t - Cextra / R) G
      have henergy : (1 / 2 : ℝ) * (weightedEnergyRate (weight R) (u-v) t - Cextra / R) + A ^ 2 ≤
          G * weightedEnergy (weight R) (u-v) t + C0 / R ^ 2 + C1 / R * B ^ (3/2 : ℝ) +
          CP * pressureEnvelope R A B := by
        rw [hAsq]
        have heq : (1/2 : ℝ) * (weightedEnergyRate (weight R) (u-v) t - Cextra/R) +
            weightedDissipation (weight R) (u-v) t =
            ((1/2 : ℝ) * weightedEnergyRate (weight R) (u-v) t +
              weightedDissipation (weight R) (u-v) t) - (1/2 : ℝ) * (Cextra/R) := by ring
        rw [heq, hbalance]
        ...
        have hshifted := sub_le_sub_right hsum ((1/2 : ℝ) * (Cextra/R))
        convert! hshifted using 1 <;> dsimp only [C0, C1, pressureEnvelope] <;> ring
      have hb := hshift henergy
      change weightedEnergyRate (weight R) (u-v) t ≤
        2 * G * weightedEnergy (weight R) (u-v) t + (D + Cextra) / R
      rw [add_div]
      linarith only [hb]
  intro t ht x
  exact sub_eq_zero.mp (hwzero t ht x)

end NSFormalization.Source.BoundedReferenceComparison
