...
The domain is any nonempty bounded open set. The potential is smooth on an
open neighborhood of the closed spatial slab during the classical lifespan;
no global extension, periodicity or temporal support condition is required.
...
def restSolutionOmega (ν : ℝ) (Ω : Set Space)
    (hΩ : IsOpen Ω ∧ Bornology.IsBounded Ω ∧ Ω.Nonempty) (φ : SpaceTimeScalar)
    (T : ℝ) (hφ : SmoothOnClosedSlab (Ico 0 T) Ω φ) (hT : 0 < T) :
  ...
  pressure_smooth :=
    (show SmoothOnClosedSlab (Ico 0 T) Ω (-φ) from by
      obtain ⟨N, hN, hsub, hs⟩ := hφ
      exact ⟨N, hN, hsub, hs.neg⟩).domainNormalizePressure
  ...
  pressure_gauge := by
    intro t ht
    apply domainNormalizePressure_integral hΩ.1 hΩ.2.1 hΩ.2.2
    exact (hφ.integrableOn_slice hΩ.2.1 ht).neg
...
theorem zero_from_restOmega
    ...
    u.velocity (t, x) = 0 := by
  let v := restSolutionOmega ν Ω hΩ φ T hφ hT
  ...
  have hd (s : ℝ) (hs : s ∈ Ioo 0 b) : HasDerivAt E (-2 * ν * D s) s := by
    have h := difference_energy_identity (ibp_of_isOpen_isBounded hΩ.1 hΩ.2.1) hΩ.1 hΩ.2.1 u v
