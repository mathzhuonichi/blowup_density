...
/-- Physical RHS of the differentiated inhomogeneous H¹ norm. Pressure is
already cancelled in `inhomogeneousEnergyIdentityT`. -/
theorem weightedEnergyIdentityT
    {ν T : ℝ} {a : SpatialField} {f : SpaceTimeField}
    (w : ClassicalSolutionT ν a f T) (hf : f ∈ forceClassT)
    {t : ℝ} (ht : t ∈ Ioo (0 : ℝ) T) :
    HasDerivAt
  ...
  have hb : SmoothPeriodicT (fun x => f (t, x)) :=
    ⟨hf.1.comp (contDiff_const.prodMk contDiff_id), hf.2.1 t (mem_univ t)⟩
  ...
  obtain ⟨F, hF⟩ := exists_force_datum hf.1 hf.2.1 1 t
  obtain ⟨N, hN⟩ := exists_convection_datum w 1 ht'
  have hd := inhomogeneousEnergyIdentityT w hf ht hG hF hN
