...
/-! Terminal gradient energy for genuine regularized heat paths, retaining the trace term. -/
noncomputable section
namespace NSFormalization.Section4.A01.HeatGradientTrace
open MeasureTheory Set EulerLiftedGradientSpace EulerCylinderSobolevSpace
  EulerSobolevLaplacian EulerSobolevHeatGenerator EulerHeatGradientEnergy
open scoped Topology
variable (period : ℝ) [Fact (0 < period)]

/-- The actual heat PDE controls terminal gradient energy and time-integrated dissipation.
The source is measured only by its undifferentiated L² norm in the bound. -/
theorem heat_gradient_trace_dissipation (u : ℝ → SobolevSpace period 3)
    ...
    gradientEnergy period (restrictOperator period (by norm_num : 1 ≤ 3) (u t)) +
  ...
  change G t + ν * (∫ r in s..t, L r) ≤ G s + ν⁻¹ * (∫ r in s..t, F r)
  linarith

/-- Dropping only the nonnegative dissipation gives the terminal gradient trace bound. -/
theorem heat_gradient_trace_bound (u : ℝ → SobolevSpace period 3)
    (f : ℝ → SobolevSpace period 1) (ν s t : ℝ) (hν : 0 < ν) (hst : s ≤ t)
    (hu : ContinuousOn u (Icc s t)) (hf : ContinuousOn f (Icc s t))
    (hd : ∀ r ∈ Ioo s t, ∀ i : Fin 4,
      HasDerivAt (fun a => value period (derivativeOperator period 2 i (u a)))
    ...
    gradientEnergy period (restrictOperator period (by norm_num : 1 ≤ 3) (u t)) ≤
      gradientEnergy period (restrictOperator period (by norm_num : 1 ≤ 3) (u s)) +
        ν⁻¹ * (∫ r in s..t, ‖value period (f r)‖ ^ 2) := by
  have h := heat_gradient_trace_dissipation period u f ν s t hν hst hu hf hd
  have hL : 0 ≤ ∫ r in s..t, ‖laplacianEvaluation period 3 (by norm_num) (u r)‖ ^ 2 :=
    intervalIntegral.integral_nonneg hst (fun _ _ => sq_nonneg _)
  exact (le_add_of_nonneg_right (mul_nonneg hν.le hL)).trans h

end NSFormalization.Section4.A01.HeatGradientTrace
