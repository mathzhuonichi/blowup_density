...
namespace BlowupDensity.Contracts.V2.MultipleRegions
...
def energyGradientOmega (Ω : Set Space) (T : ℝ) (z : SpaceTimeField) : ℝ≥0∞ :=
  (∫⁻ t in Ioo (0 : ℝ) T,
    (eLpNorm (fun x ↦ spatialGradient z t x)
      2 (volume.restrict Ω)) ^ (2 : ℝ)) ^ ((2 : ℝ)⁻¹)

/-- `03-torus.tex:511-535` (revised): bounded-domain twin, over the registered imported packet. -/
structure MultipleRegionsOmegaAPI {ν : ℝ}
    (P : BlowupDensity.Contracts.V1.PacketImportAPI ν) (T : ℝ) (Ω : Set Space) (_hΩ : IsOpen Ω ∧ Bornology.IsBounded Ω ∧ Ω.Nonempty)
  ...
  -- TORUS→Ω: see COMPARISON_OMEGA.md for this domain field.
  placement : Fin N → DomainPlacementData P.toPacketAPI
  ...
  component : ∀ j : Fin N,
    ClassicalSolutionOmega ν Ω (0 : SpatialField)
      (scaledForce P (placement j).x₀ T (ε j)) T
  ...
  component_pin : ∀ j : Fin N,
    (component j).velocity = scaledVelocity P (placement j).x₀ T (ε j) ∧
  ...
  component_force_support : ∀ j : Fin N, ∀ t : ℝ, ∀ x : Space,
    x ∉ Metric.ball (regionCenter j) (regionRadius j) →
      scaledForce P (placement j).x₀ T (ε j) (t, x) = 0
  ...
  assembled_force_formula :
    assembled_force =
      finiteForceSum (fun j ↦ scaledForce P (placement j).x₀ T (ε j))
...
/-- `paper/revised/sections/03-torus.tex:511-535`: existence for each registered
imported packet, prescribed bounded domain, horizon and family of balls.
This definition proves no inhabitant. -/
def multipleRegionsOmegaStatement : Prop :=
  ∀ (ν : ℝ), 0 < ν → ∀ (P : BlowupDensity.Contracts.V1.PacketImportAPI ν),
    -- TORUS→Ω: prescribe the domain and its owner's domain-class witness.
    ...
    ∀ (T : ℝ), 0 < T → ∀ (N : ℕ), 0 < N →
      ∀ (regionCenter : Fin N → Space) (regionRadius : Fin N → ℝ),
        ...
        (∀ j : Fin N, closure (Metric.ball (regionCenter j) (regionRadius j)) ⊆ Ω) →
        Pairwise (fun i j : Fin N ↦
          Disjoint (Metric.ball (regionCenter i) (regionRadius i))
            (Metric.ball (regionCenter j) (regionRadius j))) →
        Nonempty (MultipleRegionsOmegaAPI P T Ω hΩ regionCenter regionRadius)

/-- Proposition 3.16: both the torus and bounded-domain no-slip branches. -/
def multipleRegionsStatementV2 : Prop :=
  V1.multipleRegionsStatement ∧ multipleRegionsOmegaStatement

end BlowupDensity.Contracts.V2.MultipleRegions
