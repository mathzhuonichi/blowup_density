...
/-!
# Contract: critical periodic regularity for small forcing

This is the reconciled T20 contract from `research/T20/Spec.lean:894-1246`.
It reuses the already registered T10/T11 vocabulary from
`Contracts.V1.TorusLocalTheory` and the T12 calculus vocabulary from
`Contracts.V1.MeanZeroCalculus`.  Only the T20-specific definitions below are
restated; `Bindings.CriticalRegularityT` gives an `rfl` drift guard for each
one.  The separately restated `ClassicalSolutionT` structure is transported
field by field in the binding, following the TorusLocalTheory structure
exception.

Provenance: `meanPathT` through `periodicPairing` and the complete 23-field
record are copied token-for-token from the T20 block of the reconciled Spec.
The registered `squaredHTwoIntegralT`, `maximalLifespanT`,
`forceSobolevENormT`, `gradientTensor`, `laplacian`, and membership
predicates are imported rather than restated.
-/

noncomputable section

namespace BlowupDensity.Contracts.V1.CriticalRegularityT
...
/-- Existential statement form of Proposition 3.7. -/
def criticalRegularityStatement : Prop := Nonempty CriticalRegularityTAPI

end BlowupDensity.Contracts.V1.CriticalRegularityT
