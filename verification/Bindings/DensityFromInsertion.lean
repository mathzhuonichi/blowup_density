...
/-- The if direction of Theorem 4.1(ii). -/
theorem breakdownDenseR_zero_of_subcritical :
    ∀ ν T : ℝ, 0 < ν → 0 < T → ∀ q : ℝ≥0∞, (q = 1 ∨ q = 2) →
      ∀ s : ℝ, s < 2 / q.toReal - 3 / 2 → BreakdownDenseR ν 0 T q s := by
  intro ν T hν hT q hq s hs
  exact breakdownDenseR_of_subcritical ν T hν hT q hq s hs 0
