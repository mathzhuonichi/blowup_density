...
- `test_contract_policy.py`: thirteen import-policy, module-selection and article-proof coverage tests.
- `build_changed_lean.py --base-ref <commit>`: CI compilation of changed Lean files;
  an unavailable base after an initial push or history rewrite selects all committed modules.
...
- `check_reader_documents.py`: document labels, citations, coverage markers,
  proof locations and corpus hashes; it does not build PDFs.
