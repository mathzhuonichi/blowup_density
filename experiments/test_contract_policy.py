...
from build_changed_lean import changed_paths, targets
...
class ChangedModuleHistory(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / 'formalization').mkdir()
        (self.root / 'formalization/Old.lean').write_text('-- Old proof\n')
        self.git('add', '.')
        self.git('commit', '-qm', 'Initial snapshot')
        self.base = self.git('rev-parse', 'HEAD').strip()
        (self.root / 'formalization/New.lean').write_text('-- New proof\n')
        self.git('add', '.')
        self.git('commit', '-qm', 'Add proof')
        (self.root / 'formalization/Untracked.lean').write_text('-- Local scratch\n')

    def git(self, *args):
        return subprocess.check_output(['git', *args], cwd=self.root, text=True)

    def test_available_base_selects_only_changed_modules(self):
        self.assertEqual(changed_paths(self.base, root=self.root), ['formalization/New.lean'])

    def test_unavailable_history_selects_all_committed_modules(self):
        for base in ('0' * 40, 'f' * 40):
            with self.subTest(base=base):
                self.assertEqual(changed_paths(base, root=self.root),
                                 ['formalization/New.lean', 'formalization/Old.lean'])
