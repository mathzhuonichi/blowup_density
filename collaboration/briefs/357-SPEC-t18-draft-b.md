# Lane 357-SPEC-t18-draft-b — Section 3 node T18 `thm:insertion` (exact local insertion on the torus): double-blind draft B of the Lean statement

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/357-SPEC-t18-draft-b` (git branch `erenup/357-SPEC-t18-draft-b`, based on `origin/erenup/integration-section3`).
One of two **independent, mutually invisible** drafts (rule 2 of `CLAUDE.md`): **do NOT read** `research/T18/` or `collaboration/briefs/` entries for the other T18 lane (356-SPEC-t18-draft-a).
...
1. `research/T18/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T18/DraftB.lean`, 0 errors). In a namespace
   `BlowupDensity.T18.DraftB`, one structure `PeriodicInsertionAPI` (Type-valued, taking as parameters the packet import, placement, scaling and correction records the paper's
...
2. `research/T18/COMPARISON_B.md`: paper-clause → Lean field table with the R42 counterpart field for each (or "torus-only"); choices; ambiguities; "needs a lemma" list
...
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T18/REPORT_357.md`.
...
a draft is expected to be several hundred lines with a table in `COMPARISON_B.md` covering every clause. No field may be `True`, a tautology, or a definition ignoring its
arguments; print `grep -nE ': *True|:= *0$|→ *True' research/T18/DraftB.lean` (empty) in the report. Spend the time; do not stop early.
