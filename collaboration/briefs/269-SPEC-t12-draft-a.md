# Lane 269-SPEC-t12-draft-a — Section 3 node T12 "mean-zero Sobolev calculus and critical embeddings": double-blind draft A of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/269-SPEC-t12-draft-a` (git branch `erenup/269-SPEC-t12-draft-a`, based on `origin/erenup/integration-section3`). One of two **independent, mutually invisible** drafts
...
1. `research/T12/DraftA.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T12/DraftA.lean`) with a `structure <Name>API : Prop` whose fields are
...
2. `research/T12/COMPARISON_A.md`: paper-clause → Lean field table; choices; ambiguities; "needs a lemma" list; which ℝ³ (Section 4) results are the counterparts (name the Section 4 module/contract).
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T12/REPORT_269.md`.
