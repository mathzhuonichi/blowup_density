# Lane 411-SPEC-t21-draft-b — Section 3 node T21 "main assembly" (`thm:main` (i)+(ii), `cor:nondensity`): double-blind draft B of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/411-SPEC-t21-draft-b` (git branch `erenup/411-SPEC-t21-draft-b`, based on `origin/erenup/integration-section3`).
...
1. `research/T21/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T21/DraftB.lean`, 0 errors). In a
   namespace `BlowupDensity.T21.DraftB`, one structure per result — `NonDensityAPI` (`cor:nondensity`: the explicit open ball in the `L¹_t H^{1/2}` norm, its radius `c ν` with
...
2. `research/T21/COMPARISON_B.md`: paper-clause → Lean field table (with the Section 4 R41 counterpart field or "torus-only"); choices; ambiguities; "needs a lemma" list
...
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T21/REPORT_411.md`.
...
`grep -nE ': *True|:= *0$|→ *True' research/T21/DraftB.lean` (empty) in the report. Spend the time; do not stop early.
