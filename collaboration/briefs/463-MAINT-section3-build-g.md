# Lane 463-MAINT-section3-build-g — full compile check of Section 3 on the integration branch after the #354–#421 merge batch (49 contracts)
...
## Deliverable — `logs/SECTION3_BUILD_20260919g.md` (same sections as the template)
...
4. Gates from the worktree root: `make check`, `make test` (expect all 49 contracts `checked; standard logical axioms only` — record the
   actual count), `make test-mutations`, `python3 experiments/check_contracts.py --base-ref main` (record `registered_contracts` (expect 49) and
...
what is not green (exact text) / commands run and results). Also write it to `research/MAINT/REPORT_463.md`.
