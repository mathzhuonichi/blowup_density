# Lane 264-SPEC-t10-draft-b — Section 3 (torus T³), node T10 "periodic data layer": double-blind draft B of the contract statements (`Contracts/V1/TorusData.lean` to be)

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/264-SPEC-t10-draft-b` (git branch `erenup/264-SPEC-t10-draft-b`, based on `origin/erenup/integration`). This is one of two **independent,
...
1. `research/T10/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T10/DraftB.lean`, 0 errors) with: (a) the coefficient carrier
...
2. `research/T10/COMPARISON_B.md`: paper notion → Lean definition table; every choice (weight normalization `2π`, real vs complex coefficients, how periodicity of the physical field is
...
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T10/REPORT_264.md`.
