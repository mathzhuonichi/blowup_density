# Lane 276-SPEC-t22-draft-b — Section 3 node T22 "bounded-domain norm layer (restriction / zero extension)": double-blind draft B of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/276-SPEC-t22-draft-b` (git branch `erenup/276-SPEC-t22-draft-b`, based on `origin/erenup/integration-section3`). One of two **independent, mutually invisible** drafts
...
1. `research/T22/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T22/DraftB.lean`) with a `structure <Name>API : Prop` (or a
...
2. `research/T22/COMPARISON_B.md`: paper-clause → Lean field table; choices; ambiguities; "needs a lemma" list; which Section 4 results are the counterparts and what can be reused verbatim.
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T22/REPORT_276.md`.
