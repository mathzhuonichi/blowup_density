# Lane 307-SPEC-t24-draft-b — Section 3 node T24 "further constructions" (`prop:affine`, `prop:multiple`, `prop:conservative`): double-blind draft B of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/307-SPEC-t24-draft-b` (git branch `erenup/307-SPEC-t24-draft-b`, based on `origin/erenup/integration-section3`). One of two **independent, mutually invisible** drafts (rule 2 of `CLAUDE.md`):
...
1. `research/T24/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T24/DraftB.lean`). Import `Contracts.V1.TorusData`
   ...
   with source lines) only the still-unregistered declarations you use. Then, in a namespace `BlowupDensity.T24.DraftB`, state three separate structures `AffineVariationAPI`,
...
2. `research/T24/COMPARISON_B.md`: paper-clause → Lean field table (with Section 4 counterpart fields where they exist); choices; ambiguities; "needs a lemma" list; implementation candidates.
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T24/REPORT_307.md`.
