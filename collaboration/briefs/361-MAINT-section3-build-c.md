# Lane 361-MAINT-section3-build-c — full compile check of Section 3 on the integration branch after the #317–#325 merge batch

You are a Lean 4 (v4.34.0-rc2 + Mathlib) build-verification worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/361-MAINT-section3-build-c` (git branch `erenup/361-MAINT-section3-build-c`, based on
`origin/erenup/integration-section3` after the merges #317–#325 (T13 `ConstantEndpoints`/`TorusIdentity`/`WholeSpaceIdentity`, T14 `PacketEnergy` + contract
`T01.packet_import` (40 contracts), T16 `LocalPotential`/`BallPotential`, T24 spec). Read `CLAUDE.md` and the previous report
**`logs/SECTION3_BUILD_20260918b.md`** (reproduce its structure and tables exactly; it is the template).
...
## Deliverable 1 — `logs/SECTION3_BUILD_20260918c.md` (same sections as the template)
1. Environment table (commit checked = `git rev-parse HEAD`, toolchain, threads).
2. Module inventory: `find formalization/NSFormalization/Section3 -name '*.lean' | sort` (expect 49: T10 7, T11 30, T12 4, T13 4, T14 1, T16 2 — count them, do not trust this
   expectation) and per-directory counts.
...
4. Gates from the worktree root: `make check`, `make test` (expect all 40 contracts `checked; standard logical axioms only` — record the
   actual count), `make test-mutations`, `python3 experiments/check_contracts.py --base-ref main` (record `registered_contracts` (expect 40) and
...
what is not green (exact text) / commands run and results). Also write it to `research/MAINT/REPORT_361.md`.
