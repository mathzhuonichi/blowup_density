# Lane 380-MAINT-section3-build-d — full compile check of Section 3 on the integration branch after the #328–#337 merge batch

You are a Lean 4 (v4.34.0-rc2 + Mathlib) build-verification worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/380-MAINT-section3-build-d` (git branch `erenup/380-MAINT-section3-build-d`, based on
`origin/erenup/integration-section3` after the merges #328–#337 (T16 `LatticeLift`/`Assembly` + contract `T02.local_potential` (41 contracts), T13 `KernelComparison`,
T15 `ParsevalZero`/`HaarBridge`/`Bridges`, T12 `Cutoff`, T17 `CorrectionProfile`, T18 spec). Read `CLAUDE.md` and the previous report
**`logs/SECTION3_BUILD_20260918c.md`** (reproduce its structure and tables exactly; it is the template).
...
## Deliverable 1 — `logs/SECTION3_BUILD_20260918d.md` (same sections as the template)
1. Environment table (commit checked = `git rev-parse HEAD`, toolchain, threads).
2. Module inventory: `find formalization/NSFormalization/Section3 -name '*.lean' | sort` (expect about 60: T10 7, T11 30, T12 5, T13 6, T14 1, T15 3, T16 4, T17 1 — count them, do not
   trust this expectation) and per-directory counts.
...
4. Gates from the worktree root: `make check`, `make test` (expect all 41 contracts `checked; standard logical axioms only` — record the
   actual count), `make test-mutations`, `python3 experiments/check_contracts.py --base-ref main` (record `registered_contracts` (expect 41) and
...
what is not green (exact text) / commands run and results). Also write it to `research/MAINT/REPORT_380.md`.
