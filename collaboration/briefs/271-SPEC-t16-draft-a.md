# Lane 271-SPEC-t16-draft-a — Section 3 node T16 "local divergence-free cutoff (lem:potential)": double-blind draft A of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/271-SPEC-t16-draft-a` (git branch `erenup/271-SPEC-t16-draft-a`, based on `origin/erenup/integration-section3`). One of two **independent, mutually invisible** drafts
...
1. `research/T16/DraftA.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T16/DraftA.lean`) with a `structure <Name>API : Prop` whose fields are
...
2. `research/T16/COMPARISON_A.md`: paper-clause → Lean field table; choices; ambiguities; "needs a lemma" list; which ℝ³ (Section 4) results are the counterparts (name the Section 4 module/contract).
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T16/REPORT_271.md`.
