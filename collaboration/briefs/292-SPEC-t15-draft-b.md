# Lane 292-SPEC-t15-draft-b — Section 3 node T15 "scaling at fixed viscosity" (`prop:scaling`): double-blind draft B of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/292-SPEC-t15-draft-b` (git branch `erenup/292-SPEC-t15-draft-b`, based on `origin/erenup/integration-section3`). One of two **independent, mutually
...
1. `research/T15/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T15/DraftB.lean`).
   ...
   registered`) only the declarations you use. Then, in a namespace `BlowupDensity.T15.DraftB`, state:
...
2. `research/T15/COMPARISON_B.md`: paper-clause → Lean field table (with the `I03.scaling` counterpart field for each); choices (periodization
   rendering, `ε` range, which norms are `ℝ≥0∞`); ambiguities; "needs a lemma" list; implementation candidates in `Paper1/`.
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T15/REPORT_292.md`.
