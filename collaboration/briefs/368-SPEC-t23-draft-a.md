# Lane 368-SPEC-t23-draft-a — Section 3 node T23 (`cor:boundary`, interior no-slip insertion on a bounded domain): double-blind draft A of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/368-SPEC-t23-draft-a` (git branch `erenup/368-SPEC-t23-draft-a`, based on `origin/erenup/integration-section3`).
...
1. `research/T23/DraftA.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T23/DraftA.lean`, 0 errors). In a
   namespace `BlowupDensity.T23.DraftA`, one structure per result — `BoundaryInsertionAPI` (cor:boundary; the hypotheses on `Ω` as parameters, the no-slip uniqueness and collar clauses as fields) — each field exactly one clause of the paper's statement (hypotheses and conclusions each
...
2. `research/T23/COMPARISON_A.md`: paper-clause → Lean field table (with the Section 4 counterpart field or "torus-only"); choices; ambiguities; "needs a lemma" list
...
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T23/REPORT_368.md`.
...
`grep -nE ': *True|:= *0$|→ *True' research/T23/DraftA.lean` (empty) in the report. Spend the time; do not stop early.
