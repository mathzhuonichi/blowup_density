...
Continuation (U11: Ccriterion, Ccriterion_pos), GlobalRegularity (U12)}.lean`. **U5 `constantTransportCommutesLambda` is now proved by lane 452 in `Section3/T20/TransportLambda.lean` (merged into this worktree; read `research/T20/REPORT_452.md`) — use it as the field value.** Earlier note: U5 was never needed by any unit (U8/U10b run on the Fourier side) — check whether the
