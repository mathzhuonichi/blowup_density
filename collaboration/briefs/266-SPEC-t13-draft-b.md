# Lane 266-SPEC-t13-draft-b — Section 3 node T13 "uniform localization of fractional norms" (lem:localization): double-blind draft B of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/266-SPEC-t13-draft-b` (git branch `erenup/266-SPEC-t13-draft-b`, based on `origin/erenup/integration`). One of two **independent, mutually
...
1. `research/T13/DraftB.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T13/DraftB.lean`) with: the Gagliardo double integral `I_R s f`
...
2. `research/T13/COMPARISON_B.md`: paper-clause → Lean field table; choices (how the periodization/support-in-a-ball hypothesis is rendered; scalar vs vector; the exact constant
   normalization); ambiguities; "needs a lemma" list.
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T13/REPORT_266.md`.
