# Lane 294-SPEC-t17-draft-a — Section 3 node T17 "bounds for the background correction" (`lem:correction`): double-blind draft A of the Lean statements

You are a Lean 4 (v4.34.0-rc2 + Mathlib) **specification** worker on the repository checked out at your working directory
`/data_8T/ping/blowup_density/.claude/worktrees/294-SPEC-t17-draft-a` (git branch `erenup/294-SPEC-t17-draft-a`, based on `origin/erenup/integration-section3`). One of two **independent, mutually
...
1. `research/T17/DraftA.lean` (create the directory): a file that **elaborates** (`cd verification && lake env lean ../research/T17/DraftA.lean`).
   ...
   declarations you use. Then, in a namespace `BlowupDensity.T17.DraftA`, state: the correction force `H_ε` of `eq:H` as an explicit `def` from the cutoff data
...
2. `research/T17/COMPARISON_A.md`: paper-clause → Lean field table (with the `I02.*` counterpart field for each); choices; ambiguities; "needs a lemma" list;
   implementation candidates in `Paper1/`.
3. Commit on your branch (never push/merge/rebase). Report in four parts; write it to `research/T17/REPORT_294.md`.
